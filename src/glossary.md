...
## Manager Matching
Gitea 按 owner scope 和 repository tag 匹配可领取 create operation 的 Manager。

<span id="manager-capacity"></span>
## Manager Capacity
Manager 最近上报的本地 create/resume 可接收能力快照，用于展示、诊断和 FetchOperation 准入检查。
...
Gitea 后台任务处理 operation 超时、stale report、状态分歧和清理。

<span id="stale-report"></span>
## Stale Report
Manager 上报的 `codespace_uuid`、`operation_id`、`manager_id` 或 operation status 已不匹配当前 codespace 状态的过期上报。
...
## Runtime Inventory
Manager 通过 `ReportInstances` 上报的本地 Runtime Instance snapshot，用于 Gitea 计算 expected/reported 差异。
