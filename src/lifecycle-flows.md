...
- 数据库事务提交后，公共入口释放 repository 和 owner lock，再调用 `cleanupDeletedRepository(plan)` 执行既有文件清理。repository 与 Codespace 关系已经由数据库事务确定，文件清理不参与并发判定且可能耗时，因此放在锁外执行。清理失败继续使用 Gitea 现有日志和 system notice，不回滚已提交的 repository 删除，也不增加补偿队列。
...
删除服务需要串行化时直接调用 Gitea `globallock.Lock`，不实现 Codespace Locker 或 mutex pool。完整顺序是 `codespace_manager_addresses`、按 `owner_id` 升序的 `codespace_owner_{owner_id}`、按 `repository_id` 升序的 `repo_working_{repo_id}`、按 `manager_id` 升序的 `codespace_manager_{manager_id}`、按完整小写 UUID 升序的 `codespace_{uuid}`；同层 key 先去重，每条路径只取得所需层级但不能逆序，完成或中途失败时逆序释放。Declare 地址变更使用地址、Manager lock；owner-scoped Manager 删除使用 owner、Manager 和全部相关 Codespace lock。普通同时涉及 Manager 与 Codespace 的写 RPC 仍按 Manager、Codespace 顺序。入口在事务前取得所需 lock 并调用不重复加锁的内部实现；token、日志和 cache 写入都在 Codespace lock 内重新确认记录和 binding。事务提交后先释放 lock，再尽力清理 cache；数据库记录已经不存在，后续请求取得 lock 后重新查询也不能重建资源。持锁期间不调用 Manager、Gateway 或其他网络服务。
...
- repository 文件 cleanup 沿用 Gitea 现有提交后清理语义；Codespace 不增加同名 repository 重建锁、墓碑或补偿任务。
