...
**设计如此：devcontainer 只是内置脚本中的一种实现。**Manager 和 Gitea 的状态、RPC、当前快照与 Endpoint API 都不保存 devcontainer 运行方式、容器标识、容器用户或逻辑端口；自定义脚本只要提供相同 workspace、Git 本地配置、internal SSH 和实际 Endpoint 端口，就可以替换默认实现。
...
create 脚本取得两种规范 clone URL、首选协议和锁定 commit SHA，输出最终 workspace 路径。Manager 只在该目录 HEAD 等于锁定 SHA，且实际 remote 的本地凭据配置有效时接受 prepare。resume 脚本不取得 repository payload，使用已保存 workspace、实际 remote 和共享环境恢复本地凭据与交互入口，并保留用户当前 HEAD。默认脚本的临时目录、协议回退、Git SSH 密钥和 devcontainer 行为集中定义在[内置脚本实现](builtin-scripts.md#内置脚本实现)。
...
内置脚本默认支持直接运行和 devcontainer，但两者只属于脚本内部。脚本通过 `CODESPACE_ENV` 保存自己的实现选择、容器信息和转发状态，并最终输出同一组通用 workspace 与 internal SSH 变量。Manager 只保存规范共享环境，不解析其中的实现私有变量。

Endpoint helper 始终向 Manager 提交 Incus 通信地址上已经可以访问的实际端口。内置 devcontainer 脚本需要先在实例内建立转发，再登记实际端口；自定义脚本使用相同规则。Manager 的路由只保存该实际端口，容器逻辑端口与内部转发由脚本保存和恢复。

**设计如此：脚本实现可以变化，Incus 实例边界保持不变。**stop、delete 和 inventory 只管理 Incus 实例；内部环境的安装、恢复、端口转发和用户选择由当前脚本负责。默认直接运行/devcontainer 的具体行为和错误分类见[内置脚本实现](builtin-scripts.md#内置脚本实现)。
...
具体包安装、Git 和内部环境子步骤写入 operation 日志和失败摘要，不扩充持久状态枚举。Manager 只在前一阶段成功后推进，同一 operation 版本的阶段不回退；`ready` 成立后保持 `ready`。
...
Resume 使用已有 workspace：Manager 持久化并领取 resume payload 后启动 Runtime，先以 root 执行幂等 `init.sh`，再调用 `RequestGiteaToken` 取得新 Token 和 `server_url`、原子替换两种 Token 文件，随后依次执行 `resume.sh prepare` 和 `resume.sh activate`。prepare 从 `CODESPACE_ENV` 恢复脚本私有状态，验证 workspace 和实际 remote 的本地凭据配置；activate 恢复通用 internal SSH 输出和脚本负责的内部环境，不读取 repository payload，也不修改当前 HEAD。Manager 校验结果文件、规范共享环境、internal SSH 和 workspace 路由后推进到 `ready`。唯一 metadata 发布任务确认 Gitea 已接受包含本次 resume ready 的快照后，Manager 调用 `FinalizeOperation(final done)`。Gitea 只在 ready metadata 和 Token 行完整时写入 `running` 并清空 operation；实际 remote 协议和对应凭据由 Manager 在 ready 前验证。旧 operation 版本的 ready 不能完成本次 resume。final accepted 或幂等完成后，Manager 在本地协调锁内复检当前 operation 和 cleanup，再开放 session 准入。
...
- 内置脚本的 apt/dnf/pacman、固定用户、直接运行/devcontainer、内部转发和 lifecycle commands 由独立文档的实现验收点覆盖；Manager 核心测试只验证通用脚本契约和输出。
