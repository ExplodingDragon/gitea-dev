...
create payload 使用领取时重新读取的 repository 名称生成 HTTP(S)、SSH 和 Web URL。`git_protocol` 只表示首次 clone 的首选协议；不可用协议的 URL 为空。Manager 不从 Fetch 请求 Host 推导外部地址。
...
bootstrap 只处理外层系统和 workspace。clone 使用带 Codespace UUID 的临时目录，首选协议失败且另一个 URL 可用时在同一次 bootstrap 中清理并回退一次；锁定 commit 校验成功后原子移动到最终目录。它输出实际 UID/GID、用户和 workspace，Manager 随后把这些值与 create startup input 持久化。
...
**设计如此：**workspace 的一次性 clone 与 Dev Container 的可恢复环境分开。create 重试可以识别同 Codespace 标签的未完成 Docker 对象并清理重建，但不会重复提交一个已经完成的 workspace，也不会接管其他对象。
