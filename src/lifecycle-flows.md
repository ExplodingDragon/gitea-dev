...
Endpoint helper 始终向 Manager 提交 Incus 通信地址上已经可以访问的实际端口和明确的 `public` 布尔值。helper 默认要求认证，只有显式 `--public` 才公开普通 Endpoint；`workspace` 固定要求认证。**设计如此：持有 Runtime Token 的工作环境进程是 Endpoint 声明主体，`--public` 是 Runtime 提交的访问方式，不表示 Gitea 页面再次确认。**内置 devcontainer 脚本需要先在实例内建立转发，再登记实际端口；自定义脚本使用相同规则。Manager 的路由只保存该实际端口与访问方式，容器逻辑端口与内部转发由脚本保存和恢复。
