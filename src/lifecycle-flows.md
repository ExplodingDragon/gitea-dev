...
根配置优先作为默认候选；一级子目录中的配置分别作为独立环境，不相互融合。仓库没有配置时仍提供平台默认项，它只包含创建时的 `DEVCONTAINER_DEFAULT_IMAGE`。用户确认页展示所选来源、ref、运行环境、配置名称、当前可用 Secret 名称、尚未满足的 Secret 建议和附加仓库权限；Gitea 保存来源、路径、完整提交和原始文件 SHA256，RPC 不携带 JSONC 正文。
