...
- Dev Container 候选从创建时锁定的 PR head commit 读取；用户在确认页明确选择其中一份。
...
### Dev Container 配置与权限确认

Gitea 从创建时锁定的 commit 发现 `.devcontainer/devcontainer.json`、`.devcontainer.json` 和 `.devcontainer/*/devcontainer.json`。多份文件是候选开发环境，每次创建只选择一份。规范根配置存在时默认选中根配置；仓库没有配置时使用平台默认项，平台默认镜像来自 `DEVCONTAINER_DEFAULT_IMAGE`。

文件使用 JSONC，Gitea 通过 `tailscale/hujson` 处理注释和尾随逗号。Gitea 只读取展示名称和 `customizations.gitea.repositories`：仓库名使用精确 `owner/repository`，最多 32 个仓库、合计 128 条规则；单元为 `code / issues / pulls / wiki / releases / actions`，级别为 `read / write`。源仓库能力仍由创建者当前权限自动派生。

创建确认页展示所选 Dev Container 和每条附加仓库权限。用户可以把 `write` 降为 `read` 或 `none`，把 `read` 降为 `none`。Gitea 在确认时重新锁定 ref、重新发现所选文件并计算原始字节 SHA256；确认哈希包含配置来源、路径、提交、摘要、平台默认镜像和权限。哈希变化时返回当前确认页，不创建 Codespace。

仓库配置的路径、创建提交和 SHA256 保存到 Codespace；平台默认项保存当时的默认镜像。create payload 只携带这组类型化选择，不携带 JSONC 正文或 Gitea 授权记录。Manager clone 后复检所选文件，再由官方 Dev Container CLI 解释标准字段、Features、镜像元数据和 Compose。**设计如此：**Gitea 只解析自己负责的权限扩展，运行配置语义由 Dev Container 标准工具负责，两侧不会形成两套配置合并规则。

`environment_tag` 当前由 Gitea 固定为 `default`。仓库 Dev Container 文件可以定义内部开发环境，但不选择 Manager 或 Incus 资源模板；基础设施容量仍由部署管理员声明。

实现验收点：

- 创建确认页展示源仓库、锁定 ref、全部候选 Dev Container、当前选择和每条附加仓库单元权限。
- JSONC 注释和尾随逗号可用；文件过大、候选过多、配置语法或权限规则错误时在创建记录前返回明确错误。
- 确认前没有 Codespace、授权或 operation 数据写入；选择、内容或权限变化时要求重新审阅。
- 创建成功后 `commit_sha` 固定初始 workspace 代码，Dev Container 路径、提交和摘要固定已审阅的仓库配置；平台默认项固定当时镜像。
- 相同用户、源仓库、申请摘要和相同授权级别可以复用授权；已经降权或撤销的授权不会被新创建流程自动提升。
- Manager 只收到一个类型化 Dev Container 选择，协议和本地 state 不包含 Gitea 附加仓库授权副本或配置正文。
...
`user_id` 表达 Manager 管理边界，tag 表达部署侧运行能力。站点全局 Manager 用于站点级容量，个人 Manager 用于该用户自有容量。当前 create 固定匹配 `default`，仓库 Dev Container 配置不参与 Manager 选择。站点全局 Manager 的 `default` 环境适用于站点范围内有权创建 Codespace 的仓库代码；个人 Manager 的 `default` 环境适用于该用户有权读取的仓库代码。需要不同信任边界的环境由部署管理员使用独立 Manager 身份管理。

**设计如此：仓库定义内部开发环境，部署方定义外层资源。**Dev Container 可以影响容器内工具链和生命周期命令，但不能改变 Incus 实例类型、profile、设备或 Manager 范围。这个边界让仓库继续使用标准配置表达开发需求，同时避免为 tag 增加仓库级 ACL。
...
脚本可以使用 Manager 内置版本，也可以由部署方通过本地配置替换。Manager 核心固定消费 init、start、stop、共享环境和通用输出；内置套件在 Incus 实例内准备 Docker 和固定版本的官方 Dev Container CLI，并把选中的单份配置交给 CLI。完整契约和当前内置实现见[脚本契约与内置 Dev Container 实现](builtin-scripts.md)。

**设计如此：Dev Container 是唯一的仓库级开发环境。**Gitea 固定用户选择的配置来源、路径、提交和摘要，Manager 在 clone 后复检对应文件，再由官方 CLI 处理规范中的 Feature、Compose、变量替换和继承。这样仓库无需维护第二份 Codespace 专用配置，Gitea 和 Manager 也不重复实现 Dev Container 规范。部署方替换脚本时仍需产出内部容器标识、用户、工作目录和平台 Web IDE 端口，使 Gateway 的 shell、exec 与 code-server 代理使用同一运行目标。
...
create 的 `init.sh` 取得当前可用协议的规范 clone URL、首选协议和锁定 commit SHA，输出最终 workspace 路径。Manager 只在该目录 HEAD 等于锁定 SHA，且实际 remote 的本地凭据配置有效时接受 create 初始化。随后 `start.sh` 校验创建时固定的 Dev Container 选择并执行 `devcontainer up`；resume 不取得 repository payload，只从 Manager 本地 state 恢复同一选择、workspace 和凭据，再执行相同的 `start.sh`，并保留用户当前 HEAD。默认脚本的临时目录、协议回退、凭据安装与 Dev Container 启停行为集中定义在[内置脚本实现](builtin-scripts.md#内置实现)。
...
内置脚本固定在 Incus 实例内管理一个 Dev Container，并通过 `CODESPACE_ENV` 保存容器标识、内部用户和内部工作目录。完整自定义套件可以保存自身实现所需的附加状态，但必须输出同一组 workspace 和内部容器目标。Manager 只解析统一输出，不解释 Dev Container 配置正文或 CLI 内部元数据。

Dev Container 配置先用 `appPort`、Compose `ports` 或等价运行参数把服务发布到外层实例端口；Endpoint helper 再把同一端口和明确的 `public` 布尔值写入共享 manifest。Manager 读取 manifest 后建立从 Gateway 到该实例端口的受控路由。helper 默认要求认证，只有显式 `--public` 才公开普通 Endpoint；`workspace` 固定要求认证。`forwardPorts` 由官方 CLI 按规范处理，但不自动创建 Gitea Endpoint。**设计如此：写入共享 manifest 的开发环境进程是 Endpoint 声明主体，Gitea 只展示 Manager 已确认可路由的结果，并在访问时复检生命周期和权限。**

**设计如此：外层 Incus 实例负责隔离和持久化，内部 Dev Container 负责开发环境。**stop 先停止内部容器再停止外层实例；resume 启动外层实例后以同一个 `start.sh` 恢复内部容器；delete 和 inventory 仍以外层实例为资源边界。这个分工让 VM 与系统容器使用一致流程，同时让仓库配置保持 Dev Container 标准语义。
...
flowchart TB
    ...
    start[start.sh<br/>启动 Dev Container 并恢复凭据]
    validate[校验容器目标、本地凭据配置<br/>workspace 与 Endpoint 路由]
...
- 内部 Dev Container 正在运行，固定 code-server 端口的 `/healthz` 返回成功。
...
- Manager 已建立固定 `workspace` Web IDE 路由，并确认其指向当前 Dev Container 的 code-server；`workspace` 是平台保留入口，不由 Runtime Endpoint 声明覆盖。
...
- repository/ref/commit/Dev Container 前置失败不创建对象；来源数据完整但没有声明 `default` 环境的可用 Manager 时形成可查看日志的 failed 对象，且不创建 active operation。
- Gitea 使用 hujson 读取规范位置的 JSONC，把多份文件列为独立候选并只选择一份；根配置优先成为默认选择，没有根配置时平台默认项保持默认。候选之间不融合，标准字段由官方 Dev Container CLI 解释。
- 仓库不能选择 Manager tag。Gitea 固定使用 `default` 运行环境，Manager 的 tag 只描述部署侧环境能力，不承担仓库授权或信任等级判断。
...
- Gitea 数据库保存所选 Dev Container 的来源、路径、锁定提交、原始文件 SHA256 或平台默认镜像，不保存 JSONC 正文；附加仓库权限在创建确认时单独授权。
- create payload 提供固定的 `default` 运行环境、本次首选 `git_protocol`、当前可用协议的规范 clone URL、创建者身份、Dev Container 不可变选择，以及 Runtime 初始化所需的 repository、owner、创建者和 ref 数据；Web URL 基于配置的 `ROOT_URL` 并保留 `AppSubURL`，所有 URL 都不受 Manager 控制面请求 Host 影响。
- Manager 领取 create 后把创建者身份、派生后的运行用户名和 Dev Container 选择保存到本地 state；resume 只从本地 state 恢复这些初始化输入，不要求 Gitea 再次下发 repository 或用户资料。
...
- create/resume 都仅在当前 operation 版本的 `ready` 快照、Token 行、本地实际 remote 凭据配置、外层 Incus API、内部 Dev Container、workspace 路由和 Endpoint 路由完整后 final done；普通 Endpoint 服务内容和用户进程不阻塞 ready。resume 顺序为 `启动外层实例 -> 写入 seed -> start 内部容器 -> ready -> running`。
...
- 内置脚本的 apt/dnf/pacman、固定外层用户、helper、Dev Container CLI 和内部容器恢复由独立文档的实现验收点覆盖。Manager 核心测试通过精简自定义脚本覆盖相同的结果契约，真实内置 E2E 覆盖 `init -> start -> stop -> start`。
