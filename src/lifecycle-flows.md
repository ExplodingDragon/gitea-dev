...
credential-refresh 在 active operation 已清空后仍属于已绑定 Codespace 的恢复工作，Manager online 或处于有效 recovering 窗口时可继续申请 token。期间 Manager 被 disabled 时，取消后置 worker、停止 Runtime 并上报 stopped，停止确认失败时上报 failed；Manager 已派生 offline 时先 Declare recovering；Codespace 或 Manager 记录已删除时停止通信但不据此清理 Runtime。
...
- credential-refresh 可在 recovering 且无 active operation 时恢复；disabled 收敛到 stopped/failed，offline 先 Declare recovering，记录删除只停止通信。
...
- `DeleteRepositoryDirectly` 是取得锁的公共入口：先读取当前 `owner_id`，取得当前 owner lock，再取得 `modules/repository.WorkingLockKey(repoID)` 返回的 repository lock，并在锁内重新读取 repository；owner 已变化时释放并按新 owner 重试。所有上层单仓库删除入口复用该入口，不增加 repository Locker 或加锁 wrapper。
- 锁内调用 `deleteRepositoryDBLocked`。该内部函数自己开启数据库事务，在删除 repository row 前执行 `UPDATE codespace SET repo_id=0 WHERE repo_id=?`，完成现有 repository 数据库删除，并返回 `RepositoryCleanupPlan`；任一数据库步骤失败时整个事务回滚。
...
- 数据库事务提交后，公共入口释放 repository 和 owner lock，再调用 `cleanupDeletedRepository(plan)` 执行既有文件清理。repository 与 Codespace 关系已经由数据库事务确定，文件清理不参与并发判定且可能耗时，因此放在锁外执行。清理失败继续使用对应 Gitea 删除入口现有的日志、system notice 和错误返回方式；某个入口可以在数据库已经提交后向 caller 返回文件清理错误，但不能回滚或恢复已删除的 repository、owner 或 Codespace 关系，也不增加补偿队列。
...
- `deleteRepositoryDBLocked` 只在调用者已经持有对应 owner 与 repository lock 时使用；owner purge 逐个取得 repository lock 后可直接调用它，避免公共入口重复取得 owner lock 或开启嵌套事务。
...
用户或组织普通删除只在现有业务前置条件通过后进入 owner lock。删除服务在锁内收集该 owner 的 Manager ID，按 ID 升序取得对应 Manager lock，再收集三条关系命中的 Codespace UUID 并按序取得 Codespace lock；随后开启最终事务重新检查现有删除前置条件和当前关系集合，同一事务调用 `DeleteOwnerCodespaceResourcesLocked` 并删除用户或组织记录。提交并释放全部 lock 后，再执行 Codespace cache 与已有事务后文件清理。owner/Manager/Codespace lock 使 Codespace 创建、Manager 注册/删除和 Fetch claim 与删除形成明确先后；repository、package、membership 等其他 Gitea 写入仍由现有删除服务的最终检查和事务语义处理。
...
purge 保留 Gitea 现有分阶段提交语义，不把整个清理包装成一个长事务：

- 用户 purge 取得用户 owner lock，先在短事务中禁用用户并提交；随后在另一个短事务中调用 `DeleteOwnerCodespaceResourcesLocked`，删除当前通过创建者、repository owner 或 Manager owner 关系关联的 Codespace、Manager、registration token、token 和日志。
- 用户自己的 repository 按 ID 升序逐个处理：每次只取得一个 repository lock，调用 `deleteRepositoryDBLocked` 提交该 repository 的数据库删除，释放 repository lock，并累计 `RepositoryCleanupPlan`。全部 repository 数据库删除完成后释放用户 owner lock，再逐个执行 repository 与 Codespace 的锁外清理。
...
- 组织 purge 取得组织 owner lock，先在独立短事务中调用 `DeleteOwnerCodespaceResourcesLocked`；再按 repository ID 升序逐个取得 repository lock、提交数据库删除、释放 repository lock，并累计 `RepositoryCleanupPlan`。repository 数据库删除完成后，在仍持有 owner lock 的最终短事务中重检 repository/package 等剩余前置条件并删除组织记录；提交并释放 owner lock 后执行所有 repository 与 Codespace 锁外清理。
- `DeleteOrganization` 的外层事务不再嵌套调用 repository 删除。repository 数据库删除使用上述逐仓库短事务，避免内部事务提交后又被外层事务误认为可以整体回滚。
- 删除服务在 owner lock 下先收集候选 Manager ID 并按 ID 升序取得 Manager lock，再查询通过创建者、repository owner 或 Manager owner 三条关系关联的 Codespace UUID，按完整 UUID 升序取得 Codespace lock；所有应用锁均在数据库事务前取得。`DeleteOwnerCodespaceResourcesLocked` 接收调用者的事务 context，在事务内重新查询当前集合并删除，不自行开启嵌套事务。repository 删除、实际 transfer、`CreateCodespace` 记录插入、Manager 注册和 registration token 轮换参与同一 owner lock；Fetch 在对应 Manager lock 内完成 claim 或条件释放。因此最终查询要么看到 claim 后的 binding 并删除它，要么先完成删除使后续 claim 找不到 Manager 或候选记录。
- 前置清理删除这些 Codespace 的 access token、token 明文 binding、DBFS 日志与数据库记录，同时删除该 owner 的全部 Manager 和 registration token，并把 cache 与已有事务后文件清理所需输入返回给调用者。普通删除在最终事务提交后执行；purge 与 `RepositoryCleanupPlan` 一起累计到 owner lock 释放后执行。清理失败只记录现有日志，不恢复已经删除的 Gitea 数据，也不创建持久任务。
...
这里的删除边界是明确设计，不是待补偿的数据不同步缺陷。删除完成条件是 Gitea owner 前置清理事务已经清除关联记录、凭据和日志；Manager 是否在线、是否收到请求、是否删除 Runtime 都不属于该结果。删除后，Manager 身份失效或 Codespace UUID 不再存在，旧 Manager 无法通过认证和 binding 校验改写 Gitea；其他 Manager 上报未知 UUID 时也不会收到破坏性指令。因此设计不增加分布式删除确认、等待状态、重试队列、删除墓碑或 orphan Runtime 回收任务。用户在确认删除时明确接受运行侧可能保留 Runtime、volume 或本地快照，其后续处理属于 Manager 部署运维。

### Manager 删除

Manager 删除是 Gitea 侧同步管理操作。个人 owner、组织管理员或站点管理员提交删除并确认影响范围后，服务先按固定顺序取得所需 owner lock 和目标 Manager lock，在锁内重新查询全部绑定 Codespace，取得对应 Codespace lock，再进入一个本地事务：

1. 重新查询 `manager_id` 当前仍绑定的全部 Codespace，以事务内结果为准。
2. 删除这些 Codespace 的 access token、明文 binding、DBFS 日志元数据和数据库记录。
3. 删除 Manager 记录及其 secret verifier。
4. 提交事务并逆序释放所持 `globallock`。
5. 锁外尽力清除相关 Runtime Metadata 和未消费 open code cache；清理失败记录服务端日志，不改变删除结果。

删除服务需要串行化时直接调用 Gitea `globallock.Lock`，不实现 Codespace Locker 或 mutex pool。完整顺序是 `codespace_manager_addresses`、按 `owner_id` 升序的 `codespace_owner_{owner_id}`、按 `repository_id` 升序的 `repo_working_{repo_id}`、按 `manager_id` 升序的 `codespace_manager_{manager_id}`、按完整小写 UUID 升序的 `codespace_{uuid}`；同层 key 先去重，每条路径只取得 [数据模型锁矩阵](data-model.md#gitea-cache-与-keyed-lock) 指定的层级且不能逆序，完成或中途失败时逆序释放。Fetch 和 ReportInstances 持有 Manager 后按需取得 Codespace；单 Codespace command RPC 只取得 Codespace lock，并在锁内事务中重新确认 Manager 记录、binding 和版本，因此不会形成 Codespace 反向等待 Manager 的锁环。入口在事务前取得所需 lock 并调用不重复加锁的内部实现；token、日志和 cache 写入都在 Codespace lock 内重新确认记录和 binding。事务提交后先释放 lock，再尽力清理 cache；数据库记录已经不存在，后续请求取得 lock 后重新查询也不能重建资源。持锁期间不调用 Manager、Gateway 或其他网络服务。
...
- 用户或组织删除在任何 owner repository 删除前提交 owner 前置清理事务，清理当时仍由三条关系关联的 Codespace、access token、日志、Manager 和 registration token。
- Fetch 与 Manager/owner 删除通过同一 Manager lock 排序；删除事务内重新查询后，不会留下 `manager_id` 指向已删除 Manager 的 Codespace。
...
- 用户/组织普通删除的已知前置条件失败时不清理 Codespace；最终事务在 owner lock 内重检并同时删除 Codespace 资源与 owner。
...
- owner 前置清理事务提交并释放所持 `globallock` 后尽力清除相关 cache；未知 UUID 的后续 inventory 不触发运行侧清理。
- Manager 删除同步清理其绑定 Codespace、token、日志和数据库记录，并尽力清理 cache；不检查 Manager 状态，也不向 Manager 发送指令。
- 与删除并发的已认证 RPC 必须在同一 keyed lock 内重新检查记录和 binding，删除完成后不能重建 Gitea 资源。
- 所有路径按 Gateway/SSH 地址、owner、repository、Manager、Codespace 的固定层级和各层 ID 升序取得所需 lock；应用锁均在相关数据库事务开始前取得，压力测试下不发生锁顺序反转。
