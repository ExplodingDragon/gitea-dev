...
- `models/codespace`：状态字段、固化 Git 协议、Manager 地址、Codespace Token、Git SSH Key 关系、索引和查询。
...
- Runtime API 测试覆盖 init.sh 在首次 create 原子生成 Ed25519 密钥、create 重试与 resume 校验并复用已落盘密钥、Manager 只转发公钥、known_hosts 严格校验、resume 保留 HEAD，以及私钥缺失或公私钥不匹配时停止本轮启动并返回明确失败。
...
- Web/RPC 配置测试覆盖非法大小关系启动失败、control-plane timeout 不大于四分之一 Manager 离线超时、protobuf 最大不可拆分 request/response 的实际编码大小、lease 的整数毫秒精度、Manager 离线/queue/Open Code/自动暂停的整数秒精度、control-plane timeout 包含认证/锁等待/数据库/cache，deadline/caller cancel 分别返回 Connect `DeadlineExceeded`/`Canceled` 且无业务 detail，以及日志 JSON 行换行保留、默认 limit、400 参数错误和 409 `current_offset` 恢复。消息上限不足时启动错误指出实际值、要求值和消息类型；超限输入在业务事务前返回 `ResourceExhausted`。启动测试分别覆盖 HTTP 默认/存量协议与 `DISABLE_HTTP_GIT`、内置 SSH 首启生成 Host Key、内置 SSH 复用已有 Key、外部 SSH 显式 known_hosts、存量 SSH 协议与 SSH 服务配置，以及排空模式关闭两种 Git 接入后继续管理清理。
...
- Manager/Gateway 测试覆盖完整本地配置解析、同一状态目录单进程锁、四种 FinalizeOperation outcome 的 worker 处理、resource absent 触发完整 inventory、同一 UUID 的运行侧动作与 cleanup 串行、cleanup 先持久化 pending、取消并等待 worker、关闭 session、删除带归属字段的 Incus 实例且最后删除本地快照；分别在收到响应前、写 pending 前、实例删除进行中和 Incus 实例删除后删除本地快照前模拟崩溃，确认前两种由更高 generation inventory 重获授权、后两种由 pending 快照续做。测试还覆盖以 Fetch 请求开始单调时钟加相对时长建立保守 lease、两端墙上时钟偏差和 RPC 延迟不扩大授权、lease 到期暂停普通 Incus 变更、Manager 重启后全部 worker 先暂停且成功 Fetch 续租后才恢复、abort 相对时长为 0 且仅缩减清理、stale worker 省略 observed 后不重新取得 payload，以及 create/resume/stop/delete 分别经原超时和 inventory 收敛、Runtime HTTP 固定 JSON/category 和生命周期矩阵、`POST /boot` 在本地原子持久化首个终态结果后返回成功、operation final 或后续 delete 后相同 POST 重试、source IP 通过指定通信网卡到 Incus identity 的唯一反查、单个 Incus 实例的完整归属字段与损坏本地状态处理、完整 inventory/Runtime 映射和 worker 上下文恢复后先 Declare online、逐 Codespace 恢复任务完成前不领取新 create/resume、active resume 在 Fetch 恢复后继续 Token/credential/ready/final、站点排空/offline/更高 delete operation 接管、Gitea token/server URL 的 `0600` 文件原子刷新、path-scoped Git helper、`GITEA_TOKEN_FILE` 轮换读取和 ready 顺序、active stop 只从本地 token 文件恢复 mask 且读取失败时丢弃不安全缓冲日志、Runtime Token 文件原子替换并由 init.sh 刷新受控服务、存在 active worker 时 Fetch 退避受最早续租时点限制、Endpoint 本地声明/路由/generation 原子提交、目标变化或删除前关闭 session、仅 label 变化保持 session、metadata 失败不回滚本地路由、功能关闭时本地 Endpoint 提交保留并重试 metadata、metadata 成功空响应使用请求快照确认 ready、较新 generation 取代较旧待上报快照、旧请求成功时按发送 generation 复检并继续发送当前快照、Gitea 延迟旧响应不覆盖已接受的更高 generation、生命周期与 Endpoint 变更的本地锁顺序、generation 保留已使用的最大值、固定 upstream root path、HTTP/HTTPS 控制面/用户入口/upstream、Gateway 本地固定 HTTP 失败状态、完整 UUID host 派生、workspace Web SSH 回退、wildcard DNS/TLS、Host 与 binding 匹配、可信转发头、Gateway session cookie 清除、upstream `Set-Cookie` 父域与保留名称过滤、open code 303 清理、普通 HTTP 每次请求转发前复检、WebSocket/SSH 定时复检、功能关闭后的两类收敛边界、Endpoint 与 SSH 共用 session 上限、SSH connecting 固定 30 秒超时且取消只减一次计数、公钥删除拒绝新连接但不直接关闭已有 transport、TTL/idle/cookie 属性、SSH 四维限流、SSH key 丢失恢复和 secret 轮换响应丢失恢复。
...
- Incus 生命周期测试覆盖 create 创建 stopped 实例后启动、两个 token 文件的临时写入/权限/fsync/rename、init.sh flock、create 锁定 SHA、resume 保留用户 HEAD、正常关机超时后强制停止、delete 全量确认实例缺失和 cleanup_pending 崩溃续做。虚拟机测试验证 guest agent 可用前文件/exec API 保持等待，系统容器走同一 Manager 代码路径。
- Runtime Token 测试确认 create/resume 轮换、普通 Manager 重启不轮换、明文不进入 Incus 配置/exec 参数/日志、文件与 verifier 不一致时关闭准入并停止实例，以及下一次 resume 生成新 token 并由 init.sh 刷新受控服务。
- Manager metadata 发布测试确认每个 Codespace 只有一个 generation 写入者且同一时刻最多一个请求；boot、Endpoint、internal SSH、恢复和周期刷新并发时只发布当前完整快照。任一成功空响应对应的请求快照包含当前 operation ready 就解除 create/resume 等待，即使本地已有更高 Endpoint generation；发布任务仍继续到最新 generation，resume 失败后清除本轮发布上下文且 stopped 不发布历史 ready。
- Codespace 当前快照故障测试固定临时文件写入、文件 fsync、rename、内存快照替换和响应返回各边界。rename 前失败保持旧快照并返回错误；rename 后业务结果成立，目录 fsync 失败只记录诊断。进程重启读取完整旧版或新版时，Fetch、inventory、cleanup 和 Runtime POST/PUT/DELETE 都能幂等收敛，不存在提交待确认状态。
...
- Manager 重启测试确认 Declare online 只恢复控制面；恢复出的 operation worker 在成功 Fetch 续租取得新相对有效时长前不修改 Incus。每个 running Codespace 在凭据、internal SSH、当前路由和 ready 重报完成后分别开放本地准入，stopped/failed/cleanup 保持关闭，单个临时失败不阻塞其他健康 Codespace。
