...
- Cron 测试覆盖 queued timeout、普通 running lease timeout、达到固定总执行期限的 running timeout、Manager 可用性和 failed retention 的 100 条 keyset 批次、单记录短事务、单条失败继续、数据库级错误终止和下一轮重试失败项；`reconcile_codespace_states` 不扫描或修复开发凭据，`cleanup_failed_codespaces` 随 failed 记录物理删除 Token 与 Git SSH Key。
