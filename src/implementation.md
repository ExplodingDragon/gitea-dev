...
- integration 测试覆盖 create、stop、resume token 轮换、session revalidate fail closed，以及用户、组织或 Manager 删除同步清理 Gitea 资源且不产生 Manager instruction。
...
- integration 测试覆盖 Basic、Bearer 和 `DISABLE_QUERY_AUTH_TOKEN` 开关两种 query 行为；已接受的 PAT 写入实际 `AccessTokenID`，OAuth2 access token、Actions token/JWT、session、reverse proxy 和 SSH 身份不写 PAT ID。API pre-middleware 按精确 method/route 写入 policy，授权点用一条查询重读 token row、可选 binding 和 Codespace 工作状态；clone/fetch/push/LFS、Contents/Diff Patch commit、branch/tag/status、Issue、Pull Request/Review/merge、Release、Wiki、Actions 查询/dispatch/rerun/artifact 下载、当前用户和公共版本/signing key 经过 binding、route 允许面和创建用户当前权限后成功。测试同时覆盖可见其他 repository 返回 403、不可见或不存在目标保持 404、`sudo` 拒绝、同仓库 PR update、fork PR 只读/merge、fork head update/删除拒绝、跨 repository Issue dependency 写入拒绝、有效 scoped workflow、对象存储 artifact 直传、严格登录配置下无 PAT 的有效 artifact raw 签名和 LFS 直传 URL。repository 管理、Hook/Deploy Key/Protection/Mirror、Actions Secret/Variable/Runner、Package、PAT/OAuth/Key、用户/组织/站点管理、普通 Web、Git dumb/wiki、upload-archive、push-to-create、未标记 API 和其他 repository 在副作用前返回 403；新增 route 默认拒绝。用户以正常凭据按 ID 删除绑定 token 返回 409，Codespace token 自身调用 token 管理 API 返回 403，普通 PAT 行为保持不变。
