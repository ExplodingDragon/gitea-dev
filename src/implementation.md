...
- Codespace 健康检查测试覆盖稳定 `running` / ready 选择、轮次开始时的 UUID 集合、本轮新进入对象延后到下一轮、本轮成功传输业务数据的内部 SSH session 形成成功结果、UUID 分散调度、`min(capacity_total,32)` 检查池、单 UUID 单请求、固定 SSH exec 命令、严格 Host Key、无 PTY/环境、使用配置的超时（默认 10 秒）和 4 KiB 输出上限。测试确认首次可恢复失败关闭新准入、30 秒复检只累计单对象次数、成功清零、生命周期条件变化移出本轮并清零；本轮全部仍符合条件的对象取得首次结果且未触发共享故障暂停后，连续 3 次失败的对象才持久化 `health_stop_pending`、停止实例并幂等上报 `stopped`。repository、workspace 内容、普通 Endpoint 和用户进程不参与检查，直接运行、devcontainer 和自定义脚本都只使用通用 internal SSH。
...
- 状态目录恢复测试覆盖根快照和每个 Codespace 快照都写入 `state_format_version=1`。根快照缺失、损坏、身份不匹配，或任一已经存在的 Codespace 快照不可读、不是合法 JSON、缺少格式版本、版本为 0、负数、未知或较新时，Manager 在绑定 listener、发送 RPC 或修改 Incus 前整体退出并保持文件与实例原状；以后明确支持的旧格式迁移必须先原子完成全部快照迁移，再开始外部动作。单 Codespace 快照文件缺失，或文件已经确认为版本 1 但完整结构校验失败时，Manager 关闭该 UUID 准入、按 Incus 归属字段写最小清理状态，并删除对应实例和本地状态文件。后续完整 inventory 使 running/stopped 进入 failed、deleting 完成物理删除，active create 在原 deadline 到期后进入 failed。测试在清理各阶段模拟崩溃，确认清理状态续做且不按旧 operation 猜测恢复。
