...
Codespace 生命周期分成 Gitea 下发动作和 Manager 上报运行状态两部分。
...
Gitea 能同时处理用户权限、仓库状态、token 生命周期和数据库事务，所以由 Gitea 决定要执行的动作并写入主状态。Manager 能看到 Runtime 的实际运行情况，所以由 Manager 上报执行进度、运行信息和最终结果。这样拆分后，重启、重试和数量差异都按同一套规则处理。

实现验收点：

- `codespace.status` 由 Gitea State Finalization 写入。
- Manager 上报运行信息和 operation 执行结果。
- 用户动作先进入 Gitea operation，再由 Manager pull 执行。
- Runtime 差异由 Gitea 根据数据库状态返回处理指令。
...
| `deleting` | delete operation 已创建，绑定 Manager 正在清理 Runtime Instance，Gitea 等待状态写入后删除记录。 |
...
- create 创建后进入 `queued`，领取成功后进入 `booting`。
...
| `running` | operation 已被 Manager 领取，lease 有效或处于恢复窗口。 |
...
`operation_id` 写入 `FetchOperation` 返回数据，并由 `UpdateOperation`、`UpdateLog`、`RequestGiteaToken` 携带。
...
| --- | --- | --- |
| create | `create` | 创建后 `queued`，领取后 `booting` |
| resume | `resume` | `stopped -> resuming` |
| stop | `stop` | `running -> stopping` |
| delete | `delete` | 当前可删除状态 -> `deleting` |

create 在 Manager 领取成功时绑定 `manager_id`。resume、stop、delete 使用已有 `manager_id`。
...
- create 领取成功后写入 `manager_id` 并进入 `booting`。
...
| 场景 | 处理结果 |
| --- | --- |
| `queued` 等待 create 领取超过 queue timeout | `queued -> error` |
...
operation deadline 表达本次执行租约，recovery deadline 表达维护恢复宽限。分成两个时间点后，Gitea 在 Manager 或自身重启后仍能接受当前 `operation_id` 的上报；长期没有恢复证据时，再进入明确状态。
...
- recovery deadline 到期后由 reconciliation 写入明确结果。
...
- 通过 [State Finalization](glossary.md#state-finalization) 写入明确结果。
- 吊销失效 Gitea Token。
- 写入 `status_message`。
...
维护重启期间，Gitea 给 Manager 时间重新上报完整运行信息；完整 snapshot 到达后，Gitea 按数据库主状态和当前 operation 处理差异。这样既能处理正常维护抖动，也能在真实差异出现时给出明确结果。
...
- recovery deadline 到期后通过 State Finalization 写入明确结果。
