...
| `running` | operation 已被 Manager 领取；lease 是否当前有效由 `operation_deadline_unix` 判定。 |
...
预检通过后，Fetch 先处理当前 Manager 的 running operation，每条在刷新 observed lease 或返回当前 payload 前都先判定 deadline。deadline 未到期时，功能启用或站点排空下的 stop/delete 按以下规则处理：相同版本出现在 `observed_operations` 时只批量刷新 lease，并在 `renewed_leases` 返回 UUID、版本和相对有效时长；observed 版本较低时返回当前 payload 并刷新 lease；未提交该 UUID 表示本地上下文缺失，Gitea 保持 operation 等待原 deadline。deadline 已到期时，Gitea 执行 timeout State Finalization，不返回 payload、续租回执或 abort，该项不计入 `max_operations`。站点排空下的 running create/resume 在 deadline 未到期时返回 `abort_create|abort_resume`；这些 abort 都是不刷新 lease 的一次性清理命令，Manager 清理本轮工作并 final failed，Gitea 将 create 映射到 failed、resume 映射回 stopped。处理完 running operation 后再领取新的 queued operation。所有路径都不改变 `operation_rversion`。
...
- 返回普通执行 payload 时按本节统一租约计算刷新 `operation_deadline_unix`，并返回同一次授予的正数相对有效时长；abort payload 不刷新，且相对有效时长为 0。
...
operation_deadline_unix=向未来取整到 Unix 秒(grant_time + lease timeout)
```

`grant_time` 是领取或续租事务内为本次授权读取一次的 Gitea 当前时间；向未来取整取的是不早于目标时刻的最小 Unix 秒边界。响应中的 `lease_valid_for_milliseconds` 是同一个 lease timeout 的精确毫秒值。服务端数据库的绝对截止边界因此不会早于 `grant_time + lease timeout`，而 Manager 从更早的请求开始时间计算本地截止点，所以本地授权不会晚于 Gitea 侧授权。绝对截止时间不通过协议返回，避免 Manager 把两端墙上时钟用于执行授权。create 领取时额外写入 `manager_id`。领取不递增 `operation_rversion`。
...
- 领取、返回当前 payload 和续租使用同一租约计算：Gitea 内部绝对 deadline 向未来取整到 Unix 秒，协议只返回精确等于配置值的相对有效时长。
...
resume worker 在 active operation 内取得新 Token、写入 Runtime credential；SSH 模式校验 Runtime 已有私钥与公钥配对，并通过 `EnsureCodespaceGitSSHKey` 确认 Gitea 仍绑定同一公钥，然后上报同版本 `ready` Runtime Metadata。初始化阶段和 `running` 都可直接使用 Token 与 Git SSH Key，便于 init 在 final 前验证实际仓库访问；面向用户的 open 和 Gateway SSH 仍等待 final done。Gitea 接受 final 时把主状态和用户交互能力一起切换为 running，并清空 active operation。Manager 重启后先暂停恢复出的 resume worker；本地 payload、boot 结果和 worker 阶段完整时才把该版本放入 Fetch，收到成功续租和新的相对有效时长后继续到 ready。上下文缺失或服务端已超时的 operation 不会重新执行。final 幂等提交后不需要 operation 结束后的凭据刷新任务。
...
4. 更新 codespace 主状态。
5. 按目标主状态和固化 Git 协议保留或物理删除 Token 与 Git SSH Key。
...
| outcome | Manager 行为 |
| --- | --- |
| `final_accepted` | 清除该版本 operation payload 和 worker 阶段；最新 boot 终态结果继续保留，用于相同 Runtime API 请求幂等重试。 |
...
SSH 协议的 `codespace_ssh_key` 属于 Codespace 整体生命周期：首次 create 生成，create 重试和 resume 校验并复用，不记录 operation 版本。有效 create/resume 初始化期与 running 都能使用；稳定 stopped 保留关系但拒绝 Git 命令，failed/deleting 和物理删除清理关系与 `PublicKey`。HTTP 协议不创建该关系。该协议差异只增加 final 的凭据前置条件，不增加主状态或 active operation 类型。

实现验收点：

- State Finalization 同事务写主状态、Token 与 Git SSH Key 生命周期、时间戳和 active operation 清理；active operation 清空后日志追加窗口随之关闭。
- create/resume final done 要求当前版本 ready metadata、Token 行以及 SSH 模式完整 Key 关系；Key 不携带 operation 版本，当前 ready metadata 证明本轮 init 已确认该关系。HTTP 模式没有 Key 前置条件。
...
failed 状态报告用于 Manager 已确认的单个 Codespace 不可恢复，例如 Incus 实例根存储损坏或 Incus 明确报告该实例无法再运行。Manager 暂时离线使用 recovering，Runtime Metadata 丢失返回 metadata_rebuilding，Gateway/SSH/Endpoint 临时错误在本地重试；实例物理缺失由完整 inventory 的 missing 规则处理；active operation 的动作失败使用 `FinalizeOperation(final failed)`。failed 状态报告只携带状态和版本，Gitea 追加固定状态摘要，详细原因保留在 Manager 本地日志。

Manager 在提交 failed 状态报告前关闭该 Codespace 的 session，停止该对象尚未完成的 metadata 上报和生命周期 worker，并保留已递增的 generation 作为已使用最大值。Gitea 返回首次接受或目标主状态已经成立的幂等成功后，Manager 在本地快照中持久化 `cleanup_pending`，再按完整 cleanup 流程删除 Incus 实例、凭据和本地快照；清理失败或进程重启时由 pending 快照继续，尚存实例在后续 inventory 中上报 `runtime_state=failed`。Gitea 记录仍为 failed 或已经被其他流程物理删除时，成功的当前 generation inventory 都返回 `cleanup_local_runtime`。普通 `resource_absent` 只触发该完整 inventory，不直接授权无记录资源删除。

Manager 按错误分类处理 `ReportRuntimeTransition`：`current_operation_conflict` 停止原 transition 并 Fetch 当前 operation；`stale_operation` 停止原请求并重新上报 inventory；`stale_generation` 使用服务端当前值加一，重新读取 Incus 状态后提交；`generation_conflict` 关闭该 Codespace 的发布和交互，持久化最小清理记录并删除该 UUID 的归属实例；`manager_offline` 先 Declare recovering，再读取当前状态；`codespace_not_found` 停止当前通信并触发完整 inventory；`manager_unregistered` 或明确认证失败关闭全部入口、强制停止 Incus 实例并停止 RPC，同时保留实例根存储等待同一身份凭据恢复。所有分支保留已经使用的最高 generation。
...
`boot.stage` 使用固定阶段并包含 `credential-refresh` 和终态 `ready`，`boot.operation_rversion` 标识产生快照的 boot 上下文。create 和 resume 的 final done 都要求 boot 版本等于当前 operation 且 metadata 已为 `ready`。resume 启动 Runtime 后，在 active operation 内申请新 token、刷新 Runtime credential，再上报本次 resume 版本的 `ready`；旧版本的 `ready` 不能完成当前 resume。凭据或 ready 上报临时失败时，Manager 在 operation lease 内退避重试；确认无法写入 credential 时停止本轮启动的 Runtime，并提交 resume final failed，使主状态保持可恢复的 stopped。这样 `running` 始终表示本次启动所需的凭据和入口已经就绪，open/SSH 不存在等待另一个启动阶段的中间状态。
...
| active resume | `boot.operation_rversion` 等于当前 operation；stage 为 `credential-refresh` 或 `ready` |
...
同一 `boot.operation_rversion` 的 stage 只能按规定顺序前进；一旦接受 `ready`，后续同版本快照继续保持 `ready`。resume final failed、timeout 或 abort 会在主状态事务提交后尽力清除当前 Runtime Metadata，Manager 清除本轮 boot 发布上下文，不恢复或发布历史 `ready`。此后迟到的当前 resume 版本快照因已经没有 active resume 而返回 `stale_operation`；清理失败时数据库中的 `stopped` 主状态仍会阻止交互，下次合法 resume 使用更高 operation 版本从保留的 Incus 实例重建完整快照。

稳定 running 的凭据文件修复沿用当前 `ready` 快照，不创建新的 boot 阶段。修复成功后 Runtime 继续运行；确认无法安全写入凭据时，Manager 先关闭会话并停止 Runtime，再通过 `ReportRuntimeTransition(stopped)` 收敛主状态并保留实例根存储。根存储已损坏时改为报告 failed。这样凭据修复不会产生 running 与 boot stage 不一致的中间状态。
...
- resume final done 只在当前 operation 版本的 metadata 已为 `ready`、Token 行完整且 SSH 模式公钥关系完整时接受；公钥本身没有 operation 版本，当前 ready 证明本轮 init 已确认该关系。final 成功后无需恢复独立的凭据刷新任务。
...
| 功能启用，SSH 协议 active create/resume 缺少 Git SSH Key 关系 | init.sh 通过 Runtime API 提交已落盘公钥，Manager 调用 `EnsureCodespaceGitSSHKey`；关系缺失时补建，相同公钥幂等确认，不同公钥返回 `key_conflict` |
...
- stopped 主状态不会因 running inventory 自动恢复；只有 Gitea 下发的 resume operation 在 Token、协议所需 Git 凭据和 ready metadata 完成后，才能通过 final done 进入 running。
...
`operation_created_unix + QUEUE_TIMEOUT` 是 queued operation 等待 Manager 领取的硬截止时间。Fetch 读到过期候选时，在 Codespace lock 内按 `codespace_uuid + operation_rversion + operation_status=queued` 条件写入超时结果，然后继续处理本批其他候选；该项不计入 `max_operations`。Cron 处理未被 Fetch 扫到的过期记录。`operation_deadline_unix` 是 running operation 的 lease 截止时间；Manager 只通过 Fetch observed 批量续租刷新 deadline。
...
active operation lease 到期时使用统一处理：

| Manager 状态 | 处理 |
| --- | --- |
| online、recovering 或 offline | `now >= operation_deadline_unix` 时按上表的 running lease timeout 结果处理。 |

Manager 可用性和 operation lease 分别表达不同事实：online/recovering/offline 决定当前是否可以交互、领取或提交请求，`operation_deadline_unix` 决定已领取动作还能否继续。Fetch、final 和 Cron 都直接校验同一 deadline；第一个成功的条件更新成立后，其他请求按当前状态返回幂等或 stale 结果。**设计如此：Manager 重启和 recovering 声明不创建额外恢复期限，也不恢复已经到期的 lease。**
...
Manager 在发送 Fetch 前记录 `request_started_monotonic`，收到 operation payload 或续租回执后按 `local_worker_deadline = request_started_monotonic + lease_valid_for_milliseconds` 建立本地执行截止点。服务端授予 lease 晚于请求开始，因此把请求耗时计入已消耗时间会得到保守边界；两端墙上时钟不一致也不会延长本地授权。Manager 不接收或保存 Gitea 的绝对 Unix deadline。到达本地截止点且尚未收到新的成功 Fetch 续租或 final outcome 时，Manager 停止新的 Incus 变更并保留当前上下文。

Gitea 每次授予 lease 时，用该次 `grant_time + OPERATION_LEASE_TIMEOUT` 向未来取整到整数 Unix 秒，写入 `operation_deadline_unix`；同一响应返回配置时长的精确整数毫秒值。向未来取整允许 Gitea 侧边界最多多出不足一秒，但不会扩大 Manager 的本地执行授权。**设计如此：额外的秒内余量只用于消除数据库秒级时间戳的截断误差，Manager 始终按未取整的配置时长和本地单调时钟停止 Incus 变更。**

Manager 进程重启后已经失去原单调时钟基线，因此所有恢复出的 worker 先保持暂停。只有上下文完整的 operation 才提交同版本 observed；Gitea 在原 deadline 未到期时返回新的相对有效时长，Manager 据此建立新本地截止点后恢复 worker。Gitea 已经超时、上下文缺失或 RPC 暂时不可用时均保持暂停，不使用磁盘中的绝对 deadline 猜测剩余授权。**设计如此：Gitea 重启不改写 operation，Manager 重启也不自动延长 lease；恢复执行必须经过一次服务端成功续租。**
...
- Manager 以请求开始时的本地单调时钟加服务端相对有效时长建立保守截止点，不用绝对 Unix deadline 授权 Incus 变更。
- Gitea 的绝对 lease deadline 向未来取整到 Unix 秒，相对有效时长保持配置的精确毫秒值；Manager 本地截止点不晚于 Gitea 侧截止边界。
...
**设计理由：开发凭据由 operation 请求和生命周期事务维护。**`RequestGiteaToken` 为 active create、active resume 或稳定 running 创建或修复 Token；SSH 模式的 `EnsureCodespaceGitSSHKey` 在 create/resume 初始化期确认 Codespace 生命周期内唯一的公钥关系。有效初始化期与 running 都可直接访问绑定仓库。stop final 和 resume 失败/超时删除 Token 并保留 Git SSH Key；failed、deleting 和物理删除删除两类凭据。每次变化都由明确的生命周期阶段约束，周期任务只处理 operation 超时和 Manager 可用性。
