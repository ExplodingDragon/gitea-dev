...
- RequestRuntimeAccess 请求携带 `codespace_uuid`、`operation_rversion` 和固定上报的 `RuntimeGitSSHKey.public_key`；服务端根据当前 Codespace、active operation、Manager 和功能状态决定返回运行访问材料或 `state_unavailable`。请求不携带自报用途，调用方无法用额外模式字段改变授权结果。
