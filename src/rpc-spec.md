...
service ManagerService {
  ...
  // Returns or issues the current token for active create, active resume, or running recovery.
...
message EnsureCodespaceGitSSHKeyRequest {
  string codespace_uuid = 1;
  // Canonical SSH wire-format public key blob parsed by Manager from init.sh input.
  bytes public_key = 2;
}
...
- Runtime Metadata 的 boot 上下文按状态校验：active create 使用当前 operation 和 create 阶段顺序，active resume 使用当前 operation 的 `credential-refresh|ready`，running 固定为 boot 版本不大于当前 operation 的 `ready`；stopped 且无 active operation 时拒绝 metadata。同一 boot 版本一旦 ready 就保持 ready。
...
- `RequestGiteaToken` 在功能启用、Manager 声明为 online 或 recovering 且 heartbeat 有效，并且 Codespace 为已领取且租约未到期的 create/resume 初始化阶段，或主状态为 running 时返回或签发 Token，并始终返回规范化 `server_url`。Token 在 create 与 resume 初始化阶段和 running 都可通过后续认证；active delete 和站点排空拒绝读取、签发或修复 Token。
- create/resume payload 的 `git_protocol` 必须等于 Codespace 创建时固化的 `http|ssh`。HTTP 模式不调用 `EnsureCodespaceGitSSHKey`；SSH 模式在 create 首次生成密钥，并在 create/resume 初始化中通过该 RPC 创建或确认同一公钥。请求公钥为空、无法解析、算法不是 Ed25519 或超过 Gitea SSH key 大小限制时返回 `invalid_argument`。
...
- create/resume payload 始终携带已固化的 `git_protocol`；HTTP 模式不产生 Git SSH 公钥，SSH 模式的 final done 要求 Codespace 已有完整公钥绑定。
- `EnsureCodespaceGitSSHKey` 请求只携带 UUID 和公钥字节；用户、仓库、协议、初始化阶段和生命周期状态由服务端当前关系取得，响应只返回 Runtime 建立严格 SSH Host Key 校验所需的规范化行。
- 相同公钥和响应丢失可以幂等重试；不同公钥不会替换当前绑定。公钥不携带 operation 版本，Manager 使用本地 boot session 把 Runtime 请求关联到当前生命周期操作。
...
- `CreateOperationPayload` 与 `ResumeOperationPayload` 都携带创建时固化的 `GitProtocol`；SSH 模式通过 `EnsureCodespaceGitSSHKey` 创建或确认 Codespace 公钥，HTTP 模式不调用该 RPC。
