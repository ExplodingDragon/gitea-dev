...
- `queued|booting|error|首次 create 流程中的 deleting`：中心日志布局；除 deleting 外只允许 delete。
- `running|stopping|stopped|resuming|非首次 create 流程中的 deleting`：左右分栏；左侧日志，右侧控制与信息。
...
这样设计的原因是 create、open、SSH、resume、stop、delete、logs 都会重复使用用户状态、仓库状态、repository 权限、Manager 状态和 Runtime Metadata。统一入口可以让 Web 页面、RPC 和 Gateway 得到同一结论，避免权限规则在多个 handler 中逐渐分叉。

服务层判定复用 Gitea 现有用户、组织、仓库、unit、visibility、blocking、restricted user、login restriction、2FA 和 repository permission 逻辑。
...
| 仓库状态 | repository 状态矩阵允许 create，目标 ref/commit 可解析 |

Repository 状态矩阵：

| repository 状态 | create | open / SSH | resume | stop / delete / logs | 设计原因 |
| --- | --- | --- | --- | --- | --- |
| 正常且 code unit 可读 | 允许 | 允许 | 允许 | 允许 | 用户具备代码读取能力时，codespace 是该用户的私有开发环境入口。 |
| `archived` | 返回 repository archived 分类 | 返回 repository archived 分类 | 返回 repository archived 分类 | 允许 | archived 表示仓库进入只读/冻结管理状态，停止交互可以避免继续产生运行侧变化；管理操作仍用于资源回收和审计。 |
| `empty` | 返回 empty repository 分类 | 返回 empty repository 分类 | 返回 empty repository 分类 | 允许 | 空仓库没有可锁定 commit，无法形成可复现 workspace；已有对象仍应可查看日志和清理。 |
| migrating / pending transfer / broken | 返回 repository unavailable 分类 | 返回 repository unavailable 分类 | 返回 repository unavailable 分类 | 允许 | 这些状态下 repository 权限、路径或 git 数据可能正在变化，交互入口暂停可以避免 clone/checkout 与权限判定出现不一致。 |
| source repository deleted | 返回 source repository deleted 分类 | 返回 source repository deleted 分类 | 返回 source repository deleted 分类 | 允许 | repository 删除后 codespace 只保留生命周期和日志管理能力，用户能看到来源已删除并完成清理。 |
| mirror | 允许 | 允许 | 允许 | 允许 | mirror 本身仍是可读取 repository，同步来源属性不改变远程开发入口；实际写入能力继续由用户对 Gitea repository 的权限决定。 |
| create 目标 ref/commit 不可解析 | 返回 ref not found 分类 | 已有 codespace 按锁定 `commit_sha` 管理 | 允许发起 resume，Manager checkout 失败时进入 `error` | 允许 | create 需要锁定 commit；已有 codespace 已保存 `commit_sha`，原 ref 后续移动或删除不改变该历史对象的管理能力。 |
...
- `user_display_name`
- `user_deleted`
- `repo_id`（repository 删除后为空）
- `source_repository_state`（`available` / `deleted` / `unavailable`）
- `ref_type`
- `ref_name`
- `commit_sha`
- `manager_id`
...
Minimal Info 面向列表页、组织管理员管理视图和 repository 删除后的弱关联展示。它只提供识别对象、判断状态和发起允许动作所需的信息。token、internal SSH、upstream 和日志正文保留在对应的专用接口或内部组件中，可以减少列表接口的敏感数据暴露面，也让组织管理员能够清理资源而不进入其他用户 workspace。
...
- 更新 Manager 名称、版本、gateway 地址、tags 和诊断 metadata。
- 更新 `last_online_unix`。
...
SSH 是 Manager 的必备能力。Web Endpoint 和 SSH 都属于 codespace 的交互入口，统一要求 Manager 声明 SSH 地址，可以让 UI、权限判定和 Gateway 部署健康检查有稳定能力基线。
...
- `codespace_uuid`
- `lease_deadline_unix`
- create/resume 所需 repository/ref/commit 字段

delete 和 stop 是资源回收动作，优先推进可以释放运行侧资源。resume 和 create 会占用容量，由 Manager 当前容量决定领取时机。`operation_id` 绑定本次下发动作，Manager 后续 operation-bound RPC 都用它校验归属。

实现验收点：

- 单次 `FetchOperation` 返回一个 operation。
- 领取通过数据库条件更新完成。
- create 领取同事务写入 `manager_id`、`operation_status=running`、`operation_deadline_unix`，并推进到 `booting`。
...
Manager 负责报告动作结果，Gitea 负责把结果写成主状态、token 状态、日志封闭状态和状态消息。State Finalization 在同一事务内完成这些写入，保证用户看到一致的生命周期结果。
...
- 重复 final 返回幂等结果。
- State Finalization 同事务处理主状态、operation 状态、token 和日志封闭。
- 过期 `operation_id` 返回 stale 分类，当前主状态保持稳定。

### UpdateLog

- 写入 DBFS 路径 `codespace_log/{codespace_uuid}.log`。
- 按 byte offset 追加已脱敏日志。
- 校验 `codespace_uuid + operation_id + manager_id` 匹配当前 running operation。
- offset 等于当前日志大小时追加。
...
- 只允许当前 `operation_id` 对应的 running operation 追加日志。
- operation 进入 `done|failed` 后，日志进入封闭状态。
...
- `log_expired=true` 时返回空 `lines` 和明确的过期状态。
- delete done 后删除 DBFS 日志。
- `error` 状态日志保留到用户 delete 或 `FAILED_RETENTION_DAYS` 清理。
...
- 校验 `codespace.status in (booting,running,stopping,resuming) && codespace.manager_id == caller`，不匹配时返回 stale 分类。
- 只接受 `booting|running|resuming|stopping` 状态写入。
- `queued|stopped|deleting|error` 状态返回 stale 分类。
...
| `instances[].observed_operation_id` | Manager 看到的本地 operation 标识 |
...
| missing `running` runtime | 记录 divergence，吊销 token，进入 `error` |
| missing `stopping` runtime | 接受当前 stop operation 进入 `stopped` |
| missing `deleting` runtime | 接受当前 delete operation 完成物理删除 |
| missing `booting/resuming` runtime | 在恢复窗口内等待当前 operation 继续上报，窗口到期进入 `error` |
...
所有 operation-bound RPC 都携带 `codespace_uuid` 和 `operation_id`，Gitea 通过 `codespace.operation_id`、`codespace.operation_status` 和 `codespace.manager_id` 完成校验。
...
`FetchOperation` 返回给 Manager 的 operation 字段：

| 字段 | 适用类型 | 说明 |
| --- | --- | --- |
| `operation_id` | 全部 | 当前 operation 标识 |
...
- PR 场景下 `create|resume` 返回数据同时包含 base/head clone URL 与 web URL。
- `stop|delete` 返回数据不包含 repository clone/web URL、base/head URL、ref、commit 或 pull 字段。
...
- repository 访问在公共 token 判定入口追加 repo binding 校验。
...
- repo-bound token 判定在 scope 校验外，额外校验 `target_repo_id == codespace.repo_id`。
- codespace-bound token 的 repository 访问范围固定为 `codespace.repo_id`；访问其他 repository 时返回 repo binding mismatch 分类。
...
- access token 未被 active codespace 绑定时，按 Gitea 现有 token 逻辑处理。
- access token 被 codespace 绑定时，`token_id` 与 `codespace.gitea_token_id` 匹配。
- 被绑定 codespace 处于 `booting|running|resuming`。
- `target_repo_id` 与 `codespace.repo_id` 匹配。
- target repository 已删除或当前不可访问时返回 repo unavailable 分类。
- 非 repository 资源返回 unsupported resource 分类；需要 Runtime 使用的非 repository 信息由 create/resume 环境变量注入。

这样设计的原因是 Gitea access token scope 是 category 级，`write:repository` 只能表达“可以做 repository 类动作”，不能表达“只能访问哪个 repository”。repo-bound token 判定把 scope 能力和 repository 绑定拆开，既复用现有 token 体系，又避免 Runtime token 变成通用 PAT。
...
| 任务 | 默认调度 | 职责 |
| --- | --- | --- |
| `reconcile_codespace_states` | `@every 1m` | 检查中间态（booting/stopping/resuming/deleting）的 operation deadline、recovery deadline、Manager offline timeout 和 Runtime inventory 差异，按状态机处理并更新 token |
...
`reconcile_codespace_states` 定时扫描中间态 codespace，处理 `operation_deadline_unix` 超时、`operation_recovery_deadline_unix` 到期、Manager 离线和 Runtime inventory 差异，让 codespace 按状态机进入明确结果。`cleanup_failed_codespaces` 和 `cleanup_codespace_logs` 分别处理数据生命周期，控制过期记录和日志的长期堆积。
