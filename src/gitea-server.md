...
`GET /-/codespaces/{uuid}/state` 返回创建者详情页的当前状态 HTML 片段，`GET /-/codespaces/{uuid}/logs` 返回日志数据。两者属于创建者 Web 页面的内部数据接口；第一版用户交互采用服务端 Web 页面，不定义版本化 Codespace REST API。
...
- 初次详情渲染与状态片段刷新使用同一个创建者详情服务、权限检查和操作集合；版本化协议范围保持为 ManagerService 和 Manager Runtime API，状态片段按 Web 路由维护。
...
每个 owner scope 最多保存一行 registration token。`GetOrCreateRegistrationToken` 在行存在时返回原明文，不存在时才创建；`RotateRegistrationToken` 在 owner 写锁和同一事务中原地替换随机 token；`DeactivateRegistrationToken` 物理删除当前行，没有记录时幂等成功。settings 页面可以显示当前 token 明文，因为它是可重复使用的部署注册凭据。`owner_id=0` 的 global token 只由站点管理员管理，用户或组织删除时物理删除所属 scope 的 registration token。读取、轮换和停用使用不同动作，可以避免管理员仅打开页面就意外轮换正在使用的凭据。

**设计理由：Registration Token 表保存每个 owner 当前有效的注册入口。**轮换原地替换当前值，停用物理删除记录；认证只需检查当前行。失败诊断写入服务端日志，凭据历史无需进入数据库或保留任务。

Manager 删除确认页展示绑定 Codespace 数量，并说明 Gitea 会同步删除这些 Codespace、开发凭据、日志和 Manager 地址行，但不会联系 Manager 或保证 Runtime 回收。确认后，服务层不读取 Manager runtime state，保持 owner/Manager lock 并按 UUID keyset 逐 Codespace 短事务清理，空集合复检后在最终事务中删除 Manager 地址行与 Manager；已经通过认证的并发 RPC 也必须在最终写入前重新检查记录和 binding。

用户自助删除、管理员 Web/API 删除用户和 inactive-user Cron 统一调用 `services/user.DeleteUser`；组织 Web/API 和 last-owner purge 统一调用 `services/org.DeleteOrganization`。Codespace 清理挂在这两个服务入口中，先完成适用的 Gitea 前置检查，再按 [用户与组织删除](lifecycle-flows.md#用户与组织删除)执行分阶段清理。管理员删除用户因此与用户自助删除具有相同的数据关系清理结果；purge 在组织处理前释放用户 owner 写锁，最终重新取得并复扫。普通用户或组织删除精确清理自身正数 owner scope，站点级 `owner_id=0` Manager、地址和 registration token 保持有效。

Gitea 中会改变删除判定关系的入口统一进入 owner 关系写服务边界：repository 的普通创建、fork、template、migration、adopt、push-to-create、transfer 和 delete；package/version 的通用、container 与 Terraform 写入；组织成员与 team 成员写入；Codespace、Manager 和 registration token 写入。服务边界取得涉及 owner 的 `owner_write_{owner_id}`，在事务中复读 owner 后才提交关系。这样管理员删除用户与并发业务写入共享一个明确提交顺序，而不是依赖各 router 恰好做过一次锁外存在性检查。
...
- 用户自助、管理员 Web/API、inactive-user Cron 和 purge 使用同一个用户服务删除入口；组织 Web/API 与 last-owner purge 使用同一个组织服务删除入口。
- 用户或组织删除成功前复检 Codespace、owner-scoped Manager、registration token 和 Gitea 原有前置条件；普通账户删除不会改变 `owner_id=0` 的全局管理资源或无关 Codespace。
- 全部 repository 创建形式、package/version 专用入口以及组织与 team 成员写入都通过 owner 关系写服务边界；owner 已删除或类型不匹配时在事务中返回明确错误，不提交悬空关系。
...
创建者详情使用 `CodespaceOwnerDetail`，在上述字段基础上增加 `git_protocol`、当前 Codespace Token 是否存在及其创建时间和末八位、SSH 模式 Git Key 是否就绪及登记时间、`log_line_count / log_size`、自动暂停持久选择和有效超时，以及后述规范化 `workspace/endpoints/ssh`。自动暂停字段为 `auto_stop_mode`、`auto_stop_timeout_seconds`、`auto_stop_enabled` 和 `effective_idle_timeout_seconds`；`interaction_generation` 只用于 Manager 协议，不返回 Web 页面。凭据展示字段只说明 init 是否完成，不返回 Token verifier、salt、密文、明文、公钥正文或指纹。
...
`workspace` 始终使用固定 `endpoint_id=workspace`。当前 metadata 中存在同名 Endpoint 时使用它的 label，否则使用请求语言对应的 “Workspace” 文案；这个对象只描述稳定的默认入口，不揭示 Manager 最终连接 Runtime upstream 还是内置 Web SSH。`endpoints` 只包含 metadata 中非 `workspace` 的 Endpoint，按 `endpoint_id` 升序返回。服务端负责 label 规范化，页面不解析完整 Runtime Metadata。

`ssh` 只由绑定 Manager 对外声明的 `gateway_ssh_addr` 和 Gateway host key 展示字段构造。服务端拆分规范化的 host 与 port，用户名固定为 `cs-{完整 UUID}`，command 固定由这些字段生成。该结构只提供用户实际连接所需的公开地址和 host key 核对信息，内部 SSH host、Runtime user、upstream 和任何 token 都不会进入详情响应。

只有创建者详情在具有 Interactive Access 且当前 metadata ready 时返回普通 `endpoints`。`ssh` 还要求绑定 Manager online、`internal_ssh` 完整，并且 Manager 的公开 SSH 地址、host key algorithm、SHA256 fingerprint 和更新时间全部有效；任一条件缺失时省略该对象。治理列表数据结构从类型上没有 `workspace/endpoints/ssh` 字段。
...
- 创建者的 `running` 且无 active operation 时返回 `configure_auto_stop/stop/delete`，功能启用、Manager online 且 metadata ready 时加入 `open_workspace`；存在普通 Endpoint 时加入 `open_endpoint`；SSH 展示字段和内部条件完整时加入 `ssh`。
...
创建者页面数据和治理页面数据分开定义，是为了让数据权限由服务端类型保证，而不是依赖模板隐藏字段。治理列表只提供识别对象、判断状态和发起允许动作所需的信息，使管理员能够治理资源而不进入其他用户 workspace。这些结构的稳定使用方是服务端模板和状态片段；第一版的版本化协议范围是 ManagerService 和 Manager Runtime API。
...
- 完整详情页和状态片段对相同身份与对象使用相同 `allowed_actions`；只有存在普通 Endpoint 时才有 `open_endpoint`，只有 SSH 展示字段和内部就绪条件都完整时才有 `ssh`。queued idle stop 仍允许 open/SSH/continue，领取后的 stop 只保留创建者设置和 delete。
...
| `git_ssh_key_required` | `FailedPrecondition`，确保公钥绑定 | SSH 协议 create/resume final done 缺少完整的 Codespace Git SSH Key |
| `git_protocol_conflict` | `FailedPrecondition`，按 payload 协议执行 | HTTP 协议 Codespace 调用 Git SSH Key 接口，或本地协议与固化值不同 |
| `key_conflict` | `FailedPrecondition`，保留当前绑定并检查 Runtime 密钥 | Codespace 已绑定不同公钥，或该指纹已被其他 Key 使用 |
...
- final 缺少当前 ready 快照、Token 行、SSH 模式公钥与交互 cache miss 分别返回 `metadata_required`、`gitea_token_required`、`git_ssh_key_required` 和 `metadata_rebuilding`；running 没有独立的未就绪分类。
...
- 先按唯一 token 索引只读取候选 `owner_id`，该读取不构成认证成功；随后取得对应 owner 写锁（包括 `owner_id=0` 的 `owner_write_0`），在锁内按 token 重新读取当前行。复读仍存在且明文匹配时才有效，期间已 Rotate/Deactivate/owner delete 则返回 `unauthenticated`。
...
- 注册与同 scope token 的 GetOrCreate、Rotate、Deactivate 使用同一 owner 写锁；并发结果由取得锁的顺序决定：注册先完成时创建 Manager，凭据变更先完成时旧 token 的注册返回未认证。
...
- response 确认完整快照已经接受，并返回服务端选定的心跳周期、Runtime Metadata 刷新周期和控制面消息大小上限。Manager 可用性仍由请求声明的 runtime state 和 heartbeat 推导，这些响应字段不增加新的持久状态。
- 设 `MANAGER_OFFLINE_TIMEOUT` 的毫秒值为 `O`。Gitea 返回 `heartbeat_interval_milliseconds=floor(O/4)`、`runtime_metadata_refresh_interval_milliseconds=floor(O/2)`；Runtime Metadata TTL 保持 `O*2`。Manager 启动后立即以 recovering Declare，成功取得三个正数响应字段后才启动周期任务和领取流程，后续成功响应原子替换当前值。
...
**设计理由：Manager 完成完整 inventory、Runtime 映射和可继续的 worker 上下文恢复后 Declare online。**Runtime Metadata 和待提交 stopped/failed 状态报告可以在 online 后逐个处理；全部完成前，Fetch 使用零容量或省略 create/resume。Manager/Gateway 进程启动时关闭全部 Codespace 的本地 session 准入，只有逐项完成凭据、SSH、路由和当前 ready 上报后才开放，因此 online 只恢复控制面 RPC，不会让外部 cache 中保留的旧 ready 快照提前建立连接。重启恢复出的 worker 先保持暂停；本地上下文完整的 operation 只有在 Fetch 成功续租并返回新的相对有效时长后才能继续，其他 operation 由正常超时处理。
...
- Declare 响应中的三个服务端参数都为正数，心跳周期等于离线超时毫秒值的四分之一向下取整，metadata 刷新周期等于二分之一向下取整，消息上限等于当前有效配置；修改 Gitea 配置不要求同步修改 Manager 配置。
- Manager 在 operation deadline 之后才 Declare recovering 时，Fetch 和 final 均按普通超时结果处理。
- Manager 恢复完整 inventory、Runtime 映射和 worker 上下文后可先 Declare online，再完成逐 Codespace metadata、credential 和待提交状态报告；全部完成前不领取新的 create/resume，尚未恢复的 Codespace 也保持本地 session 准入关闭。
...
`operation_deadline_unix` 是 Gitea 写入数据库并用于 Fetch、FinalizeOperation 和 Cron 竞态判定的绝对截止边界。每次领取或批量续租在事务内读取一次 `grant_time`，把 `grant_time + OPERATION_LEASE_TIMEOUT` 向未来取整到最小的整数 Unix 秒后写入数据库。`lease_valid_for_milliseconds` 是同一次授予的精确相对时长，供 Manager 配合本地单调时钟限制 Incus 执行。绝对 deadline 只属于 Gitea，不通过 RPC 回传；Manager 无需保存或解释服务端墙上时间。

delete 和 stop 是资源回收动作，优先推进可以删除或停止 Incus 实例。resume 和 create 会占用容量，由 Manager 当前容量决定领取时机。resume 基于已初始化 workspace 和绑定 Manager 执行，不重新解析 repository。`operation_rversion` 绑定本次 Gitea 下发的 operation 版本，Manager 后续 operation-bound RPC 都用它校验归属。
...
`capacity_available` 范围为 `0..DeclareManager.capacity_total`。Manager 修改总容量时先成功提交完整 Declare，再用新上限 Fetch，因此 Fetch 不重复提交总容量。`max_operations` 范围为 `1..256`，只限制 `operations`；`renewed_leases` 最多与 request 的 observed 数量相同，不占 payload 名额。`observed_operations` 最多 10000 条，每个 UUID 在一次请求中唯一。每种优先级使用 `operation_created_unix, uuid` keyset 分页，按升序处理；单次 Fetch 在稳定 scope/tag 筛选后合计最多检查 1024 个 queued 候选。Manager 调用 Fetch 的周期不超过 `OPERATION_LEASE_TIMEOUT / 3`。站点排空时不领取 queued create/resume，queued 和 running stop/delete 继续正常处理。abort create/resume 不续租；create 的 `final failed` 写为 failed，resume 的 `final failed` 在确认启动回滚后写为 stopped。

Fetch 在处理每条 running operation 时，于 Codespace lock 内先检查 `operation_deadline_unix`，再决定 observed 续租、返回当前 payload 或 abort。deadline 未到期时按普通规则处理；已经到期时执行 timeout State Finalization，不返回 payload、续租回执或 abort，也不计入 `max_operations`。站点排空的 create/resume 只在 deadline 未到期时返回一次性 abort 命令且不写新 deadline。这与 FinalizeOperation 和 Cron 使用同一条件更新边界，observed 批量续租不能恢复已经超时的 operation。
...
- 普通领取、当前 payload 和批量续租把 `grant_time + OPERATION_LEASE_TIMEOUT` 向未来取整为 Gitea 的 Unix 秒 deadline，并在响应中只返回未取整配置时长的精确毫秒值。
...
create/resume final done 都要求当前 operation 版本的 Runtime Metadata 已为 `ready` 且 Codespace Token 行完整；`git_protocol=ssh` 时还要求 Codespace 公钥关系与 `PublicKey` 完整。resume worker 在 final 前请求新 Token、原子刷新 Runtime credential；SSH 模式校验 Runtime 原密钥并确认同一公钥绑定，然后上报 ready。Key 不记录 operation 版本，当前版本 ready 已经证明本次 init 完成了密钥校验和确保请求。任一步失败都仍由 active operation 的 lease、重试和 final failed 处理。这样 final done 提交后即可清除 operation，`running` 同时表示本次启动的凭据和交互入口已经就绪，不需要另一个跨重启的后置任务。
...
resume failed 只在 Manager 已确认本轮启动的 Incus 实例停止后上报，因此保留实例根存储并允许以后再次 resume。主状态事务提交后，Gitea 尽力清除本次 resume 的 Runtime Metadata；Manager 清除本轮 boot 发布上下文，不恢复历史 ready。迟到的本次 resume 快照会因 active operation 已清空而返回 `stale_operation`，下一次 resume 使用更高 operation 版本从保留的实例重建。若 Manager 随后确认实例根存储或配置本身不可恢复，使用现有 `ReportRuntimeTransition(failed)` 把无 active operation 的 stopped Codespace 写为 failed；详细原因仍留在 Manager 日志，不增加 final 字段。
...
- create/resume 的 final done 在当前 operation 版本 ready metadata、Token 行或 SSH 模式公钥缺失时被拒绝，active operation 保持可重试；成功后不再存在凭据刷新任务。
...
- `boot` 上下文按当前状态校验：active create 使用当前 operation 版本并按 create 阶段顺序前进；active resume 使用当前版本的 `credential-refresh|ready`；running 只接受 boot 版本不大于当前 operation 版本的 `ready`；stopped 且无 active operation 时拒绝 metadata。同一 boot 版本的 stage 只能前进，已经接受 `ready` 后保持 `ready`。
- `stopped` 状态下只接受 active resume 的当前启动进度；没有 active operation 时不周期发布 Runtime Metadata，也不提供面向用户的 open 或 Gateway SSH。active resume 内的 Runtime Token 和 Git SSH Key 按开发凭据阶段判定，可在 init 完成前访问绑定仓库。
...
- Manager 启动后为 active create、active resume 和 running Codespace 启动发布任务并重建 Runtime Metadata cache。稳定 stopped 不发布 metadata；本地只保留最新 boot 终态供 Runtime API 幂等重试，下一次 resume 从保留的 Incus 实例重建更高 generation 的完整快照。
...
- resume 在 active operation 内完成 `credential-refresh`、Token、固化协议对应 Git 凭据和当前版本 ready 上报；任一前置缺失时 final done 被拒绝，主状态保持 stopped。
...
`ReportRuntimeTransition` 用于 Manager 在没有 Gitea-issued active operation 时上报本地主动 stopped 或不可恢复的 failed 状态。running 只能由 Gitea 下发的 create/resume operation 通过 final done 建立。
...
**设计理由：running 只表示 Gitea 明确下发且已经完整完成的启动。**用户 resume 具有 active operation、lease、abort 和 final failed 的现成恢复边界；复用它可以在进入 running 前完成 Token、协议所需 Git 凭据和 ready 检查。inventory 看到 Gitea stopped、Runtime running 时只返回 `stop_local_runtime`，从而不会把残留进程误认作新的启动意图。
...
- `repo_id=0` 时仍可签发 Token，SSH 协议也可在 active create/resume 登记公钥；后续 Git HTTP(S)/SSH、LFS 和 repository API 没有任何 repository 能通过 binding，当前用户与公共服务信息 API 仍按开发凭据章节的登录限制和 route policy 工作。
...
**设计选择：`RequestGiteaToken` 服务于 create/resume 初始化和 running 凭据修复。**初始化阶段定义为 create 或 resume operation 已被绑定 Manager 领取、`operation_status=running` 且 `operation_deadline_unix > now`。Token 在这两个初始化分支和主状态 running 时都能直接访问绑定仓库；用户 open 与工作区 SSH 仍要求主状态 running，两类访问边界彼此独立。active stop 使用 Runtime 本地已有 credential 文件完成日志脱敏；无法恢复精确 mask 时丢弃相关缓冲日志。站点排空期间开发凭据不可使用，因此该 RPC 返回 `state_unavailable`。
...
`EnsureCodespaceGitSSHKey` 为 SSH 协议 Codespace 创建或确认生命周期级公钥。Manager 已通过 Runtime Token、来源 Incus identity 和本地 boot session 校验请求，Gitea 再以 Manager 身份、Codespace binding 和当前初始化状态作最终判定。Runtime 和该 RPC 都不提交 operation 版本；Manager 内部仍使用当前版本完成 metadata 和 final。

handler 取得 Codespace lock，锁内重新读取 Codespace 并按固定顺序处理：

1. 功能启用，调用方 Manager 记录有效、heartbeat 未超时，且与 `codespace.manager_id` 匹配。
2. `git_protocol=ssh`，主状态和 active operation 为已领取的 `creating/create` 或 `stopped/resume`，`operation_status=running` 且 `operation_deadline_unix > now`。
...
5. 关系与 `PublicKey` 都不存在时创建；关系完整且公钥相同时幂等成功。已有不同公钥时返回 `key_conflict`，保持现有绑定。
6. 创建前确认指纹未被普通用户 Key、Deploy Key 或其他 Codespace Key 使用，在短事务中插入 `codespace_ssh_key` 与对应 `PublicKey(KeyTypeCodespace)`。关系存在但 PublicKey 缺失等不完整结果返回 `internal_error`，不根据请求猜测数据库损坏的修复方式。
7. 事务提交后调用 Gitea 现有公钥授权同步入口：内置 SSH 或 `SSH_CREATE_AUTHORIZED_KEYS_FILE=false` 时数据库已经是完整授权来源；使用外部 `authorized_keys` 文件时重写该文件。当前接入面就绪后返回规范化 `known_hosts_lines`；文件同步失败返回 `internal_error`，相同公钥重试会再次同步。
...
**设计理由：公钥绑定属于 Codespace，而不是某次 operation。**create 首次生成密钥，create 重试和 resume 复用；相同公钥请求幂等，不同公钥永远不能覆盖，因此迟到请求不需要 operation 版本也不会改变当前绑定。私钥缺失或不匹配时返回硬错误：create 失败进入 failed，resume 停止本轮实例并回到 stopped，用户随后删除并重新创建 Codespace。密钥材料损坏无法安全恢复，直接失败与清理比增加密钥替换或在线修复流程更明确。

实现验收点：

- HTTP 协议、非初始化阶段、租约过期和错误 Manager binding 均在写入前返回固定分类。
- 相同规范公钥幂等返回相同 Host Key 集合；不同公钥或指纹冲突不会替换当前 binding。
- 公钥登记不要求 repository 仍存在或创建者当前仍有仓库权限；实际 Git SSH 命令在每次连接上重新判定，`repo_id=0` 不能匹配任何仓库。
- 数据库事务始终保持一条关系对应一个 `KeyTypeCodespace` PublicKey；配置使用外部授权文件时，同步失败可以通过同一请求重试，内置 SSH 与 `AuthorizedKeysCommand` 模式无需文件同步。
- stop、delete 或初始化结束先提交后，迟到的确保请求因阶段不匹配而拒绝；确保先提交时，stopped 保留但禁用公钥，failed/deleting/delete 状态事务删除公钥。
- response 只返回匹配当前 Gitea SSH host/port 的公开 Host Key 行，不返回私钥、Token 或内部连接信息。

### RequestIdleStop

`RequestIdleStop` 是 Manager 在本地确认 Codespace 已连续空闲达到有效超时后，请求 Gitea 创建普通 stop operation 的入口。Manager 掌握 HTTP、WebSocket、IDE 和 SSH 的实时连接，适合判断何时空闲；Gitea 掌握用户设置、交互动作和当前生命周期，适合决定该停止在请求到达时是否仍然成立。Manager 直接提交观察到的启用状态、超时和交互版本，Gitea 在一个事务中与当前实际值比较，不需要为两个简单设置计算摘要。
...
Gateway 使用该接口持续复检已建立的 Endpoint 和 SSH session：普通 HTTP 每次请求转发前调用，Endpoint WebSocket 和 SSH 按固定周期调用。request 使用 `oneof session`：Endpoint session 携带 `user_id / codespace_uuid / endpoint_id`，SSH session 只携带 `user_id / codespace_uuid`；调用方 Manager 必须与 codespace binding 匹配。
...
该接口通过互斥 outcome 返回空的 `allowed` 或带稳定 `category` 的 `denied`，只表达当前访问判定，不消费 Gateway Open Token、不写主状态、不记录访问历史。Gateway 仅在收到明确 `allowed` 时保留 session；其他结果都立即关闭本地 session。普通 HTTP 在每次请求转发前调用，WebSocket 和 SSH 按固定周期调用，因此功能关闭后，普通 HTTP 的下一次请求被拒绝，持续连接最迟在一个复检周期内关闭。revalidate 是持续授权边界，无法确认权限时关闭连接。成功 revalidate 不更新 `last_active_unix`；用户实际发起 open 或 SSH 认证时已经完成访问判定，并单独尽力记录活跃时间。

实现验收点：

- 普通 HTTP 每次转发前复检；Endpoint WebSocket 和 SSH session 都能在配置的 revalidate interval 内感知功能开关、登录状态、codespace 状态和 Manager 状态变化。
...
| `lease_valid_for_milliseconds` | 全部 | 普通命令为未取整的精确毫秒租约时长；abort 为 0 |
...
| `repo_clone_url` | create | 按 Codespace 固化协议生成的仓库 clone URL |
...
| `create.git_protocol` | create | 创建时固化的 `HTTP` 或 `SSH` Git 协议 |
| `resume.runtime_settings` | resume | 当前有效自动暂停设置和交互版本 |
| `resume.git_protocol` | resume | 与 create 相同的固化 Git 协议，用于恢复凭据 |
...
- create/resume 的 `git_protocol` 来自 `codespace.git_protocol`，站点默认值变化不会改变已有 Codespace；SSH 模式在 create 首次生成公钥，并在 create/resume 初始化中创建或确认同一公钥绑定，HTTP 模式继续使用限定路径的 Token helper。
...
- Gitea 在数据库保存本次领取或批量续租的绝对 deadline；Manager 只使用 `lease_valid_for_milliseconds` 和本地单调时钟约束 worker，并通过 Fetch observed 批量取得后续授权。
...
- create/resume payload 的 `git_protocol` 等于记录创建时固化的值；HTTP 与 SSH 分支使用各自单一凭据流程，不根据连接失败切换协议。
...
- HTTP 协议的 Runtime 使用 Token 完成 Git smart HTTP clone、fetch 和 push；SSH 协议使用专用 Git SSH Key。IDE、CLI 和脚本在两种协议下都使用同一 Token 调用开发允许面的 API。
...
| Code | branch/tag/commit/status、Git refs/trees/blobs/tags/notes、contents/contents-ext/file-contents；允许 branch/tag 修改、merge-upstream、Contents API 创建/修改/删除文件和 diff patch 创建远端 commit |
...
- `RequestGiteaToken` 同时返回当前 Token 和 Gitea 对外 `server_url`。Manager 将 Token 写入可原子替换的 Runtime credential 文件；HTTP 协议的 Git helper 按 Gitea host 加完整 repository path 匹配，API 客户端在两种协议下都使用同一 credential 和 `server_url`。resume 在 final done 前申请新 Token 并替换旧 credential。其他公开 repository 不携带该 Token 时仍可按 Gitea 现有匿名规则访问。

**设计理由：携带 Codespace Token 访问其他 repository，即使目标公开，也返回 repo binding mismatch，不回退为匿名访问。**认证凭据一旦出现，请求就必须服从其单仓库边界；HTTP 模式通过限定仓库路径的 credential helper 避免把该凭据发送给无关 repository，SSH Key 则由 Gitea 在命令入口限制到当前 binding。其他私有 repository 和需要跨仓库认证的 submodule 不由当前 Codespace 凭据授权。
...
- Codespace Token 在有效 create/resume 初始化期和 `running` 都能使用；凭据行、创建用户登录限制、route、主 repo binding 和创建用户当前权限仍须全部通过。Git SSH 使用后续专用 Key 判定。
...
- Git smart HTTP clone/fetch/push、LFS、Contents/Diff Patch commit、branch/tag/status、Issue、Pull Request/Review/merge、Release、Wiki、Actions 查询/dispatch/rerun/artifact 下载分别覆盖成功与现有权限拒绝。
...
- 携带 Codespace Token 的其他公开 repository Git/API 请求同样拒绝；目标不存在或对创建用户不可见时保持现有 404；HTTP helper 不向无关 repository 发送该 Token，Git SSH Key 不能通过其他 repository binding，匿名访问仍按 Gitea 现有规则工作。
...
[Codespace Git SSH Key](glossary.md#codespace-git-ssh-key) 由 Runtime 的 `init.sh` 生成，Gitea 只保存公钥。`PublicKey.Type=KeyTypeCodespace` 使 Gitea 内置 SSH 和外部 `authorized_keys` 继续使用现有 `serv key-{id}` 强制命令；`serv` 读取 key ID 后，通过 `codespace_ssh_key` 进入 Codespace 专用鉴权，而不是普通用户 Key 或 Deploy Key 分支。

`cmd/serv` 与 private serv handler 在读取 `PublicKey` 后按类型分流。`KeyTypeCodespace` 调用 Codespace 专用服务并返回现有 `ServCommandResults` 所需的 repository、创建用户和 `DeployKeyID=0`；后续 Git subprocess、LFS token 和 hook 环境继续走 Gitea 当前执行路径。普通 `KeyTypeUser` 与 `KeyTypeDeploy` 保持现有服务。客户端没有提交 Git 命令时，`KeyTypeCodespace` 只返回标准的无 Shell 提示，不进入普通用户 Key 的账户介绍分支。
...
Codespace Key 不进入普通用户 Key、Deploy Key、签名 Key 的列表、数量限制和管理 API。普通用户 Key 的读取、编辑、验证和按 ID 删除服务只接受 `KeyTypeUser`，Deploy Key 服务只接受 `KeyTypeDeploy`；即使调用者知道 Codespace key ID，也会在修改数据库前得到资源不存在。创建者详情只展示“Git SSH 凭据是否就绪”和当前登记时间，不展示公钥正文或指纹；治理列表仍不返回凭据细节。该信息用于判断 init 是否完成，运行凭据统一由 Codespace 生命周期管理。
...
- 普通用户 Key、Deploy Key、签名 Key 页面和 API 不返回或管理 `KeyTypeCodespace`；Codespace 创建者详情只展示就绪状态和时间。
...
`POST /-/codespaces/{uuid}/open` 在签发前固定选择 `endpoint_id=workspace`；显式 Endpoint 路由使用 path 中经过 DNS-safe 格式校验的 ID。两种入口都要求当前 metadata ready，普通 Endpoint 还需要存在于 metadata，`workspace` 不要求同名 Endpoint；active operation 只能为空或是本事务将取消的 queued idle stop。两种入口都在 Codespace lock 内写入 code 后推进 `interaction_generation`，取消 queued idle stop；交互事务失败时尽力删除刚写入的 code 并返回失败。只有 cache 与交互事务都成功才返回重定向，提交后尽力更新仅供展示的 `last_active_unix`。两种入口都生成完整 binding；Manager 用 `endpoint_id=workspace` 在 Runtime workspace 与内置 Web SSH 之间解析当前实际目标。
...
9. 校验 Runtime Metadata 仍为 ready；普通 ID 还必须存在于当前 metadata，`workspace` 不要求 endpoints 数组存在同名项。
10. 校验 Manager 仍在线。
...
- `GIT_PROTOCOL=http|ssh` 是新建 Codespace 的默认 Git 协议，默认 `http` 保持已有部署行为。创建记录把当时的值固化到 `codespace.git_protocol`；之后修改配置只影响新记录。HTTP 使用 Gitea 规范 Git HTTP(S) URL 和 Codespace Token，SSH 使用 Gitea 规范 SSH URL 和 Runtime 生成的 Codespace Git SSH Key。系统管理的 remote 始终使用固化协议，连接失败返回该协议的明确错误。
- 启用功能时，全部 setting 和数据库迁移完成后、Codespace Web route、Cron 和 ManagerService 注册前执行传输能力校验。默认协议以及当前 `creating|running|stopped` Codespace 使用的每种协议都必须可用：HTTP 要求 `[repository] DISABLE_HTTP_GIT=false`；SSH 要求 `[server] DISABLE_SSH=false` 且能够取得可信 Host Key。错误列出不可用协议、相关配置和受影响 Codespace 数量，避免控制面已经接受运行工作但仓库接入必然失败。
...
- `ENABLED=false` 使用排空模式并跳过 Git 传输能力校验。stop、delete、inventory 收敛、failed retention 和管理清理都不依赖 Git HTTP 或 SSH；管理员可以在仓库接入不可用时启动 Gitea 完成缩减和清理，再修复当前存量协议所需配置后重新启用。
...
- `ENABLED=false` 使用排空模式，不删除现有 Codespace/Manager 数据。Web 禁止新 create、resume、open、继续运行和 SSH，但创建者详情、创建者日志、创建者自动暂停设置、stop、普通 delete、站点管理员 force delete 与现有管理页继续可用；`ValidateOpenToken`、`VerifySSHPublicKey` 和 `RevalidateGatewaySession` 都返回 `state_unavailable`。普通 HTTP 的下一次请求在转发前关闭，WebSocket 和 SSH 最迟在一个复检周期内关闭。
- 排空模式拒绝 `RegisterManager`、全部 `RequestGiteaToken`、`EnsureCodespaceGitSSHKey`、新的 idle stop 创建和 Runtime Metadata 写入。`RequestIdleStop` 对已经存在的 idle stop 仍返回 `pending`，对没有 active operation 的 running 对象返回包含 `auto_stop_enabled=false` 的 `observation_changed`；ReportInstances 下发相同关闭设置，使 Manager 清除普通计时。已有 Manager 仍可 Declare、轮换自身 secret、ReportInstances、上报 stopped/failed transition，并通过 Fetch 领取 stop/delete 或取得已领取 create/resume 的 abort；对应 UpdateLog 和 final failed 继续可用。stop 从本地 Runtime credential 文件恢复日志脱敏值，无法确认安全的缓冲日志不上传。queued create/resume 不再领取，按现有 queue timeout 处理。
...
- `OPERATION_LEASE_TIMEOUT` 是 Manager 领取或续租 [Operation](glossary.md#operation) 的 lease 时长。该值以正整数毫秒表示；Gitea 将 `grant_time + timeout` 向未来取整到 Unix 秒作为数据库绝对 deadline，并把未取整的精确毫秒值写入成功响应的 `lease_valid_for_milliseconds`；abort 固定返回 0。这个取整方向保证数据库截止边界不会早于 Manager 收到的完整租约时长。
...
- Gitea 是功能开关的唯一判定来源。**设计如此：**关闭后的访问收敛复用三个现有访问 RPC，create/resume 收敛复用 Fetch abort，Runtime Metadata 返回 `state_unavailable`；Manager 依据这些结果处理当前工作，协议保持现有字段和调用方向。
- `ENABLED=true` 时，默认协议和当前 creating/running/stopped 记录所需的 HTTP/SSH 接入在组件注册前全部通过校验；错误指出协议、配置项和受影响数量。
...
tags:
  default:
    ...
    init_path: /usr/local/libexec/gitea-codespace/init.sh
    init_user: coder
    workspace_dir: /workspace
    gitea_token_path: /var/lib/gitea-codespace/gitea-token
    runtime_token_path: /var/lib/gitea-codespace/runtime-token
    git_ssh_private_key_path: /var/lib/gitea-codespace/git-ssh/id_ed25519
    git_ssh_public_key_path: /var/lib/gitea-codespace/git-ssh/id_ed25519.pub
    git_ssh_known_hosts_path: /var/lib/gitea-codespace/git-ssh/known_hosts
    communication_nic: codespace
    address_family: ipv4
    address_subnet: 10.80.0.0/16
```

Manager 当前配置是 `name`、tag 模板键、`capacity_total`、`gateway_url` 和 `gateway_ssh_addr` 的唯一声明来源，修改配置后通过完整 Declare 快照覆盖 Gitea 中的当前值。Declare 的 tags 直接从 `tags` 映射键生成；虚拟机或系统容器类型不向 Gitea 上报，因为 Gitea 只按 tag 匹配。Incus 可用时，`capacity_available` 取运行实例剩余名额和空闲启动 worker 的较小值；Incus 不可用时为 0。project 配额不足以容纳所有 tag 中最大的一个新建模板时，Manager 从 `accepted_operation_types` 移除 create，但有运行名额时仍可接受 resume。`capacity_total` 范围为 1..10000，且单个 Manager 管理的全部 Incus 实例数同样不能超过 10000。Manager ID、manager secret、轮换中的 pending secret、inventory generation、operation worker、有效模板快照与 Gateway SSH Host Key 私钥都保存在 `state_dir`，不写入 YAML；Runtime Git SSH 私钥位于对应 Incus 实例内。两处私钥归属不同，同一状态目录由进程锁保证只能被一个 Manager 进程使用。

`incus.project` 是 Manager 的专用 Incus project。证书应只具有管理该 project 实例所需的权限；Manager 启动时验证 project、权限和 Incus 非集群模式。每个 tag 模板完整指定实例类型、固定 image、profiles、资源、init、Token 路径、Git SSH 文件路径和通信网卡，缺少任一配置字段时启动失败并指出 tag 与字段。三个 Git SSH 路径必须位于同一专用目录且互不相同；init.sh 在 SSH 模式创建目录并按 Runtime 用户权限原子替换文件，HTTP 模式不创建密钥文件。`image` 使用 Incus remote 加完整 fingerprint 的固定引用；Manager 在 create 快照中保存 Incus 返回的 fingerprint。`instance_type` 只允许 `virtual-machine` 或 `container`，两者使用相同的 create/resume/stop/delete 和 init.sh 流程。Manager 从展开设备中取得唯一的 `type=disk,path=/` 根盘设备，用同名实例设备覆盖 `size`，并通过 `limits.cpu` 和 `limits.memory` 写入资源限制。
...
`inventory_report_interval` 默认 1 分钟，必须大于 0；Fetch 默认 2 秒并带 20% 正抖动，临时错误最多退避 30 秒。心跳周期、Runtime Metadata 刷新周期和控制面消息上限来自每次成功 Declare 响应，不在 Manager 配置中重复声明，因为这些值由 Gitea 的离线超时和消息限制决定。
...
- Manager 配置不包含心跳周期、Runtime Metadata 刷新周期或控制面消息上限；首次 Declare 成功后采用服务端值，非法响应保持 recovering 和零容量。
- listener 与对外地址可以不同；Manager 声明值、容量和 SSH 限流均能由上述配置或本地运行状态唯一确定。
- 一个 Manager 身份固定由持有其 `state_dir` 独占锁的单个进程运行；第二个进程或另一状态目录使用同一身份时启动校验失败。
- Endpoint 请求只能选择 `http|https` scheme，不能关闭 HTTPS 证书校验或指定任意 host。
- Declare tags 与配置模板键完全一致；虚拟机与系统容器模板都不会把实例类型写入 Gitea。
- Incus project、证书权限、非集群模式以及 tag 模板字段在领取 create/resume 前完成静态校验；image、展开 profile、根盘、init 路径和通信网卡的运行条件在实例创建或启动后、提交 final 前验证，错误指明对应 tag 与字段。
- 每个 tag 明确配置 Git SSH 私钥、公钥和 known_hosts 路径；SSH 模式只在 Runtime 内生成私钥，Manager 状态目录不保存该私钥。
- 已有 Codespace 使用 create 时保存的有效模板快照；修改或删除 tag 模板只影响之后的 create，不改变已有实例的 resume、stop 和 delete。共享 profile 通过新版本名称演进。
- `capacity_available` 同时受运行名额和启动 worker 限制；project 配额只能容纳已有 stopped 实例时，Fetch 接受 resume 而不接受 create，stop/delete 仍正常领取。
