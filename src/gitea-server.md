...
| `archived` | 返回 repository archived 分类 | 返回 repository archived 分类 | 返回 repository archived 分类 | 允许 | archived 表示仓库进入只读/冻结管理状态，停止交互可以避免继续产生运行侧变化；管理操作仍用于资源回收和日志查看。 |
...
delete 和 stop 是资源回收动作，优先推进可以释放运行侧资源。resume 和 create 会占用容量，由 Manager 当前容量决定领取时机。`operation_rversion` 绑定本次 Gitea 下发的 operation 版本，Manager 后续 operation-bound RPC 都用它校验归属。
...
Manager 负责报告 Gitea-issued operation 的动作结果，Gitea 负责把结果写成主状态、token 状态和日志追加窗口。State Finalization 在同一事务内完成这些写入，保证用户看到一致的生命周期结果。operation 完成后不保留 `done|failed` 状态，失败诊断从 codespace 日志读取。
...
- 重复 final 返回幂等结果。
- State Finalization 同事务处理主状态、active operation 清空、token 和日志追加窗口关闭。
...
| --- | --- | --- |
| `running` 且无 active operation | `stopped` | 写 `status=stopped`、吊销 token、写 `stopped_unix` |
