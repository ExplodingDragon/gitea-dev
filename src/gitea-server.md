...
- 站点管理员在 Manager 永久不可恢复时可通过同一路由提交 `force=true`。服务层要求站点管理员权限和显式确认，直接删除 Gitea 记录、token 和日志，并在响应中明确运行侧资源尚未确认清理。
...
Manager 在本地生成并持久化新 secret 后，使用当前 secret 认证调用该 RPC。Gitea 校验新 secret 格式与熵值，在同一事务内生成新 salt、写入新 hash；成功提交后旧 secret 立即失效，`manager_id`、admin/runtime state 和 codespace binding 均保持不变。该流程轮换现有身份凭据，不通过注册新 Manager 迁移绑定。
...
Fetch 先查询已绑定当前 Manager、`operation_status=running` 且未出现在相同版本 `observed_operations` 或本次 response 中的 operation，重新下发 payload 并刷新 lease。然后再按以下优先级领取 queued operation：
...
- running operation 重发不执行 claim、不递增版本，但刷新 `operation_deadline_unix`。
...
- 首次 final 返回 `result=accepted`。
- 重复 final 同一 `operation_rversion` 且主状态已匹配目标结果，返回 `result=idempotent_done`。
- 过期 `operation_rversion` 或主状态不匹配，返回 `result=stale_operation`，当前主状态保持稳定。
- State Finalization 同事务处理主状态、active operation 清空、token 生命周期绑定和日志追加窗口关闭。
- codespace 已被 delete final 物理删除后，已认证 Manager 对同一 UUID 的重复 final 返回 `idempotent_done`；UUID 不复用，不保存 operation 历史或 tombstone。

### UpdateLog

- 写入 DBFS 路径 `codespace_log/{codespace_uuid}.log`。
- Gitea 服务层可在 create 前置校验阶段通过内部入口写入失败或 warning 摘要；Manager operation payload 携带当前 `log_offset`。
...
- 服务端为最终状态摘要预留固定尾部空间；达到普通日志上限后拒绝原始行，但内部摘要仍可写入预留空间。
...
- request 携带 `metadata_generation`；高于 cache 当前版本时覆盖，相同版本幂等返回且不覆盖，更低版本返回 stale。
- 不写主状态。
- 校验调用方 Manager 已通过认证。
...
Gitea 先检查 active operation conflict，再检查 Manager 管理态。它只接受高于 `codespace.runtime_generation` 的事实；相同 generation 且主状态已匹配时幂等返回，不重复写主状态或 metadata；更低 generation 返回 stale。上报 running 时，`metadata_generation` 还必须通过 Runtime Metadata 版本检查。接受新 generation 后同事务更新主状态、Runtime Metadata 和 `runtime_generation`。`ReportRuntimeTransition` 不递增 `operation_rversion`，因为它不是 Gitea 下发的 operation。
...
- `gitea_token_id/gitea_token` 只有一个存在，或 token 行已经不存在时，生命周期服务先幂等清理不一致字段，再按当前工作状态重新签发。
...
- 公钥认证解析 `public_key_blob`，并确认其通过 Gitea 现有 SSH key 模型归属于 codespace 创建用户且 `Type == KeyTypeUser`（`models/asymkey/ssh_key.go`）；部署密钥（`KeyTypeDeploy`）和授权主体（`KeyTypePrincipal`）不接受。若站点强制 2FA，用户必须已启用符合站点要求的 2FA。
- 创建用户当前允许登录。
...
该接口只返回当前访问判定，不消费 Gateway Open Token、不写主状态、不记录访问历史。拒绝时 Gateway 关闭本地 session；成功 revalidate 不更新 `last_active_unix`，用户实际发起 open 或 SSH 认证时已经记录活跃时间。
...
完整快照只有在 `inventory_generation` 高于 `codespace_manager.inventory_generation` 时才计算 missing 并更新该字段；相同版本幂等返回且不重复计算，更低版本返回 stale。`instances[].runtime_state` 用于发现运行状态分歧，Gitea 返回 report transition instruction，主状态变更仍通过 `ReportRuntimeTransition` 完成。
...
| reported runtime_state 与无 active operation 的 `running/stopped` 主状态不一致 | 返回 `report_runtime_transition` instruction，由 Manager 携带新 generation 上报事实 |
...
| `create` | create | create 专属结构，其他 operation 不设置 |
...
- stop final、failed final 或 delete 请求进入 `deleting` 时吊销 token 并清空 `gitea_token_id/gitea_token`；物理删除再次执行幂等兜底清理。
...
- `repo_id` 为空时仍可签发 token，但任何 repository 访问都返回 repo binding mismatch。
...
`reconcile_codespace_states` 定时扫描 active operation、Manager 离线和 Runtime inventory 差异，让 codespace 按状态机进入明确结果。queued operation 使用 `operation_created_unix + QUEUE_TIMEOUT` 判定等待超时，running operation 使用 `operation_deadline_unix` 判定 lease 超时。codespace 只有一份连续日志且不保存 operation 历史，因此日志随 delete 或 `cleanup_failed_codespaces` 一起删除，不按“已完成 operation”单独清理。

实现验收点：

- reconciliation 只使用新版本完整 inventory 处理 missing，并收敛 token 残留。
...
- `LOG_MAX_SIZE` 为普通日志保留最终状态摘要空间，达到限制后仍能写入明确的截断摘要。
