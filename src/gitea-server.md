...
每个 owner scope 最多保存一行 registration token。`GetOrCreateRegistrationToken` 在行存在时返回原明文，不存在时才创建；`RotateRegistrationToken` 在 Codespace owner relation lock 和同一事务中原地替换随机 token；`DeactivateRegistrationToken` 物理删除当前行，没有记录时幂等成功。settings 页面可以显示当前 token 明文，因为它是可重复使用的部署注册凭据。`owner_id=0` 的 global token 只由站点管理员管理，用户或组织删除时物理删除所属 scope 的 registration token。读取、轮换和停用使用不同动作，可以避免管理员仅打开页面就意外轮换正在使用的凭据。
...
- 先按唯一 token 索引只读取候选 `owner_id`，该读取不构成认证成功；随后取得对应 Codespace owner relation lock（包括 `owner_id=0` 的 `codespace_owner_0`），在锁内按 token 重新读取当前行。复读仍存在且明文匹配时才有效，期间已 Rotate/Deactivate/owner delete 则返回 `unauthenticated`。
...
- 注册与同 scope token 的 GetOrCreate、Rotate、Deactivate 使用同一 Codespace owner relation lock；并发结果由取得锁的顺序决定：注册先完成时创建 Manager，凭据变更先完成时旧 token 的注册返回未认证。
...
- `protocol_version` 固定等于当前 ManagerService 主版本 1，不保存到 `meta_json`、Manager 行，也不参与 Manager matching。软件 `version` 和最近一次 Declare 都不能替代当前 request 的检查。
...
- Declare 协议版本不匹配时不更新声明字段、地址或 `last_online_unix`；协议不匹配的 Manager 不能保持 online。
...
- User、Deploy 和 Codespace 公钥创建使用同一 PublicKey 指纹锁并在事务内复查；不同 Codespace 和跨类型并发提交同一指纹时只有一个符合类型规则的结果，历史重复指纹返回数据完整性硬错误。
...
- Gitea 认证组增加两个轻量适配器，而不是在 `auth.Group` 外重新实现一套凭据排序。`CodespaceQueryBearer` 位于现有 OAuth2 方法之前，复用 OAuth2 当前的 `token`、`access_token`、Bearer 选择函数；`CodespaceBasic` 位于现有 Basic 方法之前，复用 Basic 当前从用户名或密码选择 Token 的规则。非 `gcs_` 值返回“不适用”，原有认证方法继续按当前顺序处理。
- 两个适配器选中的凭据以 `gcs_` 开头时调用同一个 Codespace resolver。resolver 按末八位执行一次候选查询，每个候选行同时关联 Codespace、创建用户，并通过数据库 `EXISTS` 取得与 `HasTwoFactorOrWebAuthn` 等价的 TOTP/WebAuthn 配置状态；查询还读取判定当前初始化期所需的 operation 类型、状态和截止时间，并派生 `development_credential_allowed`。随后在 Go 中以行内 salt 常量时间校验 verifier。匹配成功后，把真实用户以及包含 `kind=codespace / codespace_uuid / repo_id / status / development_credential_allowed / login_allowed` 的请求内认证数据写入 request context，同时设置 Gitea 现有 API scope middleware 所需的派生数据。
- 凭据不存在、verifier 不匹配、记录损坏，或关联 Codespace/用户不存在时，适配器返回类型化的“专用凭据已拒绝”错误。`auth.Group` 只对这个新增错误立即结束，不尝试后续 OAuth、Basic、Session、Reverse Proxy 或 SSPI；其他认证方法原有的错误继续沿用 Gitea 当前回退语义。数据库等基础设施错误按内部错误结束请求。这样只有已经被现有顺序选中的 `gcs_` 取得排他处理权，不改变非 Codespace 凭据的组合行为。
- API query token 服从 Gitea 现有 `DISABLE_QUERY_AUTH_TOKEN` 和候选优先级；配置关闭时 query 参数不进入 `CodespaceQueryBearer`。Web 认证入口只处理 Authorization header，并在 Session 之前执行相同的 Bearer/Basic 适配器。Git HTTP 和 LFS 的现有认证入口复用相同解析函数、resolver 和专用拒绝错误。
- API 开发入口在具体 method/route 注册处使用可检查的类型化 marker 写入请求级策略 `self|public_info|repository|signed_artifact`；marker 同时提供认证前 middleware 和只读注册元数据。普通 `Methods` 注册展开每个 HTTP method，并记录完整 route pattern 与 policy；没有 marker 就是 `unsupported`。pre-middleware 在 API `AfterRouting` 认证链之前执行，因此后续认证和统一守卫读取路由器已经匹配的精确 policy，不解析 URL 前缀，也不从能力说明推导权限。
- `RouterPathGroup` 支持具体 matcher 的 pre-middleware。外层 wildcard route 在 API 认证前只执行一次内部 matcher 选择，将所选 matcher、解析后的参数、具体 route pattern 和 policy 写入请求上下文；后续 `ServeHTTP` 直接执行该 matcher 的普通 middleware 与 handler，不再次匹配。没有内部 matcher 命中时不写 policy，并进入原有 NotFound。注册元数据把外层前缀和内部 pattern 合成为清单使用的完整路径，例如 `/commits/<ref:*>/status`。
...
开发 API 的能力范围如下。该表解释为什么需要这些 API；可执行的完整 method/route 集合由 [`codespace-token-routes.yaml`](codespace-token-routes.yaml) 定义：
...
| Pull Request | list/get/create/edit/close/reopen/delete、diff、commit/file、comment、review、reviewer request、update branch、立即 merge、scheduled auto-merge 的创建/取消，以及 merge 后按 Gitea 现有规则删除 head branch；删除复用 `DELETE /issues/{index}`，由 Gitea 现有 Issue 服务识别并清理 Pull Request 关系与 Git ref |
...
能力分类不用于推导路由。路由注册处按 [`codespace-token-routes.yaml`](codespace-token-routes.yaml) 的每个 method/path 安装 `self|public_info|repository|signed_artifact` marker；混合读写 group 和 `RouterPathGroup` 都逐个 method 或 matcher 注册。清单的 `incoming_method_rules` 同时声明 Gitea 现有 HEAD 到 GET 的匹配行为，显式 HEAD handler 则直接列入对应 route。守卫把没有 marker 的请求解释为 `unsupported` 并返回 403，因此父 group、相同 path 的其他写方法、未命中的 PathGroup 路径和后来新增的 Gitea API 都不会从相邻路由继承权限。

路由器在构建期间保存不可变的 marker 注册快照。相同 method/path 出现重复 marker、冲突 policy，或 PathGroup 内部 pattern 无法形成完整注册路径时直接触发 Go `panic`，使 Gitea 初始化或测试立即失败。实现测试从该快照收集全部 `(HTTP method, route pattern, policy)`，规范化并排序后与 YAML 清单比较。YAML 是设计侧的完整允许面，不参与运行时 URL 匹配；marker 是运行时唯一判定来源。变更开发能力时必须在同一提交中修改清单、路由 marker 和对应权限测试，三者任一遗漏都会使测试失败。

**设计如此：普通 route 与 PathGroup matcher 都在认证前产生同一种精确 marker，并使用一份可读取的清单审阅完整允许面。**把 policy 挂在 `/commits/*` 外层会使以后新增 matcher 自动获得权限；在认证后重新解析路径则会复制路由逻辑。认证前单次选择具体 matcher 可以保留带斜线 ref 的现有 API，又让未列 matcher 默认拒绝。YAML 负责完整审阅和差异检测，注册 marker 负责请求执行，两者通过不可变注册快照和测试保持一致。
...
Actions artifact metadata 和原始下载入口属于开发允许面，artifact delete 仍拒绝。最初的 artifact 下载请求使用 `repository` policy，完整检查 Codespace Token、工作状态、repository binding、创建用户当前权限和 artifact 后，沿用 Gitea 现有两种结果：存储支持直传时重定向到对象存储签名 URL，否则生成 Gitea raw URL。

raw GET route 使用 `signed_artifact` policy，并把现有 HMAC、expires、artifact ID 和 artifact 状态校验作为该 route 的完整认证方式。marker 在 `AfterRouting` 认证链前写入 policy；API authentication、严格登录检查、Codespace Token 守卫和普通 scope 检查看到该 policy 后不解析或使用 Session、PAT、Basic、query token 或 `gcs_` 凭据，直接把请求交给 raw handler。有效、失效或已经随 stop 删除的 Codespace Token header 都不改变签名校验结果。客户端跟随对象存储 302 时仍不转发 PAT，避免把 Gitea 凭据发送给对象存储，并使两类签名 URL 都可以独立使用到原定过期时间。
...
- repository route 先沿用 Gitea 现有 assignment 与可见性错误，再比较 binding；`self|public_info` 按各自固定目标处理，`signed_artifact` 跳过普通身份认证并由签名 handler 独立授权，其余 route 返回 unsupported resource。
...
- 普通 route 和 PathGroup 具体 matcher 的精确 marker 都在认证前写入 policy；PathGroup 只匹配一次并复用已选 matcher，未命中路径不取得父级 policy。Web 认证后守卫默认拒绝绑定 Token；只有 Git smart HTTP、LFS 和显式标记的 API route 能通过授权点，普通 Web、未标记 API 和同 group 未标记 method 在副作用前稳定拒绝。
- 测试从不可变注册快照收集的全部 `(method, route pattern, policy)` 与 `codespace-token-routes.yaml` 一致；清单中没有重复三元组、未知 policy 或无法匹配的 route，并验证无显式 handler 的 HEAD 按清单规则使用 GET marker。重复或冲突 marker 在路由构建阶段触发 `panic`；任何 marker 增删或 policy 变化都会使测试失败，YAML 不进入运行时请求路径。
...
- artifact delete 返回 403；工作状态可通过 `repository` policy 取得 artifact 下载 URL，启用严格登录检查时无 PAT 的有效 raw 签名仍可下载。`signed_artifact` route 不解析 Session、PAT、Basic、query token 或 `gcs_`，有效、失效和已删除 Token header 不改变签名结果；stop final 提交后尚未通过最初授权点的请求不能签发新 URL，已经签发的 artifact 或对象存储 URL 使用到原过期时间。
- 新增 API route 只有同时写入精确清单并安装相同 Codespace route policy 才进入允许面；其余 route 使用 `unsupported` 并返回 403。
