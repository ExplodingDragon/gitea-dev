...
- 默认 open 不依赖 Runtime Metadata 中存在 `workspace` 记录；显式 Endpoint 路由拒绝 `workspace` 并只打开当前 metadata 中的普通 Endpoint。
...
| Endpoint | 默认 `workspace` 要求当前 metadata 已 ready；其他 [Endpoint](glossary.md#endpoint) 必须存在于 metadata |
...
- 规范化 `gateway_url` 在全部已注册 Manager 中唯一。首次声明或 URL 变化时，Gitea 在单实例全局锁内按 `scheme + lower-case host + effective port` 检查冲突；相同 Manager 的未变化 heartbeat 直接复用现值。Endpoint host 不包含 `manager_id`，唯一 origin 保证 DNS 请求只到达持有对应 Runtime 映射的 Manager deployment。
...
最终 create claim 使用单条条件更新重新确认未绑定 queued create、当前 Manager enabled/online、`repo_tag` 仍属于最新 tags，并要求 `repo_id>0` 且 repository 记录仍存在；owner-scoped Manager 还要求该 repository 当前 `owner_id` 等于 Manager owner，global Manager 不限制 owner。并发 repository transfer 与 claim 以该语句看到的数据库顺序为准：claim 先成立时 binding 固定，transfer 先成立时旧 owner Manager affected rows 为 0。这样不需要跨服务复用 repository 工作锁，也不会让候选查询与条件更新之间的 owner 变化产生错误 binding。

`max_operations` 范围为 `1..256`；`observed_operations` 最多 10000 条且 UUID 不得重复。每种优先级使用 `operation_created_unix, uuid` keyset 分页，按升序处理；单次 Fetch 在稳定 scope/tag 筛选后合计最多检查 1024 个 queued 候选，防止每增加一种 operation 类型都线性放大数据库读取。Manager 调用 Fetch 或续租的周期不超过 `OPERATION_LEASE_TIMEOUT / 3`。disabled Manager 不领取 queued create/resume；queued 和 running stop/delete 继续正常处理。abort create/resume 不续租，lease 先超时时 Gitea 已写入的 failed 与 Manager 随后提交的 `final failed` 幂等收敛到同一结果。
...
单次 Fetch 不持有覆盖整批操作的事务。running lease 刷新和每条 queued claim 都在各自短事务中条件更新；claim 提交后再构造 payload。create repository/user 数据加载或 payload 构造失败时，服务在单独短事务中按当前 `codespace_uuid + operation_rversion + manager_id + operation_status=running` 条件释放该 claim：恢复 queued 和 operation 时间字段，create 同时恢复 `manager_id=0`。该候选失败会写服务端日志并继续处理同批后续候选。数据库连接等系统性错误终止本次 RPC 且不返回 response；已经提交的 claim 保持 running，下一次 Fetch 通过 running payload 重发恢复。每条 claim 独立提交与 Actions 领取任务的事务边界一致，也让响应丢失和中途故障使用同一种恢复路径。
...
- 上述仍在有效执行边界内的已观察 operation 由 Fetch 刷新 lease，且不计入 response 的 `max_operations`；disabled create/resume 在 deadline 未到期或到期后仍处于有效 grace 时返回 abort 并计入 response，超过可恢复边界时直接 timeout。
...
- Gitea 信任 Runtime Metadata cache 仅用于普通 Endpoint existence check、`workspace` ready check 和 UI 展示。主状态校验基于数据库 `codespace.status`，与 cache 信任无关。
...
| `running/stopped` 且无 active operation | `failed` | 写 `status=failed`、吊销 token 并清除交互 cache；不伪造停止时间 |
...
running fact 在 Codespace lock 内先幂等写入或验证 metadata cache，再提交 `status/runtime_generation` 数据库事务；数据库失败时 stopped 主状态继续阻止交互。相同 generation 的 running 重试必须补写或验证 metadata，不能只因数据库主状态已经 running 而跳过。stopped/failed fact 先提交主状态、runtime generation 和 token 事务，再幂等清除 Runtime Metadata 与未消费 Open Token；响应丢失后的同 fact 重试先补做 cache 清理再成功返回。failed fact 首次提交时写 `updated_unix=now`，幂等重试不刷新该字段，并通过内部日志入口追加固定摘要；详细原因只写 Manager 本地日志。
...
- running fact 重试可修复丢失的 metadata cache；stopped/failed 重试可补做 cache 与 Open Token 清理。
...
- creating/running 且创建用户存在时返回当前 token 或在空 binding 上签发一个新 token；running stop 只可读取现有 token，不会签发或轮换。
...
- 验证时执行运行时检查（codespace 状态、用户权限、有效 Endpoint），而非仅检查 code 是否有效。普通 Endpoint 必须仍在 Runtime Metadata 中；`workspace` 只要求 metadata 已 ready，实际连接 Runtime 同名 Endpoint 还是默认 Web SSH 由 Manager 决定。
...
- 用户或组织删除事务完成后，关联 access token、codespace token binding 和 codespace 记录同时不存在。
...
`POST /codespace/{uuid}/open` 在签发前固定选择 `endpoint_id=workspace`；显式 Endpoint 路由使用 path 中经过 DNS-safe 格式校验的 ID。普通 Endpoint 需要存在于当前 metadata，`workspace` 只需要 metadata 已 ready。两种入口都生成完整 binding，Gitea 不签发空 `endpoint_id`，Manager 也不需要为 Web SSH 增加特殊 token 类型。
...
index value = 尚未消费的 code_hash 集合
...
- code 签发、消费和 Codespace 物理删除按 `codespace_uuid` 使用同一本机 keyed lock；签发同时写 code 与 index，消费同时删除二者，物理删除通过 index 清除全部未消费 code。
...
2. 在本机锁保护下以 `code_hash` 查询 cache，命中后立即删除 code 并从对应 index 移除。
...
8. 校验有效 Endpoint：普通 ID 仍存在于当前 Runtime Metadata；`workspace` 的 metadata 仍为 ready。
9. 校验 Manager 仍在线。
...
- Codespace 物理删除后，其 Runtime Metadata 和仍可索引的未消费 open code 均从 cache 删除；即使 index 已丢失，后续消费也因 Codespace 不存在而拒绝。
...
`cleanup_failed_codespaces` 对到期记录直接执行 Gitea 本地物理删除：清理 access token、明文 token binding、单文件日志、数据库记录以及对应 cache，不创建 delete operation，不联系 Manager，也不检查 Manager 的 enabled/online/recovering 状态。failed 已经是不可恢复终态，保留期只用于给用户读取日志和手动 delete；到期后继续等待运行侧确认不会增加 Gitea 数据安全性，反而会让终态记录无限保留。Manager 以后上报该 UUID 时按无数据库记录处理，不返回 `cleanup_local_runtime`。
