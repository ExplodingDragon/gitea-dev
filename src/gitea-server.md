...
| create 目标 ref/commit 不可解析 | 返回 ref not found 分类 | 已有 codespace 按锁定 `commit_sha` 管理 | 允许发起 resume，Manager checkout 失败时进入 `failed` | 允许 | create 需要锁定 commit；已有 codespace 已保存 `commit_sha`，原 ref 后续移动或删除不改变该历史对象的管理能力。 |
...
delete 和 stop 是资源回收动作，优先推进可以释放运行侧资源。resume 和 create 会占用容量，由 Manager 当前容量决定领取时机。`operation_rversion` 绑定本次下发的 operation 版本，Manager 后续 operation-bound RPC 都用它校验归属。
...
| `stop` | `status=stopped, operation_status=done` | `status=failed, operation_status=failed` |
| `delete` | 物理删除 codespace、日志和绑定数据 | `status=failed, operation_status=failed` |

Manager 负责报告动作结果，Gitea 负责把结果写成主状态、token 状态和日志封闭状态。State Finalization 在同一事务内完成这些写入，保证用户看到一致的生命周期结果。
...
| missing `running` runtime | 记录 divergence，吊销 token，进入 `failed` |
...
| 字段 | 适用类型 | 说明 |
| --- | --- | --- |
| `operation_rversion` | 全部 | 当前 operation 版本 |
...
- stop/delete/failed/source repo 删除/user 删除时吊销 token。
- 同一 codespace 的 active token 对应 `creating`、`running`，以及 `stopped` 下的 active resume operation。`stopped` 且没有 active resume、`deleting`、`failed` 状态下 token 已吊销且不可申请。
...
- 被绑定 codespace 处于 `creating|running`，或处于 `stopped` 且当前为 active resume operation。
...
| 任务 | 默认调度 | 职责 |
| --- | --- | --- |
| `reconcile_codespace_states` | `@every 1m` | 检查 queued operation timeout、running operation lease、Manager offline/recovering 和 Runtime inventory 差异，按状态机处理并更新 token |
| `cleanup_failed_codespaces` | `@daily` | 清理超过保留期的 `failed` 状态 codespace 记录和日志；清理超过保留期的 inactive registration token |
