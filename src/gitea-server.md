...
确认页按照仓库与 ref、运行环境、Dev Container、附加仓库权限、推荐 Secret 的顺序展示创建条件。页面使用一个表单承载最终选择和一个创建按钮，因为这些输入共同组成同一次创建决定；预览刷新只携带 ref 与 Dev Container 选择，不把权限或 Secret 放入 URL。运行环境由用户从站点全局 Manager 与当前用户个人 Manager 已声明的 tag 中选择。环境 tag 由部署者映射到实例类型、镜像、资源和网络；仓库 Dev Container 只定义内部开发环境，不控制这些基础设施信息。
...
附加仓库权限按仓库分组，每个仓库单元使用无权限、读取或写入单选项表达本次授权；推荐 Secret 使用分隔列表展示名称、说明和当前可用状态。创建校验失败时页面可以保留环境与权限选择，但不回显 Secret 明文。没有可见环境时页面明确说明基础设施尚不可用并提供返回仓库的操作，而不是展示一个无法提交的表单。
...
**设计理由：创建入口位于 repository 代码页的 Code 面板。**创建请求需要明确的 repository/ref 上下文，因此复用该页面已经解析好的 repository、ref 和权限结果；`GET /-/codespaces` 用于展示当前用户已有对象，顶部导航中的 Codespaces 入口直接进入该列表。环境选择位于创建确认页，是因为该页已经集中展示 Dev Container、附加仓库权限和 Secret，用户可以在一次提交前确认完整运行条件。
...
- 确认页只有一个最终创建表单，按运行环境、Dev Container、附加权限和推荐 Secret 的顺序展示；错误响应不回显 Secret 值。
...
详情页使用 repository 名称作为对象标题，并在标题下展示 ref、可跳转的短 commit 和创建时间。UUID 继续作为路由和服务端对象标识，但不占用创建者页面的主要信息区域，因为创建者定位工作区时使用 repository 和 ref 更直接。状态摘要和当前可用操作位于同一全宽页头，长 repository、ref 和操作按钮可以分别换行，避免窄栏挤压。
...
Overview 使用较宽的连接区域和较窄的运行上下文区域。Workspace 是页头唯一的主要打开操作，因此在 Overview 和 Logs 中位置一致。当前允许的普通 Continue、Stop 和 Delete 也直接显示在页头：Continue 与 Stop 使用次要按钮，设置使用图标按钮，Delete 位于末尾并使用红色确认样式。详情页动作数量有限，直接显示可以让用户清楚看到当前生命周期能力和删除影响，不需要通过更多菜单寻找。额外连接方式按用途分为 SSH 和 Ports 两个独立章节：SSH 使用完整宽度的命令复制控件，主机密钥算法、SHA256 指纹和更新时间收纳在可展开的校验信息中；Ports 按 Service、Port、Visibility 和打开操作分列，操作列表头只提供无障碍名称，移动端把名称与操作放在第一行、端口与访问范围放在第二行。运行上下文展示环境标签、ref、可跳转的短 commit、创建时间、最后活动时间、当前有效自动暂停和 CPU、内存、磁盘用量。Runtime 就绪时展示当前连接信息和实时资源，其他状态集中展示仍然有效的运行上下文。
...
- 用户 repository 和组织 repository 都通过代码页 Code 面板的 Codespaces tab 展示当前用户在该 repository 下的对象；创建入口使用 `GET /{owner}/{repo}/codespaces/new` 生成确认计划，最终创建使用 `POST /{owner}/{repo}/codespaces`，并由服务端解析 repository、ref 和 commit。
...
- 创建入口只有 repository 代码页 Code 面板中的确认页链接，确认页提交到同一 repository 的 Codespace 集合；全局页面只负责列表和对象详情。
...
- 当前允许的普通 Continue、Stop 和 Delete 在页头直接显示，Delete 位于末尾并保留红色确认样式；详情生命周期操作不使用更多菜单。Ports 的图标操作列宽度稳定，列表头只提供无障碍名称。
...
创建者详情采用与 Gitea 其他对象页一致的全宽标题、状态摘要、操作区和页签。标题使用 repository 名称，ref、可跳转的短 commit 和创建时间作为次要信息；UUID 只承担路由和服务端对象身份。Overview 的连接区域占主要宽度，运行上下文在桌面端位于较窄的第二列，低于 992 像素时按自然文档流排列。连接区域按 SSH 和 Ports 分节，Workspace 打开操作固定在页头。页头始终直接展示当前可执行的 Continue、Stop 和 Delete：主要的 Open Workspace、Resume 或 Continue 位于最前，普通 Continue 与 Stop 使用次要按钮，设置使用图标按钮，Delete 作为最后一个红色确认操作。**设计如此：**详情页的生命周期操作数量有限且影响明确，直接展示比单独的更多菜单更容易发现，也能让删除操作的危险样式始终可见；按钮按当前服务端权限自然出现，不为不存在的动作预留位置。
...
- 状态、操作和页签使用全宽页头，Workspace 只在页头提供一次主要打开操作。当前允许的普通 Continue、Stop 和 Delete 直接显示，Delete 位于操作区末尾并保留红色确认样式；详情页不使用生命周期更多菜单。SSH 使用独立章节和完整宽度命令控件；Ports 使用 Service、Port、Visibility 和固定图标操作列，操作列表头只提供无障碍名称，移动端使用两行结构。主机密钥校验可以展开查看并复制指纹。Auto-stop 使用状态片段之外的公共弹窗，状态轮询不会覆盖正在编辑的模式和时间。
...
- `DeclareManager` 提交 Manager 客户端当前稳定配置和运行状态的完整快照，不是注册后不可修改的配置。客户端可修改并重新声明名称、版本、Gateway/SSH 地址、环境 tag 与说明、SSH host key 信息和 `manager_runtime_state`；容量由 Manager 本地配置与每次 Fetch 的当前可用槽位表达。
...
runtime:
  ...
  environments:
    - tag: debian-lxc
      description: Lightweight Debian development environment
      type: lxc
      source:
      ...
      resources:
        ...
        root_disk: 10GiB
    - tag: company-base
      description: Company virtual machine baseline
      type: vm
      source:
        instance:
          project: base-images
...
Manager 当前配置是 `node.name`、`runtime.environments[].tag`、`runtime.environments[].description`、`gateway.http.public_url` 和 `gateway.ssh.public_addr` 的唯一声明来源，修改后通过完整 Declare 快照覆盖 Gitea 中的当前值。Declare 的环境声明直接从 `runtime.environments[]` 生成。`node.capacity_total` 只限制 Manager 本地 Runtime 总数，不进入 Declare。**设计如此：**tag 是 Gitea 侧让用户选择基础设施环境的稳定标识，description 解释该选择；二者都不是 Incus image、profile、project 或实例名称，镜像和 profile 调整不应改变 Gitea 中的用户语义。虚拟机或系统容器类型不向 Gitea 上报，因为 Gitea 只按 tag 匹配。普通配置不包含 `startup_capacity_available`、`cleanup_capacity_available`、Gateway SSH Host Key 指纹或 Host Key 更新时间：两类可用容量由 Manager 在 Fetch 前按本地 Runtime 和 worker 状态计算，Host Key 指纹由 `node.state_dir` 中的 Gateway SSH Host Key 派生后 Declare。
