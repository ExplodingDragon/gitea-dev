...
delete 和 stop 是资源回收动作，优先推进可以释放运行侧资源。resume 和 create 会占用容量，由 Manager 当前容量决定领取时机。`operation_id` 绑定本次下发动作，Manager 后续 operation-bound RPC 都用它完成归属校验。
...
- stale `operation_id` 返回 stale 分类，当前主状态保持稳定。
...
Manager 通过 `ReportInstances` 上报本地 Runtime inventory snapshot。

request 字段：

| 字段 | 说明 |
| --- | --- |
| `snapshot_id` | 本次 inventory snapshot 标识 |
| `snapshot_complete` | 本次上报是否为完整 snapshot |
| `instances[].codespace_uuid` | 本地 Runtime 对应 codespace UUID |
| `instances[].runtime_state` | Manager 观测到的 Runtime 状态 |
| `instances[].observed_operation_id` | Manager 观测到的本地 operation 标识 |
| `instances[].observed_unix` | 本次观测时间 |
...
| missing `stopping` runtime | 接受当前 stop operation 收敛到 `stopped` |
...
stale report 使用分类响应而不是改写主状态，是因为 Manager 上报可能来自旧 lease、重启后的残留任务或已经被 Gitea reconciliation 接管的 operation。保持主状态不变可以保护 Gitea 的状态权威，同时给 Manager 明确 cleanup 或停止上报的信号。
...
| 任务 | 默认调度 | 职责 |
| --- | --- | --- |
| `reconcile_codespace_states` | `@every 1m` | 检查中间态（booting/stopping/resuming/deleting）的 operation deadline、recovery deadline、Manager offline timeout 和 Runtime inventory 差异，按状态机收敛并处理 token |
...
`reconcile_codespace_states` 定时扫描中间态 codespace，处理 `operation_deadline_unix` 超时、`operation_recovery_deadline_unix` 到期、Manager 离线和 Runtime inventory 差异，让 codespace 按状态机收敛到明确结果。`cleanup_failed_codespaces` 和 `cleanup_codespace_logs` 分别处理数据生命周期，控制过期记录和日志的长期堆积。
