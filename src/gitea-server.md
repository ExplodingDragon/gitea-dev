...
站点管理页列出所有 Codespace 和 Manager，并提供 force delete、Manager enable/disable/直接删除及 global registration token 管理；用户和组织设置页只管理当前 owner scope 的 registration token 与 Manager。第一版不增加对应 public API，页面直接调用同一 Codespace 服务层，避免 Web 与后续内部调用形成两套删除或凭据规则。

Manager 管理态通过 `SetManagerAdminState(managerID, targetState)` 修改。服务取得目标 Manager lock，在短事务中重新读取记录；目标值与当前值相同则幂等成功，否则只更新 `admin_state`，保留 heartbeat、runtime state、tags、地址、binding 和 active operation。重新启用后直接按现有 `last_online_unix` 和 `runtime_state` 派生可用性：仍在 online 时效内即可参与领取，heartbeat 已超时则继续视为 offline，直到 Manager 成功 Declare。管理态只表达 Gitea 是否接受新运行和交互，不伪造 Manager 在线事实，也不向 Manager 发送指令。
...
Manager 删除确认页展示绑定 Codespace 数量，并说明 Gitea 会同步删除这些 Codespace、token 和日志，但不会联系 Manager 或保证 Runtime 回收。确认后，服务层不检查 Manager admin/runtime state，直接提交本地删除事务；删除与 Manager/Codespace 写入共用 keyed lock，已经通过认证的并发 RPC 也必须在最终写入前重新检查记录和 binding。
...
- Manager enable/disable 在 Manager lock 内幂等更新且不修改 heartbeat；重新启用后仍按当前 heartbeat 和 runtime state 派生可用性。
...
- `manager_id`、`owner_id`、`admin_state`、secret verifier、inventory generation 和已有 Codespace binding 不由 Declare 修改。Manager secret 只通过 `RotateManagerSecret` 轮换，`admin_state` 只通过 Gitea 管理入口修改。
- 更新 `last_online_unix`。
...
- API 开发入口在具体 method/route 注册处使用 Gitea `PreMiddlewareProvider` 写入请求级策略 `self|public_info|repository|signed_artifact`；没有标记就是 `unsupported`。pre-middleware 在 API `AfterRouting` 认证链之前执行，因此认证后的统一守卫能读取已经匹配的精确 route，而不需要解析 URL 前缀或维护第二份运行时路径表。
- API 统一守卫位于现有 PAT 认证和登录限制之后、`sudo` 与其他 route middleware 之前。Codespace token 携带非空 `sudo` query 或 `Sudo` header 时返回 `unsupported_resource`；即使创建用户是站点管理员，也保持 `ctx.Doer == codespace.user_id`，不切换执行身份。
- 每个 API、Git HTTP 或 LFS 请求在副作用前按 `AccessTokenID` 用一条数据库查询重新读取 access token row 及可选的 `codespace.gitea_token_id` binding。row 与 binding 同时存在时进入 Codespace 校验，只有 row 时按普通 PAT 继续，row 已不存在时返回现有 401。该查询是本次请求的授权生效点：stop/failed/delete 在查询前提交则拒绝请求，在查询后提交则当前请求按现有 Gitea 行为完成，后续请求拒绝。
...
- repository API 守卫复用现有 `repoAssignment` 的 owner/repository 解析、重定向、可见性和创建用户权限逻辑。实现把其主体提取为请求内幂等的 repository assignment；Codespace 守卫提前调用后，原 route middleware 复用结果。repository 不存在或对创建用户不可见时保持现有 404；已经解析且可见但 ID 不等于 `codespace.repo_id` 时才返回 403 repo binding mismatch。
...
**设计说明（有意如此）：授权生效点是副作用前的 token row + binding 单次重读，不是最初解析 Authorization header 的时刻。**这使 stop 与新请求只有“请求先获得授权并完成”或“吊销先提交并拒绝请求”两种完整结果；系统不尝试取消已经进入 Git subprocess、LFS 传输或 API handler 的请求。
...
- 授权点以一条查询重新读取 token row 与可选 binding；只有 row 时按 Gitea 普通 PAT 逻辑处理，row 已不存在时返回现有 401，row 与 binding 同时存在时继续以下判定。
...
- codespace-bound token 只有在 `creating/running` 状态且 token ID、route、主 repo binding 和创建用户当前权限均通过时才能使用 Git/LFS 或开发 API。
...
- API 精确 method/route marker 在认证前写入 policy；只有 Git smart HTTP、LFS 和显式标记的 API route 能通过授权点，普通 Web、未标记 API 和同 group 未标记 method 在副作用前稳定拒绝。
- 授权点单次重读 token row 与 binding；并发 stop 在授权点前提交时返回 401，在授权点后提交时当前请求完成且后续请求拒绝，不会把已吊销 Codespace token 当作普通 PAT。
