...
- lease 过期后 progress 和 lease update 返回 lease expired 分类。
- 终态 `done|failed` 在 `deadline_unix` 已过期但 reconciliation 接管前仍可由 State Finalization 处理。
- lease 过期但 reconciliation 标记 failed 前，Manager 上报 `done|failed` 可以由 State Finalization 接受。
...
`reconcile_codespace_states` 定时扫描中间态 codespace，处理 `operation_deadline_unix` 超时和 Manager 离线导致的停滞，让 codespace 从中间态收敛到明确结果。`cleanup_failed_codespaces` 和 `cleanup_codespace_logs` 分别处理数据生命周期，控制过期记录和日志的长期堆积。
