...
**设计说明（有意如此）：第一版不增加全局 `/codespaces/new`。**创建必然已有 repository 上下文，复用 repository 页面可以避免第二套 repository 选择和创建表单；`GET /codespaces` 只负责展示当前用户已有对象。
...
**设计说明（有意如此）：全局 `/codespaces` 是 Codespace 产品的固定 Web 命名空间。**这些静态路由在 Gitea 的通用用户名、组织名和 repository 路由之前注册，因此即使数据库中已经存在名为 `codespaces` 的用户或组织，匹配的 `/codespaces` 与 `/codespaces/{uuid}` 仍优先进入 Codespace 页面。固定命名空间让 repository 删除、转移和权限变化后仍有稳定的对象入口；当前设计接受同名通用路由被遮蔽的兼容性结果，不增加别名、迁移规则或另一套保留前缀。
...
**设计说明（有意如此）：repository 路径只是创建上下文和来源筛选，不是既有 Codespace 的身份路径。**创建成功后，对象的规范地址只包含 `codespace.uuid`。这样 repository 删除、改名、转移或变得不可访问时，既有 Codespace 的详情和生命周期操作仍保持稳定，也与 create 完成后不再依赖 repository 的状态模型一致。
...
**设计说明（有意如此）：协议幂等以同一 `operation_rversion` 为边界，不把普通 Web POST 描述为网络级强幂等。**每次被服务端接受的新 create POST 创建一个新 Codespace；生命周期 POST 按到达时的主状态和 active operation 返回成功、冲突或不存在。当前设计不保存 HTTP 请求键或删除墓碑，因此响应丢失后的重复 create 可能代表新的创建，物理删除后的重复 delete 返回不存在。明确这一边界可以避免实现人员误以为仅靠当前 Codespace 行就能证明两个独立 HTTP 请求是同一意图，也避免为第一版增加幂等历史表。
...
站点管理页列出所有 Codespace 和 Manager，并提供 force delete、Manager 直接删除及 global registration token 管理；用户和组织设置页只管理当前 owner scope 的 registration token 与 Manager。第一版不增加对应 public API，页面直接调用同一 Codespace 服务层，避免 Web 与后续内部调用形成两套删除或凭据规则。

**设计说明（有意如此）：管理页现在和以后都不提供单 Manager enable、disable、pause 或 quarantine。** 计划排空由 Manager 在 Fetch 中上报 `capacity_available=0` 且不接受 create/resume；维护中断使用 recovering/offline；身份需要永久撤销时直接删除 Manager。单一管理开关无法同时保持 active operation、Token、Gateway session 和 Runtime transition 的可逆性，因此不把这些不同职责重新合并为 Manager 管理态。
...
**设计说明（有意如此）：Registration Token 表只保存当前入口，不保存 inactive 或轮换历史。**停用后的旧凭据不参与认证和恢复，系统也不提供凭据审计；物理删除让记录存在性直接表达有效性，并删除不必要的 retention 流程。

Manager 删除确认页展示绑定 Codespace 数量，并说明 Gitea 会同步删除这些 Codespace、token 和日志，但不会联系 Manager 或保证 Runtime 回收。确认后，服务层不读取 Manager runtime state，保持 owner/Manager lock 并按 UUID keyset 逐 Codespace 短事务清理，空集合复检后删除 Manager；已经通过认证的并发 RPC 也必须在最终写入前重新检查记录和 binding。
...
| `state_unavailable` | `FailedPrecondition / false` | 功能关闭、工作状态 token pair 缺失或当前生命周期不允许该动作 |
...
- 注册与同 scope token 的 GetOrCreate、Rotate、Deactivate 使用同一 owner lock；并发时只产生“旧 token 先完成一次注册”或“新状态先成立并拒绝旧 token”两种完整结果。
- RegisterManager 的锁前 token 查询只用于定位 owner，锁内复读才是认证线性化点。
...
心跳间隔小于 offline timeout 三分之一，是为了让一次短暂网络抖动或单次 heartbeat 延迟不会直接触发 offline，同时让 Gitea 能在数个心跳周期内发现真实离线。`recovering` 运行态让 Gitea 区分“Manager 正在维护恢复”和“Manager 完全不可达”，从而保留已有 codespace 主状态并暂停新的 create/resume 领取。恢复窗口从本次维护声明或已经成立的 offline 时刻中较早的事实起算，使 heartbeat、迟到 Declare 和 Cron 扫描顺序都不能延长 `MANAGER_RESTART_GRACE`。

**设计说明（有意如此）：Manager 完成完整 inventory、Runtime 映射和 worker 上下文恢复后即可 Declare online。**Runtime Metadata、credential-refresh 和 pending stopped/failed fact 是逐 Codespace 后置收敛，可以在 online 后继续；Manager 在这些工作完成前不调用接受 create/resume 的 Fetch。online 声明本身不主动分配 operation，这个顺序既不会提前增加运行工作，也避免恢复超过 hard grace 后后置 RPC 因 recovering 状态无法完成。
...
- Manager 恢复完整 inventory、Runtime 映射和 worker 上下文后可先 Declare online，再完成逐 Codespace metadata、credential 和 pending fact；全部完成前不领取新的 create/resume。
...
**设计说明（有意如此）：global Manager 与 owner-scoped Manager 同时匹配时没有优先级。**满足当前 owner scope、tag、online 和 capacity 条件的 Manager 都参与同一条件 UPDATE，首个更新成功者取得 create；binding 成立后固定，不因另一个 Manager 的 owner scope 更具体而迁移。global Manager 是可服务所有 owner 的站点容量，不是 owner-scoped Manager 的延迟兜底。该语义保持 claim 为一次数据库竞争，不增加等待窗口、容量预留或新的失活状态。
...
`max_operations` 范围为 `1..256`，只限制 `operations`；`renewed_leases` 最多与 request 的 observed 数量相同，不占 payload 名额。`observed_operations` 最多 10000 条且 UUID 不得重复。每种优先级使用 `operation_created_unix, uuid` keyset 分页，按升序处理；单次 Fetch 在稳定 scope/tag 筛选后合计最多检查 1024 个 queued 候选，防止每增加一种 operation 类型都线性放大数据库读取。Manager 调用 Fetch 或续租的周期不超过 `OPERATION_LEASE_TIMEOUT / 3`。站点排空时不领取 queued create/resume，queued 和 running stop/delete 继续正常处理。abort create/resume 不续租；create 的 `final failed` 收敛为 failed，resume 的 `final failed` 在确认启动回滚后收敛为 stopped。
...
| `resume` | `status=running, last_active_unix=now, clear active operation`（`stopped_unix` 不清零） | `status=stopped, revoke token, clear active operation` |
...
resume failed 只在 Manager 已确认本轮启动进程回滚后上报，因此保留 workspace 并允许以后再次 resume。若 Manager 随后确认资源本身不可恢复，使用现有 `ReportRuntimeTransition(failed)` 把无 active operation 的 stopped Codespace 收敛为 failed；详细原因仍留在 Manager 日志，不增加 final 字段。
...
- `updated_unix` 只在创建记录、创建或替换 active operation 和首次 final/timeout/missing/transition 收敛时更新；claim、续租、交互、metadata、日志、token 修复、幂等结果和 `repo_id` 置 0 不刷新该字段。
...
校验顺序固定为：读取 codespace 并检查 `manager_id` binding；检查 active operation conflict；检查功能开关和 Manager runtime state；检查 `observed_operation_rversion`；检查 `runtime_generation`；检查主状态与 fact 是否兼容；running fact 最后校验并规范化 metadata。功能启用且 Manager online 或处于有效 recovering 窗口时可以提交三种 fact；站点排空时只允许 stopped/failed；已派生 offline 的 Manager 先 `DeclareManager(recovering)` 再上报。主动 running fact 必须携带 `stage=credential-refresh`、`boot.operation_rversion=observed_operation_rversion` 的新 metadata generation。`ReportRuntimeTransition` 不递增 `operation_rversion`，因为它不是 Gitea 下发的 operation。
...
**设计说明（有意如此）：主动 running 只由 Manager 启动 Runtime 前持久化的当前 boot 上下文表达。**Manager 对响应丢失重试同一 running fact；普通 inventory 看到 Gitea stopped、Runtime running 时只会收到 `stop_local_runtime`，不能据运行事实猜测主动 resume。该边界让 running stop 超时形成的 stopped 结果保持稳定，同时不增加 operation 历史或意图字段。
...
Manager 在 failed fact 前关闭 session，取消 pending metadata/Endpoint mutation 和后置 worker。请求被首次接受或按目标状态幂等成功后，Manager 可立即删除本地 Runtime、workspace 和 volume；清理失败时继续在 inventory 中上报 failed，由 failed 主状态返回 `cleanup_local_runtime`。未知 UUID 和 `resource_absent` 仍不构成本地删除授权。

Manager 处理 transition Connect error 时不重试无法成立的原请求：`current_operation_conflict` 转为 Fetch 当前 operation；`stale_operation` 转为完整 inventory 调和；`stale_generation` 按 detail 的当前值加一并重新读取 backend 事实；`generation_conflict` 在已知的相同 generation 上加一，仅重报仍为当前值的事实；站点排空拒绝 running fact 时停止 Runtime 并改报 stopped；`manager_offline` 先 Declare recovering 后重建当前事实；`codespace_not_found` 停止该 Codespace 的后续上报，`manager_unregistered` 停止全部 Gitea 通信，两者都不清理 Runtime。这些分支只使用现有 error category、Fetch、Declare 和 inventory，不增加响应字段。

实现验收点：

- transition 请求不携带不参与判定的观察时间和原因字段。
- 主动 running 只接受 `credential-refresh` metadata，随后 token/credential/ready 收敛完成前不提供交互。
- Manager 在启动 stopped Runtime 前持久化主动 running boot 上下文；响应丢失时重试显式 fact，inventory 不恢复该意图。
- 站点排空时可提交 stopped/failed fact，running fact 返回 state unavailable。
- 功能启用且 Manager online/recovering 时可提交三种 fact，offline Manager 必须先 Declare recovering。
...
- failed fact 成功后可立即删除本地 Runtime、workspace 和 volume，未完成清理由 failed inventory instruction 继续收敛。
...
- 调用方 Manager 必须与 `codespace.manager_id` 匹配，且已声明为 online 或处于有效 recovering 窗口。recovering 允许恢复已绑定 Codespace 的当前凭据，既包括 active create，也包括 resume final 或主动 running fact 后已无 active operation 的 `credential-refresh`；它不允许领取新 create/resume。
...
- `credential-refresh` worker 收到站点排空的 `state_unavailable` 时取消后置工作、停止 Runtime，递增 runtime generation 并上报 stopped；停止确认失败时改报 failed。`manager_offline` 要求先 Declare recovering 再重试；`codespace_not_found|manager_unregistered` 终止后置通信但不删除 Runtime；更高 stop/delete operation 已存在时由 active operation 接管。
- `status=stopped|failed|deleting` 返回状态不可用。
- `repo_id=0` 时仍可签发 token；后续 Git HTTP(S)、LFS 和 repository API 没有任何 repository 能通过 binding，当前用户与公共服务信息 API 仍按 Token 管理章节工作。
- token pair 通过下述完整性检查时返回保存的 `gitea_token` 明文；ID 非零本身不构成可返回条件。
- `gitea_token_id=0` 时以固定名称 `codespace-{完整 codespace_uuid}` 和严格的规范化 scope `write:issue,write:repository,read:user` 签发新 access token，写入非零 `gitea_token_id` 和 `gitea_token`，再返回明文。固定 UUID 名称不会因 repository 或展示名称变化而改变，也便于现有 PAT 列表识别来源。
- 完整 token pair 同时满足：ID 非零、明文非空、access token row 存在、row `UID == codespace.user_id`、规范化 scope 严格等于 `write:issue,write:repository,read:user`，并且明文使用 row 的 salt 计算后与 `TokenHash` 匹配。`Name` 只用于展示，不参与完整性判定。
...
- 任一完整性条件不满足时，生命周期服务锁定 codespace 行：只要非零 token ID 仍由该 Codespace 引用且对应 row 存在，就先通过生命周期专用入口删除该 row，再原子写回 `0 + 空字符串`；仅在当前为工作状态且创建用户仍存在时重新签发。binding 表达该 row 已作为 Runtime 凭据暴露，因此修复优先吊销它，避免错误凭据在清空 binding 后继续有效。
- 系统签发不调用普通 PAT UI/API 的名称占用检查，允许用户普通 PAT 与系统 token 使用相同 `codespace-{uuid}` 展示名称。安全判断和删除保护始终使用 token ID；用户按重名删除时沿用 Gitea 现有 multiple-match 422，按 ID 删除可精确定位。
...
- 功能启用时，Manager online 或处于有效 recovering 窗口、Codespace 为 creating/running 且创建用户存在的有效请求返回当前 token 或在空 binding 上签发一个新 token；running stop 只可读取现有 token，不会签发或轮换。
...
- credential-refresh 的站点排空、offline、记录缺失和更高 operation 分别收敛到 stopped/failed、Declare recovering、停止通信或 active operation 接管。
...
- 新 token 的名称固定为 `codespace-{完整 codespace_uuid}`，规范化 scope 固定为 `write:issue,write:repository,read:user`。
- 成功响应始终包含非空 token 和规范化 `server_url`。
- pair 完整性验证覆盖 row UID、严格 scope 和明文 hash；名称不同不触发修复，同名普通 PAT 也不阻塞签发。
...
Manager 通过 `ReportInstances` 上报本地 Runtime inventory 快照。handler 在认证后取得调用方 Manager lock 并持有到整个请求完成，在锁内重新读取 Manager 和 generation；需要写入单个 Codespace 时按规范 UUID 顺序逐项取得 Codespace lock，并在短事务中复检 binding。Manager 删除和 owner 对已绑定 Codespace 的清理取得同一 Manager lock，因此请求要么先完成并随后被删除流程清理，要么在删除提交后读取不到 Manager 或 binding，不会用旧认证快照恢复记录。
...
| 功能启用或站点排空时，Gitea 为 running、Runtime 为 stopped 且无 active operation | 返回 `report_runtime_transition(current_operation_rversion)`，由 Manager 使用该版本携带新 generation 上报 stopped fact |
...
**设计说明（有意如此）：`cleanup_local_runtime` 是删除该 UUID 对应 Runtime、workspace 和 volume 的破坏性授权。**Gitea 只在仍有记录且 `status=failed`，或记录明确绑定其他 Manager 时返回它；stopped、按 resume/stop timeout 收敛后的资源、未知 UUID 和未绑定 creating 都保留。failed 是不可恢复终态，因此可以清理；其他场景缺少足够事实证明持久 workspace 应被删除。
...
- `ReportInstances` 全请求持有调用方 Manager lock，逐项写入按 UUID 取得 Codespace lock；Manager 删除或 owner 清理已绑定 Codespace 后，旧请求不能恢复 generation、Codespace 或 cache。
...
- running 主状态对应 stopped Runtime，以及 failed inventory 的 `report_runtime_transition` 始终返回当前 operation 版本，本地版本基线丢失后仍可安全上报缩减能力的 fact。
...
- 站点排空时不会收到要求上报 running fact 的 instruction。
...
**设计说明（有意如此）：物理删除把“日志文件不存在”视为成功，而不是数据损坏。**创建后立即失败、从未被领取或内部摘要写入失败都可能合法地没有日志文件；让该情况阻断 Codespace、owner 或 Manager 删除会把诊断文件错误提升为资源生命周期前置条件。
...
- Gitea Actions 把运行中 task token 识别为携带 `task_id` 的内部 actor，再由 repository 权限入口按 task、目标 repository 和 unit 计算权限；Git receive 与 LFS 显式复用该入口。Codespace 复用其中“认证凭据绑定运行对象、目标 repository 再次判定”的结构。
- Actions 内部 actor 用于自动化任务，不代表发起 Codespace 的真实用户；直接复用会让 commit、Issue、Pull Request、Review 和评论失去正确的用户归属。Codespace 因此继续使用创建用户 access token，让 Gitea 现有权限和业务记录保持原语义。
- Gitea access token scope 只表达 category，没有单 repository scope。Codespace 使用固定 category scope 进入开发相关 API，再以 `codespace.repo_id` adapter 限定唯一 repository；两层判定使 token 能完成正常开发，又不会退化成创建用户的通用 PAT。
...
- 所有 codespace token 的规范化 scope 严格等于 `write:issue,write:repository,read:user`；`write:repository` 承载 Code、Pull Request、Release、Wiki 和受限 Actions API，`write:issue` 承载 Issue 及与 Pull Request 共用的 Issue API，`read:user` 只供当前身份查询。
- scope 不是最终授权。实际操作继续由 token binding、创建用户当前权限、repository/unit 状态、分支保护及各 handler 原有业务检查共同决定。
...
- access token `Name` 固定为 `codespace-{uuid}`，用于用户识别；系统签发允许与普通 PAT 重名，安全判断和删除保护只使用 token ID。
...
- 普通 Gitea PAT 通过现有 Basic password、Bearer header 或启用时的 query token 认证成功后，认证分支除写入现有 `IsApiToken/ApiTokenScope` 外，还把实际 token ID 作为 `AccessTokenID int64` 写入 request context。query token 严格服从 Gitea 现有 `DISABLE_QUERY_AUTH_TOKEN`；配置关闭时沿用现有未认证结果。OAuth2 access token、Actions token/JWT、登录 session、reverse proxy 身份和 SSH 身份不写该字段。
...
- API 统一守卫位于现有 PAT 认证之后、`sudo` 与其他 route middleware 之前。它识别 binding、校验 token pair、功能开关、Codespace 工作状态和 route policy，并把已验证的 binding 放入 request context；`repository` policy 随后由现有 repository assignment 在解析出可见目标后比较 repo ID。Codespace token 携带非空 `sudo` query 或 `Sudo` header 时返回 `unsupported_resource`；即使创建用户是站点管理员，也保持 `ctx.Doer == codespace.user_id`，不切换执行身份。
- Web Router 在现有认证中间件之后安装同一识别守卫。绑定 Token 只有带明确 marker 的 Git Smart HTTP 入口可以继续到 repository adapter；其他普通 Web、下载或设置入口在 handler 前返回 403。LFS 在自身现有认证与 repository 解析入口调用同一服务判定。这样普通 Web 不依赖每个 handler 自行记得拒绝 Codespace Token。
- 每个携带 PAT 的 API、Git HTTP、LFS 或已允许 Basic/Bearer 认证的 Web 请求，在副作用前按 `AccessTokenID` 用一条数据库查询重新读取 access token row 和至多一个 `codespace.gitea_token_id` binding。row 与一个 binding 同时存在时进入 Codespace 校验，只有 row 时按普通 PAT 继续，row 已不存在时返回现有 401；异常命中多个 binding 时按数据库一致性错误失败关闭并记录服务端日志，不能任选一条授权。该查询是本次请求的授权生效点：stop/failed/delete 或站点排空在查询前成立则拒绝请求，在查询后成立则当前请求按现有 Gitea 行为完成，后续请求拒绝。
- 授权点必须重新读取 token row，不能只做“binding 不存在就是普通 PAT”的反查。生命周期事务会原子删除 token row 并清空 pair；因此并发 stop 后查询只能看到完整旧状态或 row 已不存在，不会把刚被吊销的 Codespace token 降级为普通 PAT。该热路径使用 token ID 与现有普通索引查询，不增加 cache、请求锁或进行中请求表，保证吊销结果无需等待缓存过期。
- repository API 继续复用现有 `repoAssignment` 的 owner/repository 解析、重定向、可见性和创建用户权限逻辑。assignment 取得目标后读取 request context 中已经验证的 binding，并比较 `target_repo_id == codespace.repo_id`；repository 不存在或对创建用户不可见时保持现有 404，已经解析且可见但 ID 不等于 binding 时才返回 403 repo binding mismatch。该顺序不复制 repository assignment，也不会在身份可见性判定前泄漏 binding 结果。
- Git HTTP 和 LFS 在各自现有认证、repository 解析及权限入口中调用同一 Codespace token 判定。adapter 只补充工作状态、token pair、精确 route 和单 repository binding，不替代 scope、unit、reader/writer/admin、分支保护或 handler 的现有检查。

**设计说明（有意如此）：授权生效点是副作用前对 token row 和 binding 的单次重读，不是最初解析 Authorization header 的时刻。**这使 stop、Token 吊销或站点排空与新请求只有“请求先获得授权并完成”或“状态变化先成立并拒绝请求”两种完整结果；系统不尝试取消已经进入 Git subprocess、LFS 传输或 API handler 的请求。
...
**设计说明（有意如此）：Codespace token 是绑定 repository 的开发凭据，不是通用 PAT。**它支持 commit、Issue、Pull Request、Release、Wiki 和上述 Actions 操作；管理入口即使位于同一 `/repos/{owner}/{repo}` 下、且创建用户本人有 admin 权限，也保持拒绝。这样权限边界由 Runtime 工作需要决定，而不是由 URL 前缀或创建用户的最高权限决定。

表中的能力是面向开发者的说明，路由注册处的具体 method/route marker 才是可执行允许表。混合读写 group 逐个 method 标记，不能给整个 `/repos/{owner}/{repo}` 或子 group 自动放行；全局 markup/markdown、`/repositories/{id}` 和以后新增但未标记的 route 保持拒绝。这样文档不需要复制一份容易随 Gitea 路由变化而漂移的完整 URL 清单，同时代码审查可以直接看到每个新增 route 是否进入 Codespace 开发面。
...
- repo-bound token 判定校验当前 token pair、工作状态和 `target_repo_id == codespace.repo_id`，随后继续执行创建用户现有 Git 或 API 权限检查。
- codespace-bound token 的 repository 访问范围固定为 `codespace.repo_id`；访问绑定 repository 时继续交给 Gitea 现有权限链路判断，已解析且对创建用户可见的其他 repository 返回 repo binding mismatch，原本不存在或不可见的目标保持 Gitea 现有隐藏结果。`repo_id=0` 时没有任何 repository 能通过 binding。
...
- 创建用户记录必须仍存在；账户删除流程物理删除关联 codespace 及 token，不保留等待补签的工作状态记录。
- 完整 pair 非零时返回保存的 `gitea_token` 明文，不做轮换；空 pair 才签发新 token，损坏 pair 先按修复规则吊销并清空，再决定是否签发。
...
**设计说明（有意如此）：携带 Codespace token 访问其他 repository，即使目标公开，也返回 repo binding mismatch，不回退为匿名访问。**认证凭据一旦出现，请求就必须服从其单仓库边界；Manager 通过 path-scoped credential 避免把该凭据发送给无关 repository。其他私有 repository 和需要跨仓库认证的 submodule 不由该 token 授权。

Codespace adapter 只约束请求直接寻址的 repository，不截断 Gitea 已经建立的 Issue、Pull Request、commit、fork 和 project 关系。直接 Issue/PR API 路由的目标 repository 必须等于 `codespace.repo_id`；进入 handler 后，以 `codespace.user_id` 作为当前用户，继续执行 Gitea 现有 scope、repository/unit 权限、关系校验、站点配置和业务检查：
...
这些关联写入不传播新的 Codespace binding，也不绕过目标对象原有权限。Git hook、queue 或 service 继续记录并使用 `codespace.user_id`，不把 Codespace token 转成系统身份；由 Gitea 关系派生的后续处理不再要求每个内部目标都等于 `codespace.repo_id`。实现继续复用 `pull_service.CheckUserAllowedToUpdate`、`repo_service.DeleteBranchAfterMerge`、Issue dependency 权限检查、`IssueAssignOrRemoveProject` 和 auto-merge handler 的现有判断，不在这些内部服务外再增加 repository equality 检查。第一版也不自动创建 fork，不增加 parent repository 特例、多 repository binding 或临时扩权。

**设计说明（有意如此，容易产生歧义）：直接 API 路由目标仍只能是绑定 repository，但绑定 repository 中的代码和协作对象可以通过 Gitea 已有关系触发其他 repository 或 project 上的关联写入。**例如更新 fork PR 的 head、删除 merge 后的 fork branch、创建跨 repository Issue dependency 或由 commit 关闭关联 Issue，结果可能落在 binding 之外；这是创建用户本来拥有的 Gitea 协作权限，不是 Codespace 获得了直接访问其他 repository 的能力。直接调用无关 repository API、全局搜索 Issue 或超过创建用户当前权限仍然拒绝。
...
**设计说明（有意如此，容易产生歧义）：Codespace token 的工作状态生命周期控制的是新请求，不是已经持久化业务对象的生命周期。**否则 stop 必须跨 Issue、PR、Actions 和队列实现补偿撤销，既偏离 Gitea 现有语义，也会使同一用户通过不同凭据创建的对象产生不同生命周期。
...
LFS 的 batch、object transfer、verify 和 lock route 在现有 LFS 认证与 repository 权限检查后、返回 repository 前执行 binding 校验。Gitea 自身承载的每个后续对象请求都会重新经过授权点；对象存储 `ServeDirectURL` 则是由现有存储 adapter 签发的独立短期能力，签发后绕过 Gitea，按 adapter 原有过期时间结束。
...
**设计说明（有意如此）：stop 提交后，尚未通过授权点的新请求不能再签发 LFS 直传 URL 或 Actions artifact 下载 URL；已经通过授权点的请求可以完成，已经签发的短期 URL 也不会被撤销。**这些 URL 与已经传出的响应字节一样是独立、限时的 bearer capability；当前 MinIO/Azure 直传沿用 5 分钟签名，其他存储 adapter 沿用自身期限，artifact raw 沿用 60 分钟签名，不增加签名记录、撤销表或生命周期回调。

公共 Codespace token 判定规则：

- 当前请求没有 PAT `AccessTokenID` 时，不进入 Codespace token 判定。
- 授权点以一条查询重新读取 token row，以及零个或一个 binding 和 Codespace；只有 row 时按 Gitea 普通 PAT 逻辑处理，row 已不存在时返回现有 401，row 与一个 binding 同时存在时继续以下判定，异常存在多个 binding 时按一致性错误拒绝。
- access token 被 codespace 绑定时，`token_id` 与 `codespace.gitea_token_id` 匹配。
- 当前 Codespace token pair 必须满足 RequestGiteaToken 定义的 row、UID、严格 scope 和明文 hash 完整性；损坏 pair 返回 `state_unavailable`，不因 token row 本身通过普通 PAT 认证而继续 Git、LFS 或 API 请求，reconciliation 或后续 RequestGiteaToken 负责吊销并修复。
- access token 被 codespace 绑定时，`codespace.status` 必须为 `creating|running`；其他状态返回 `codespace_not_running`。
- `[codespace] ENABLED=false` 时，现有 binding 判定仍然加载；Git/LFS/API 返回 `state_unavailable`，不能回退到普通 PAT 逻辑。`RequestGiteaToken` 只允许 active running stop 读取已经存在的完整 token pair 用于日志脱敏，其他请求均不签发或返回 token。
- `target_repo_id` 与 `codespace.repo_id` 匹配。
- `codespace.repo_id=0` 时不存在可匹配的 target repository；已解析且可见的目标返回 repo binding mismatch，不存在或不可见目标保持原错误。当前用户和公共服务信息 API 不带 target repository，仍按各自 route policy 判定。
- 目标必须是主 repository，Git wiki 和不存在 repository 的 push-to-create 不进入开发允许面。
- repo binding 匹配后，以 `codespace.user_id` 对应的当前用户继续执行 Gitea 现有 scope、unit、可见性、分支保护和所需 read/write/admin 业务检查；完整性校验已经保证 access token row UID 与该用户相同，因此请求中的 Doer 仍是创建用户，不改成系统用户。
...
Codespace adapter 补足单 repository 和开发入口边界：category scope 不表达“只能访问哪个 repository”，也不能区分开发 API 与 repository 管理 API。用户权限、repository/unit 状态和 handler 业务检查继续由 Gitea 现有实现负责，避免维护第二套权限系统。

token 生命周期以每个新 Git/LFS/API 请求的授权点为边界。stop、failed、deleting 或功能关闭成立后，后续请求重读 token row 与 binding 并拒绝；已经通过授权点并进入 handler 或 Git 处理链的请求按现有 Gitea 行为结束，不增加进行中请求表，也不从生命周期事务取消 git subprocess。Gitea 承载的 LFS 后续对象传输和 API 调用是新请求；已经签发的对象存储或 artifact 短期 URL 按前述明确例外到期。

有效 codespace-bound token 的 `repo_binding_mismatch`、`unsupported_resource` 和功能关闭统一返回 HTTP 403；授权点发现 token row 已不存在时返回现有 401。repository assignment 期间的不存在、不可见和现有权限隐藏保持 404，只有已解析且对创建用户可见的其他 repository 才返回 403。binding 通过后的 unit 和业务拒绝保持 Gitea 现有 403/404/409/422 等行为。API 继续使用 Gitea 现有 `{message,url}`，Git 使用 plain text，LFS 使用协议错误对象；分类是服务端 typed error、日志和测试语义，不增加要求客户端解析的新 JSON envelope。query token 在 `DISABLE_QUERY_AUTH_TOKEN=true` 时没有通过现有认证，因此不会进入 binding 判定。

Codespace access token 继续显示在现有 PAT 列表和 API 中，只展示固定名称、创建时间、使用时间等现有非明文字段；现有接口不会返回 token 明文。用户通过 session 或其他正常凭据删除该 token 时返回 409，因此可以识别系统凭据，但必须通过 stop 或 delete 结束其生命周期。Codespace token 自身不能进入 PAT 列表、创建或删除 API，包括当前 token DELETE；这些 route 没有开发 route policy，稳定返回 403。

删除保护：

- 不扩展 `access_token` 表。`services/auth/access_token.go` 提供外部入口 `DeleteAccessToken(ctx, tokenID, userID)` 和生命周期入口 `DeleteAccessTokenForCodespaceLifecycle(ctx, tokenID, codespaceUUID)`，让普通 PAT 删除与 Codespace 状态事务分别拥有明确的事务边界。
...
- 用户设置页、用户 token API 和“删除当前认证 token”API 的实际删除都调用 `DeleteAccessToken`，router 不再直接调用 access-token model。普通凭据到达这些入口时不会绕过 binding 保护；Codespace token 自身因这些 route 没有开发 policy 而被 API 统一守卫拒绝。
- Codespace 生命周期服务在 stop final、failed final、进入 deleting、物理删除和 failed 记录清理时调用 `DeleteAccessTokenForCodespaceLifecycle`。该入口使用调用者已有的事务 context，不开启嵌套事务；它校验 Codespace UUID 与 `gitea_token_id`，并在同一事务删除 token row、把 `gitea_token_id` 写为 0、把 `gitea_token` 写为空字符串。
...
- 用户删除中的 access token 批量删除是内部例外：owner 分阶段清理必须先逐 Codespace 通过生命周期入口删除所有关联 token，随后现有用户删除事务才可批量删除剩余普通 PAT。批量入口不能用于仍有 Codespace binding 的普通用户删除路径。
- reconciliation 按 RequestGiteaToken 定义的 ID、明文、row、UID、scope 和 hash 条件清理不完整 pair；物理删除必须在同一事务删除 token row，因此不依赖无 codespace 记录的反查。
...
- codespace-bound token 只有在 `creating/running` 且 token ID、route、主 repo binding 和创建用户当前权限均通过时才能使用 Git/LFS 或开发 API。
- Basic、Bearer 和 Gitea 当前启用的 query PAT 认证写入实际 `AccessTokenID`；query 关闭时沿用现有未认证结果，OAuth2、Actions、session、reverse proxy 和 SSH 身份不会被误标为 PAT。
...
- 授权点单次重读 token row 与 binding；并发 stop 在授权点前提交时返回 401，站点排空在授权点前成立时返回 403，在授权点后成立时当前请求完成且后续请求拒绝。
- 同一非零 token ID 异常反查到多个 Codespace binding 时授权失败并记录一致性错误，不会选择任一 binding 或降级为普通 PAT。
...
- `repo_id=0` 时仍可签发 token；没有任何 repository Git/LFS/API 能通过 binding，已解析且可见的目标返回 repo binding mismatch，不存在或不可见目标保持原错误；`GET /api/v1/user`、version 和全局 signing key GET 仍可成功。
- 功能关闭时 binding 与删除保护继续生效，现有 token 不会退化为普通 PAT；吊销只阻止之后的新请求，不承诺中断已经进入 Git、LFS 或 API handler 的请求。
- `creating -> running` 复用同一 token；stop/failed/deleting 清空字段，后续 running 请求生成新 token。
- 用户无法从通用 PAT 页面删除仍由 codespace 持有的 token。
- 用户设置页和用户 token API 经过 `DeleteAccessToken`；使用正常凭据按 ID 删除绑定 token 稳定返回 409，Codespace token 自身访问 token 管理 API 稳定返回 403。
- 生命周期吊销通过调用者事务中的 `DeleteAccessTokenForCodespaceLifecycle` 同时删除 token row 和清空 token pair，不开启嵌套事务。
- PAT 列表可见 `codespace-{完整 codespace_uuid}` 的非明文元数据；同名普通 PAT 不阻塞签发，安全判定与删除保护按 ID 工作。
...
- 对允许的 API 路由使用同一用户、同一对象分别执行普通 PAT 与 Codespace token 的差异测试；binding 和工作状态通过后，两者得到相同的 Gitea 业务权限结果，Codespace adapter 不引入第二套 Issue/PR 角色判断。
...
- artifact delete 返回 403；工作状态可取得 artifact 下载 URL，启用严格登录检查时无 PAT 的有效 raw 签名仍可下载；stop 提交后尚未通过授权点的请求不能签发新 URL，已经签发的 artifact 或对象存储 URL 不带 PAT 使用到原过期时间。
- 新增 API route 未显式安装 Codespace route policy 时保持 unsupported 并返回 403。
- owner 分阶段清理完成后，当时仍由创建者、repository owner 或 Manager owner 关联的 access token、codespace token binding 和 codespace 记录均不存在；中途失败保留 owner，重试只处理剩余项。
...
| `cleanup_failed_codespaces` | `@daily` | 清理超过保留期的 `failed` 状态 codespace 记录、token 绑定和日志 |

`reconcile_codespace_states` 定时扫描 active operation、Manager 可用性和 token binding。Runtime inventory 不持久化，差异只在 `ReportInstances` 请求内处理。queued operation 使用 `operation_created_unix + QUEUE_TIMEOUT` 判定等待超时，running operation 使用 `operation_deadline_unix` 判定 lease 超时。failed retention 从进入 failed 时写入的 `updated_unix` 起算，token reconciliation 不刷新该值。codespace 只有一份连续日志且不保存 operation 历史，因此日志随 delete 或 `cleanup_failed_codespaces` 一起删除，不按“已完成 operation”单独清理。
...
- token reconciliation 不延后 failed 清理时间；registration token 停用直接物理删除，不参与 Cron。
...
- 排空模式继续运行 `reconcile_codespace_states` 和 `cleanup_failed_codespaces`。repo-bound token 识别、repo binding 判定、通用 PAT 删除保护及站点管理员本地删除不受 `ENABLED` 开关影响。重新启用后，未进入 stopped/failed/deleting 的现有对象继续按当前状态工作。
