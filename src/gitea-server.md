...
Manager 删除确认页展示绑定 Codespace 数量，并说明 Gitea 会同步删除这些 Codespace、token 和日志，但不会联系 Manager 或保证 Runtime 回收。确认后，服务层不读取 Manager runtime state，直接提交本地删除事务；删除与 Manager/Codespace 写入共用 keyed lock，已经通过认证的并发 RPC 也必须在最终写入前重新检查记录和 binding。
...
`FetchOperations` 是 Manager 批量获取 Gitea 下发动作的入口。认证完成后，handler 取得调用方 `codespace_manager_{manager_id}` lock，并在整个请求期间持有；随后在锁内重新读取 Manager 的 runtime state、heartbeat、tags 和 owner scope，再处理 running operation、queued claim 与 payload 构造。这样 Manager 删除和 owner 删除可以用同一 Manager lock 得到稳定的 binding 集合，同时仍保留每条 operation 的短事务边界。
...
Manager 通过 `ReportInstances` 上报本地 Runtime inventory 快照。handler 在认证后取得调用方 Manager lock 并持有到整个请求完成，在锁内重新读取 Manager 和 generation；需要写入单个 Codespace 时按规范 UUID 顺序逐项取得 Codespace lock，并在短事务中复检 binding。Manager/owner 删除取得同一 Manager lock，因此请求要么先完成并随后被删除事务清理，要么在删除提交后读取不到 Manager，不会用旧认证快照恢复记录。
...
| 功能启用时，Manager 上报的 running/stopped 与无 active operation 的 `running/stopped` 主状态不一致 | 返回 `report_runtime_transition(current_operation_rversion)`，由 Manager 使用该版本携带新 generation 上报事实；reported creating 只作为存在证据 |
...
| 站点排空：Gitea running、Runtime stopped | 返回 `report_runtime_transition(current_operation_rversion)`，只允许上报 stopped |
| 站点排空：Gitea stopped、Runtime running | 返回 `stop_local_runtime(current_operation_rversion)`；Manager 仅在本地版本不高于该值时停止 Runtime，Gitea 主状态保持 stopped |
...
- `ReportInstances` 全请求持有调用方 Manager lock，逐项写入按 UUID 取得 Codespace lock；Manager/owner 删除后旧请求不能恢复 generation、Codespace 或 cache。
...
- 每个携带 PAT 的 API、Git HTTP、LFS 或已允许 Basic/Bearer 认证的 Web 请求，在副作用前按 `AccessTokenID` 用一条数据库查询重新读取 access token row 和可选的 `codespace.gitea_token_id` binding。row 与 binding 同时存在时进入 Codespace 校验，只有 row 时按普通 PAT 继续，row 已不存在时返回现有 401。该查询是本次请求的授权生效点：stop/failed/delete 或站点排空在查询前成立则拒绝请求，在查询后成立则当前请求按现有 Gitea 行为完成，后续请求拒绝。
...
| Pull Request | list/get/create/edit、diff、commit/file、comment、review、reviewer request、merge 和 cancel scheduled auto-merge；update 仅在 head repository 就是绑定 repository 时允许 |
| Issue | Issue、comment、reaction、attachment、assignee、label、milestone、time、deadline、subscription、pin 和 lock 的现有完整生命周期；dependency/blocking 只允许绑定 repository 内创建和删除 |
...
所有由 Codespace token 触发的写目标都必须是绑定 repository。少数 Gitea 已经建模的 repository 关系允许第二 repository 只读：

- Pull Request create、compare、读取、Review 和 merge 可以读取同一 fork network 中的 head repository，并继续使用 Gitea 现有 code-read 权限检查。显式指定 `headOwner/headRepo` 时也必须验证 fork network，不能仅凭创建用户可读就访问无关 repository。
- Pull Request update 会把 base branch 合入 head branch，因此只有 `pr.HeadRepoID == codespace.repo_id` 时允许；fork head 返回 repo binding mismatch。
- Pull Request merge 的写目标是绑定 base repository，因此 fork PR 可以正常合并。若请求显式要求删除 fork head branch，返回 repo binding mismatch；请求未指定但 repository 默认要求删除时，本次强制为 false，使默认配置不会阻止 merge 或 scheduled auto-merge。
- `merge-upstream` 可以读取绑定 fork 已记录的 base repository，写目标仍是绑定 fork。
- Issue dependency/blocking 的创建和删除只接受绑定 repository 内的 Issue。读取已经存在的跨仓库关系时继续使用 Gitea 当前的 permission filtering，不获得独立访问权限。
- Actions dispatch 只有在 Gitea 现有 `IsScopedWorkflowSourceEffective` 判定 source 对绑定 repository owner 生效时，才可只读 scoped workflow source；生成的 Action run 始终属于绑定 repository。

**设计说明（有意如此）：第一版允许既有 fork、Issue 关系和 scoped workflow 带来的受控只读，但不实现第二 repository 写授权。**这让 PR 和 Actions 的现有协作模型可以工作，同时保持“所有副作用落在唯一 binding”这一条可验证边界；自动 fork、多 repository binding 和临时扩权均不在第一版范围内。
...
- 授权点以一条查询重新读取 token row，以及可选的 binding 和 Codespace；只有 row 时按 Gitea 普通 PAT 逻辑处理，row 已不存在时返回现有 401，row 与 binding 同时存在时继续以下判定。
...
- PR/compare 可以只读同一 fork network 的 head；fork PR update、显式删除 fork head branch、跨 repository Issue dependency 写入和无关 repository compare 返回 403；同仓库 update、fork PR merge、merge-upstream 和有效 scoped workflow 正常工作。
