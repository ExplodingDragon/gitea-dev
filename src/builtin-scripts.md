...
- Manager 核心数据、RPC、配置、结果结构和 Endpoint API 只包含通用脚本契约字段；devcontainer、Docker、Node.js、CLI、容器标识和容器用户作为案例私有状态由自定义脚本保存。
...
- Endpoint API 对所有脚本实现都只接收 Incus 通信地址上的实际端口；devcontainer 案例先在脚本内完成转发，再提交同一实际端口字段。
