...
repository owner 通过 `repository.owner_id` 表示，不在 codespace 表中重复存 `owner_id`。repository 删除后 `repo_id` 置空，空 `repo_id` 即来源 repository 已不可解析的机器状态。codespace 创建完成后按 `codespace.uuid`、`user_id` 和 `manager_id` 管理生命周期与交互入口，不依赖 repository row；保留悬空 repository ID 不能恢复来源仓库，反而会让展示逻辑多一层不可解析状态。repo-bound token 在 `repo_id` 为空时拒绝所有 repository 访问。
