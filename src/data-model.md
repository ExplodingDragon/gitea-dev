...
| `git_protocol` | `VARCHAR(8) NOT NULL DEFAULT 'http'` | 只允许 `http` / `ssh`；create 时按站点默认值固化，决定系统管理的 Git remote 和凭据类型 |
...
| `operation_deadline_unix` | `BIGINT NOT NULL DEFAULT 0` | running lease 的 Gitea 侧 Unix 秒截止边界；其他状态为 0 |
...
`git_protocol` 保存 Codespace 创建时选定的 Git 传输方式，而不是每次启动读取站点当前默认值。站点调整默认协议只影响之后创建的 Codespace；已有 workspace 的 remote、凭据和 resume 完成条件保持一致。**设计如此：**协议固化用于避免同一个 Codespace 在配置变更后出现 remote 仍为 HTTP、恢复流程却生成 SSH Key，或相反的半切换状态。
...
**设计理由：Manager 的身份、可用性和调度意愿分别由现有字段表达。** Manager row 存在表示注册身份有效，`runtime_state + heartbeat` 表示当前可用性，`capacity_available + accepted_operation_types` 表示本次是否接收 create/resume，删除 row 表示永久撤销身份。计划排空由 Manager 上报零容量，维护中断由 recovering/offline 表达。各信号职责单一，因此 operation、Token、Gateway session 和 Runtime transition 可按明确来源判定，无需额外管理状态字段。
...
Codespace Git SSH Key 是运行环境凭据，不是用户主动维护的账户 Key，也不是 Deploy Key。普通用户 SSH Key 页面与 API、Deploy Key 页面与 API、SSH/GPG 签名 Key 查询都只选择各自已有类型；Codespace Key 只由 Codespace 服务创建、确认和删除。这样公钥仍能复用 Gitea 现有 SSH key ID、内置 SSH 和 `authorized_keys` 入口，同时不会扩大到创建者的其他仓库，也不会进入 Deploy Key 的仓库所有者身份与保护分支规则。

公钥确保请求在关系不存在时创建绑定，关系已指向相同规范化公钥时返回当前结果，已有不同公钥时返回 `key_conflict`。公钥指纹与任何普通用户 Key、Deploy Key 或其他 Codespace Key 冲突时返回相同错误。公钥属于 Codespace 整体生命周期，不记录 operation 版本，也不在 resume 时轮换；因此来自旧初始化过程的相同公钥请求天然幂等，不同公钥请求始终不能覆盖现有绑定。
...
- running operation 的 `operation_deadline_unix` 是向未来取整的 Unix 秒边界，同一次授权的相对有效时长保持配置的精确毫秒值。
- `auto_stop_mode` 明确区分站点默认、自定义和永不自动暂停；自定义值通过站点范围校验，`never` 不通过超时 0 隐式表达。
- 站点默认自动暂停时间变化后，`default` 对象无需批量更新数据库即可在下一次设置下发中得到新的有效超时。
- `last_active_unix` 不参与自动暂停；Manager/Gateway 仅按实时连接和本地单调空闲计时发起请求，Gitea 用当前启用状态、超时和 `interaction_generation` 重新授权。
- Codespace 的更新只写该动作负责的字段；除记录创建和 repository 删除置 0 外，其他更新均不写 `repo_id`。
...
- 迁移为已有 Codespace 写入 `git_protocol=http`；新记录在 create 时固化站点当前默认协议，后续配置变化不改写已有记录。
- 每个 Codespace 最多存在一行 `codespace_gitea_token`；数据库只保存 Gitea Secret 密文，认证只读取 salt/hash，不读取或解密密文。
- SSH 协议 Codespace 最多存在一行 `codespace_ssh_key`，其 `key_id` 唯一关联一个 `KeyTypeCodespace` 公钥；关系表不重复保存用户、仓库、状态或权限。
...
- Codespace Key 不出现在普通用户 Key、Deploy Key 或签名 Key 查询中；SSH 强制命令只能由 key ID 解析到绑定 Codespace。
...
- `gcs_` Token 使用现有 Gitea 带盐 hash helper 和常量时间比较；密文解密结果必须重新通过同一 verifier 才能返回。
...
- inventory 的 observed operation 为 0 时表示没有完整 active operation 上下文，正数表示持有该版本的完整上下文；该字段不传输 Manager 本地保存的历史最高版本。
...
- `gateway` 地址规范化后只保留 scheme、DNS base domain 和可选 port，不保存业务 path。
- 不同 Manager 不能写入相同类型的规范化地址，冲突声明不覆盖原地址或 metadata。
- 两类规范化地址均不超过 512 bytes，服务层在写入前拒绝超限值，数据库不发生静默截断。
...
- Manager 地址唯一性由数据库约束保证；并发冲突不会产生两个持有同一 Gateway 或 SSH 地址的 Manager。
- 每个 Codespace 最多存在一个 Gitea Token，Token hash 不重复，末八位只用于缩小候选范围。
...
锁 key 由格式化 helper 构造：所有者关系写锁使用 `owner_write_{owner_id}`，Manager 使用 `codespace_manager_{manager_id}`，Codespace 使用完整规范化小写 UUID 构造 `codespace_{uuid}`。下文把所有者关系写锁简称为 owner 写锁。repository 使用的 `repo_working_{repo_id}` helper 位于 `modules/repository/lock.go` 并导出为 `WorkingLockKey(repoID)`；repository 创建完成后的文件初始化、transfer、delete 和 `CreateCodespace` 记录插入事务使用同一 repository key。Manager 地址冲突由数据库唯一约束判定，因此该对象没有锁 key。

owner 写锁保护会影响用户或组织删除判定的关系写入，而不是只保护 Codespace 表。采用这一范围的原因是：删除服务必须在最终复检与删除 owner 期间阻止新的 repository、package、成员关系或 Codespace 指向该 owner，否则单独保护 Codespace 写入仍可能留下 Gitea 原有关系。`owner_id=0` 使用 `owner_write_0`，只用于串行化 global registration token 与 Manager 注册；它不对应可删除账户。
...
| repository 创建，包括普通创建、fork、template、migration、adopt 和 push-to-create | repository owner；数据库记录提交后在释放 owner 前接续取得 repository |
| package 与 version 关系创建或追加，包括 container 和 Terraform 的专用入口 | package owner；随后取得 Gitea 既有 package 业务锁 |
| 组织成员、team 成员关系写入 | 涉及的用户与组织 owner，ID 去重后升序 |
| 组织创建时写入创建者的初始成员关系 | 创建用户 owner |
| Declare 完整快照与 Gateway/SSH 地址写入、Manager secret 轮换 | Manager |
...
| 用户或组织删除 | 在 Codespace 前置清理和最终删除阶段持有目标 owner；删除该 owner 自有 Manager 时再取得一个 Manager，删除其余关联 Codespace 时每次只取得一个 Codespace；用户 purge 进入组织、成员和 package 清理前释放用户 owner |
...
同一短事务确实需要多个锁时，代码按 owner、repository、Manager、Codespace 直接多次调用 `globallock.Lock`；同层多个 ID 去重并升序取得，完整 UUID 按字符串升序，后一个取得失败或操作完成时逆序释放。锁在受保护的数据库事务前取得，事务内重新读取关系双方，已持锁内部函数复用调用方持有的锁。repository 创建是唯一的接续场景：新 ID 只能在插入后取得，因此先在 owner 写锁内提交可见的 repository 关系，再在释放 owner 写锁前取得新 repository lock，把保护责任交给文件初始化阶段。Manager 删除持续持有 owner 和 Manager 父级锁，用户或组织删除只在需要稳定关系集合及最终删除 owner 的阶段持有目标 owner 写锁；子对象按 keyset 顺序逐个取得、提交和释放。锁内不调用 Manager、Gateway 或其他网络服务。这样既避免一次持有全部 Codespace lock 或开启覆盖全部子对象的长事务，也使已经只持有 Codespace lock 的清理路径无需反向取得 Manager 或 owner 写锁。
...
owner 写锁覆盖删除判定所依赖的全部关系写入口：repository owner 关系的创建、转移与删除，package owner 与 version 关系写入，组织成员与 team 成员关系，Codespace 创建，Manager 注册与删除，以及 registration token 变更。每个入口在锁内事务中重新读取涉及的用户或组织；记录不存在或类型不符合入口要求时返回明确业务错误，不继续使用锁前读取的旧 `*User`。container、Terraform 等直接进入 package model 的专用路径也通过同一 package service 边界提交关系，避免只有通用上传路径受保护。

组织删除服务持有组织 owner 写锁后，通过内部关系清理函数只删除该组织的 `OrgUser` 与 `TeamUser`，并复用调用方事务；公开成员入口才同时取得用户与组织 owner 写锁。关系删除会缩小最终扫描集合，组织锁也已经阻止新的成员写入，因此内部清理无需反向取得成员用户锁。这个调用边界保持多 owner 升序规则，同时避免组织 purge 与成员写入形成锁环。

这个范围有意限定在账户删除需要扫描、清理或据以拒绝删除的所有者关系。issue、comment、commit 等历史署名继续使用 Gitea 现有用户删除映射，因为它们不阻止账户删除，也不应为 Codespace 引入新的关系模型。该边界既能证明删除后没有悬空 owner ID，又避免把所有带用户信息的业务写入都串行化。
...
- repository 创建、transfer/delete 与 `CreateCodespace` 记录插入通过 `modules/repository.WorkingLockKey` 使用同一个 `repo_working_{repo_id}`；创建提交后在释放 owner 写锁前取得新 repository lock 并完成文件初始化。实际修改 owner 的 transfer 对原/新 owner ID 去重升序加锁，pending transfer 的创建、取消和拒绝只使用 repository lock。需要多个锁的路径按 owner、repository、Manager、Codespace 的固定层级取得，内部已持锁实现复用同一 key；用户、组织或 Manager 删除一次只持有一个 Codespace 子锁。
...
- 用户或组织删除逐条清理关联 Codespace 时只取得 Codespace lock 并在事务中复检三条 owner 关系；绑定到其他 owner 或 `owner_id=0` Manager 的记录也按此处理，不取得外部 Manager lock。owner 自有 Manager 的身份删除仍使用 owner、Manager、Codespace 层级。
...
- 全部 repository 创建入口、package/version 写入口、组织与 team 成员写入口都先取得涉及 owner 的 `owner_write_{owner_id}`，并在事务中复读关系双方；账户最终删除持有同一 key 时，这些入口不能提交新的删除判定关系。
- 组织 purge 持有组织 owner 写锁时使用内部关系清理函数删除 `OrgUser/TeamUser`，不反向取得成员用户锁；并发公开成员写入要么先提交并被 purge 清理，要么在锁内复读时发现组织不存在。
...
- `boot.stage` 只允许 `prepare-runtime`、`configure-ssh`、`configure-git`、`clone-repository`、`checkout-commit`、`run-init-script`、`start-ide`、`report-endpoints`、`credential-refresh`、`ready`。create 使用 `prepare-runtime` 到 `report-endpoints` 的初始化顺序并进入 `ready`；resume 使用 `credential-refresh -> ready`。同一 `boot.operation_rversion` 只按对应顺序前进，已经接受 `ready` 后保持 `ready`。
...
- create 和 resume 的 `final done` 都要求当前快照的 boot 版本等于当前 operation 且 `stage=ready`。resume 在 active operation 内申请 Token、写入 Runtime credential 并上报 ready；Gitea 还要求当前 Token 行完整，才把主状态从 stopped 改为 running。旧版本的 ready 不能完成当前恢复；cache miss 时 Manager 用正 generation 重建当前 operation 快照。Manager 保留最新 boot 版本的终态结果，直到更高 create/resume 版本替换或 Runtime 删除，用于 `/boot` 幂等重试。stopped 主状态存在已领取且租约未到期的 active resume 时，初始化所需的 Gitea Token 与 Git SSH Key 可直接访问绑定仓库；面向用户的 open 和 Gateway SSH 仍等待 `final done` 原子写入 running。**设计如此：初始化脚本必须在 final 前验证真实仓库访问，而用户入口只有在凭据、ready 和运行状态共同完成后开放。**
...
- `endpoints` 只保存 Runtime 实际声明；稳定的 `workspace` 描述由 Gitea 另行生成。同名记录存在时 Manager 使用其 Runtime upstream，不存在时 Manager 使用内置 Web SSH；两种情况的 Open Token binding 都是 `endpoint_id=workspace`。
...
- endpoint ID 在单个 codespace 内唯一，internal SSH 不进入 Gitea 页面数据或任何面向用户的响应。
- metadata ready 且没有 `workspace` 记录时，默认 workspace open 仍有效，UI 使用稳定 workspace 描述，Manager 将其解析为内置 Web SSH。
- resume 在 final 前完成同一 operation 版本的 credential-refresh、Token 写入和 ready 上报；Manager 重启后从 active operation 和本地 boot 上下文继续，final 成功后无需恢复独立凭据任务。
...
- metadata generation 耗尽时内容变化没有本地部分提交，并返回 `version_exhausted`；Manager 持久化最小 pending 后清理该 UUID，已有 operation 由完整 inventory 或 lease timeout 收敛到既定结果。
