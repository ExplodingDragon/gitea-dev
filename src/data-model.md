...
| `repo_id` | 代码来源 repository ID | 允许悬空引用，用于历史展示；repository 被删除后显示 `source repository deleted` |
...
| `id` | 自增主键 | |
| `name` | 名称 | |
| `admin_state` | `enabled` / `disabled` | 管理员控制的管理态：disabled Manager 只能执行已有任务的 stop/delete 和状态分歧上报；在线态由 `last_online_unix` 和 timeout 推导 |
...
日志存储在 DBFS 单文件中，`codespace` 行保存当前日志元数据。`operation_status == running` 时允许追加日志。
