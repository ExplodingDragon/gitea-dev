...
| `status` | 当前主状态 | |
| `operation_id` | 当前 operation 标识 | 每次 create/resume/stop/delete 创建新的标识，用于 Manager 上报归属校验 |
| `operation_type` | `create` / `resume` / `stop` / `delete` | 当前 operation 类型 |
| `operation_status` | `queued` / `running` / `done` / `failed` | `running` 表示有正在执行的操作；过期上报识别、RPC 匹配和并发保护都以此为准 |
| `operation_deadline_unix` | lease 截止时间戳 | |
| `operation_recovery_deadline_unix` | 维护恢复截止时间戳 | Gitea/Manager 重启或 Manager offline 后进入恢复窗口时写入；用于区分日常维护恢复和真实失败 |
| `operation_last_recovering_unix` | 最近恢复上报时间戳 | Manager 上报 recovering 或恢复 operation 时更新 |
...
- `endpoint_id` 由 Manager 上报，并在同一 `codespace_id` 内保持唯一。
