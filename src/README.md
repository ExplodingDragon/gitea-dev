...
| 主体 | 职责 |
| --- | --- |
| Gitea | repository、ref 与 commit 校验；用户身份与权限（复用 `CanRead(unit.Code)` 统一入口）；Codespace 生命周期状态；Codespace Manager 注册与认证（参考 Actions runner 注册模式）；独立 Codespace Gitea Token 和 Git SSH 公钥绑定；Gateway Open Token 签发与校验；用户工作区 SSH 与 Git SSH 认证判定；operation 日志存储与读取（基于 DBFS） |
| Codespace Manager | 通过 Incus 创建、恢复、停止、删除 Runtime Instance；把 repository tag 映射为本地实例模板；管理 Runtime Token、Runtime HTTP API、Runtime Metadata、Endpoint upstream 和通信地址 |
...
flowchart LR
    ...
    Runtime["Incus Runtime Instance<br/>VM 或系统容器<br/>init.sh / sshd / 服务"]
...
- 每个已注册 `manager_id` 同一时刻对应一个 Manager 进程。operation worker、generation 和 Incus 实例映射由这个进程统一维护；Manager 通过本地状态目录独占锁保证同一目录只有一个进程启动。部署时为每个并行进程注册独立身份和状态目录。
...
**数据边界**
- Gitea 保存生命周期状态、固化 Git 协议、独立 Codespace Token、Git SSH 公钥绑定、Manager 最新状态报告版本和 Codespace 日志；Git SSH 私钥只保存在 Runtime，Incus 实例、模板快照、镜像、资源规格和网络选择由 Manager 保存。
...
- Runtime Instance 按 Codespace 固化协议访问 Gitea 标准 Git HTTP(S) 或 SSH，并访问 repository Web URL；Codespace 专用调用只发给 Manager Runtime HTTP API。
...
sequenceDiagram
    ...
    opt Git SSH
        Runtime->>Manager: 上报 init.sh 生成的 Git SSH 公钥
...
- Runtime 始终使用代表创建用户的 Codespace Gitea Token 调用开发协作 API；HTTP 协议同时用它完成 Git smart HTTP，SSH 协议使用 init.sh 生成、只绑定当前 Codespace 仓库的专用 Key。两种 Git 入口都继续执行 Gitea 当前用户、repository、unit、分支保护和权限检查。
- Git 协议在 Codespace 创建时按站点默认值固化。已有 Codespace 的 resume 继续刷新原协议凭据，站点默认值变化只影响新建对象，避免 workspace remote 与凭据流程发生半切换。
...
- **设计选择：Token 行的持久生命周期与单次请求授权分别判定。** Token 行随有效 create/resume 初始化期或 `running` 保留，并在这三个阶段直接用于绑定仓库的 Git/LFS/API；稳定 `stopped` 没有 Token。resume final failed/timeout、stop、`failed`、`deleting` 或物理删除会物理删除 Token 行。站点排空或用户登录限制成立时，新请求分别返回状态不可用或登录受限；条件恢复且 Codespace 仍处于允许阶段时，同一 Token 可继续使用。repository 删除后 `repo_id` 写为 0，Token 仍可签发和保存，但 repository 请求均因没有匹配的绑定仓库而拒绝。
...
- stopped Runtime 只通过 Gitea 下发的 resume operation 恢复；Manager 在该 operation 内申请新 Gitea Token，并按固化协议刷新 HTTP helper 或确认 init.sh 已有 Git SSH Key，上报当前版本 ready 后才能 final done 进入 running。初始化阶段的开发凭据可以直接访问绑定仓库，面向用户的 open 和 Gateway SSH 等待 running。
...
- Manager 可修改名称、版本、tags、容量、Gateway/SSH 地址和 host key，并通过完整 Declare 快照覆盖 Gitea 当前展示与匹配数据；Declare 响应同时下发服务端选定的心跳周期、Runtime Metadata 刷新周期和双向消息上限。Gitea 不保存声明历史，已有 Codespace binding 不随声明变化迁移。
...
- Gitea 重启和 Manager 重启按日常维护事件处理。重启不改变 codespace 主状态、active operation 或 Gitea deadline，交互入口可以临时返回 metadata_rebuilding/recovering 分类；Manager 重启后暂停恢复出的 worker，本地上下文完整的 operation 只有成功 Fetch 续租并取得新的相对有效时长后才继续，之后的 inventory 按正常当前状态差异处理。
...
- Gateway session 使用 TTL、idle timeout 和持续授权复检：普通 HTTP 在每次请求转发前同步检查，WebSocket/SSH 按定时器检查。Open Code 交换先建立最长 30 秒的 `connecting` session，浏览器首次无 code 请求将其激活；同一浏览器重复 Open 时只原子替换请求 cookie 指向的同 Host、同 binding 旧 session，其他浏览器不受影响。
- 自动暂停由 Manager/Gateway 根据 HTTP、WebSocket、IDE 和 SSH live session 及本地单调时钟判断；running/ready、设置启用、没有 worker 且 session 为 0 时开始 Codespace 级计时，覆盖首次 ready、最后一个 session 关闭和重新启用。达到有效超时后，Manager 调用 Gitea，由 Gitea 使用当前启用值、有效超时、交互版本和 active operation 创建普通 stop。暂停完成仍是 `stopped`，用户通过普通 resume 恢复；`default/custom/never` 为每个 Codespace 提供站点默认、自定义或关闭空闲暂停的明确设置。延迟快照最多影响本地计时，不能绕过 Gitea 当前值复检。
...
- resume 完全使用已初始化 workspace，在进入 running 前申请新 Gitea Token、按固化协议刷新 Runtime Git 凭据并上报 ready，不重新读取 repository payload 或 checkout 初始 commit。
...
- `[codespace] GIT_PROTOCOL=http|ssh` 选择新建 Codespace 默认协议并固化到记录。功能启用时，默认协议和存量 creating/running/stopped 记录所需的 HTTP/SSH 接入面必须可用；HTTP 检查 `DISABLE_HTTP_GIT`，SSH 检查 SSH 服务和可信 Host Key，配置错误在 Codespace 组件初始化前阻止启动。
...
- `ENABLED=true` 时默认协议和存量对象需要的每种 Git 接入都在 Codespace 组件初始化前通过校验；排空模式允许关闭 Git HTTP 或 SSH 并继续完成 stop、delete 与管理清理。
...
- 每个 Codespace 只有一个串行 metadata 发布任务管理完整快照和 generation。`ReportRuntimeMetadata` 的成功空响应确认 Gitea 接受本次请求快照；请求携带当前 operation ready 时即可继续 final，后续 Endpoint generation 异步收敛。Endpoint 声明以字段形式保存在既有 Codespace 当前快照中，使路由、boot、internal SSH 和 generation 共享同一原子 rename 提交点；极端掉电后读取完整旧版或新版时，通过 Fetch、inventory 和 Runtime 幂等请求恢复。同步期间新增入口暂不展示，已删除入口由 Gateway 当前路由复检拒绝。
...
- 创建者详情页面数据中的 workspace label、非 workspace Endpoint 列表和 SSH 命令由服务端规范化；`open_endpoint` 只在列表非空时出现，SSH 只展示 Manager 公开地址和 host key。完整详情页与状态 HTML 片段使用同一服务端数据和权限判断。组织所有者和站点管理员使用不含详情、日志、自动暂停、Endpoint 或 SSH 的治理列表数据，只能执行当前状态允许的直接管理动作。
