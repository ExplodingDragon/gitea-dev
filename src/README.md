...
| 主体 | 职责 |
| --- | --- |
| Gitea | repository、ref 与 commit 校验；用户身份与权限（复用 `CanRead(unit.TypeCode)` 统一入口）；Codespace 生命周期状态；Codespace Manager 注册与认证（参考 Actions runner 注册模式）；独立 Codespace Gitea Token 和 Git SSH 公钥绑定；Gateway Open Token、公共 Endpoint 与已有 session 校验；用户工作区 SSH 与 Git SSH 认证判定；operation 日志存储与读取（基于 DBFS） |
| Codespace Manager | 通过 Incus 创建、恢复、停止、删除 Runtime Instance；使用部署侧 `default` 环境创建外层实例；执行可替换的 Dev Container 生命周期脚本；周期验证 running Codespace 的基础交互能力；管理 Gitea Token 交付、Git SSH 材料、Runtime Metadata、CPU/内存/磁盘用量采样、Endpoint 路由和 Incus 后端 |
| Codespace Gateway（Manager `serve` 进程内组件） | 需要认证与公共 Endpoint 接入；用户 SSH 接入；Gateway session 和公共连接管理；通过 Manager 身份调用 Gitea 完成访问校验；通过 Incus proxy、exec 和 file API 接入 Runtime |

本文承担整体边界和阅读导航。生命周期转换以[状态机](state-machine.md)为完整定义，持久字段以[数据模型](data-model.md)为完整定义，通信字段与校验以[RPC 协议](rpc-spec.md)为完整定义，Codespace Token 的 API 入口策略以 [`codespace-token-routes.yaml`](codespace-token-routes.yaml) 为完整定义，运行编排与恢复分别以[生命周期流程](lifecycle-flows.md)和[维护与恢复](maintenance-recovery.md)为完整定义，脚本输入、输出和内置 Dev Container 行为以[脚本契约与内置 Dev Container 实现](builtin-scripts.md)为完整定义；Gitea 与 Manager/Gateway 文档负责把这些规则落实到各自组件。总览和术语表保留跨组件规则的简要说明，组件章节会保留实现该组件所需的输入、结果和验收点。维护同一规则时先修改承担完整定义的专题文档，再同步相关摘要和组件行为，使每条规则有唯一的完整定义，同时让读者在当前章节取得足够的实现条件。
...
flowchart LR
    ...
    subgraph ManagerDeployment["Codespace Manager 部署"]
        Manager["Codespace Manager<br/>worker / Incus / Gateway"]
        Gateway["Codespace Gateway<br/>Endpoint / SSH 入口"]
    end

    Runtime["Incus Runtime Instance<br/>VM 或系统容器<br/>脚本准备的 workspace / Endpoint 服务"]
...
- 每个已注册 `manager_id` 在受支持部署中对应一个 `gitea-codespace serve` 进程。该进程同时运行 Manager worker、Gateway HTTP/WebSocket 和 Gateway SSH listener，并在内存中统一维护 operation worker、generation、Incus 实例映射、Endpoint 路由、session 索引和 Codespace 协调状态。Manager 通过本地状态目录独占锁保证同一目录只有一个进程启动；部署时为每个并行进程注册独立身份和状态目录。
...
- Manager 核心执行 init、start、stop 的统一脚本契约；内置套件以 Dev Container 提供开发环境，本地自定义套件必须产生相同的 workspace、Dev Container 目标、共享环境、结果与 ready 条件。具体契约和默认实现见[脚本契约与内置 Dev Container 实现](builtin-scripts.md)。
...
sequenceDiagram
    ...
    Gitea-->>Gateway: 允许
    Gateway->>Gateway: 解析当前 Dev Container 的 code-server Web IDE
    Gateway->>Runtime: 通过已验证的固定 Web IDE 目标代理
```

实现验收点：

- 用户 Endpoint/SSH 数据流不经过 Gitea，鉴权请求通过 ManagerService 到达 Gitea。
- Runtime 不访问 Manager 端口；Endpoint 声明写入实例内 manifest，Manager 通过 Incus 主动读取。
- Gateway 不把 open code、Gitea token 或 Manager Secret 转发给 Runtime。
...
- Codespace 复用 Gitea 现有用户、组织、仓库、权限（`CanRead(unit.TypeCode)` 统一入口）、Token hash/Secret 工具、SSH key、登录限制、git、Pull Request 和 Actions task 领取模型；Codespace Token 使用独立表，不复用普通 PAT 行。
...
- **设计选择：Token 行的持久生命周期与单次请求授权分别判定。** Token 行随有效 create/resume 初始化期或 `running` 保留，并在这三个阶段用于源仓库、已确认附加仓库和公开匿名读取；稳定 `stopped` 没有 Token。resume final failed/timeout、stop final、`failed`、`deleting` 或物理删除会物理删除 Token 行。active stop 创建前已有的 Token 可以继续使用，但 Manager 不能重新请求明文。站点排空或用户登录限制成立时，新请求分别返回状态不可用或登录受限；条件恢复且 Codespace 仍处于允许阶段时，同一 Token 可继续使用。源 repository 删除后 `repo_id` 写为 0，并清理以它为来源的附加授权；Token 只保留固定身份/公共信息入口和匿名公开读取。
...
- Codespace Token 使用 `gcs_` 专用前缀和独立表；Web/API 认证入口先识别该前缀并进入 Codespace 专用认证分支。该分支验证失败时直接返回认证失败，普通密码、PAT、Session 或 Reverse Proxy 认证不再处理该凭据。resolver 通过一次候选查询验证 Token，并生成包含 Codespace、创建用户、源仓库、附加仓库授权和登录限制的请求内认证数据；后续权限检查复用这份数据。API 显式标记当前用户、公共服务信息、signed artifact，以及 Code、Issues、Pull Requests、Wiki、Releases、Actions 仓库能力；未分类路由默认拒绝。**设计理由：Codespace Token 是用户开发凭据，但不是完整账户凭据。**源仓库使用用户当前权限，附加仓库逐仓库、逐单元确认，公开读取按匿名权限；仓库管理、账户、组织、Package、Notification、创建仓库和创建 fork 不属于能力范围。通知使用 Gitea 现有机制；SSH 认证限流由 Gateway 执行，Manager RPC 通过认证、双向消息大小和控制面超时限制资源使用。
...
- Dev Container 是唯一仓库级开发环境配置。Gitea 使用 hujson 读取 JSONC 候选，只解析名称和 `customizations.gitea.repositories`，候选之间不融合；标准字段由官方 Dev Container CLI 解释。仓库不能选择 Manager tag，基础设施环境固定由部署侧 `default` 决定。
- 失败为终态，通过 delete 退出。
...
- Manager 周期使用 Incus file/exec 和 Endpoint proxy 状态执行固定无副作用检查，确认 stable running Codespace 的基础交互能力。每轮开始时固定符合条件的 UUID 集合，全部仍符合条件的对象取得首次结果后统一判断共享故障；未达到共享条件时，连续 3 次失败的对象停止 Incus 实例、保留根存储并通过现有 stopped 状态报告收敛。检查不访问 repository、用户进程或普通 Endpoint 服务内容，也不增加脚本入口和 Gitea 状态。
...
- 默认 open 始终绑定逻辑 Endpoint `workspace`，Manager 把它代理到当前 Dev Container 的固定 code-server Web IDE。Gitea 只授权稳定的 `endpoint_id=workspace`；Runtime Endpoint 不能声明这个保留 ID，因此用户入口不会随应用端口声明改变。
...
- 公共 Endpoint 的普通 HTTP 请求检查本地状态和最多 1 秒的新鲜 `ValidatePublicEndpoint` allowed，持续连接按固定周期复检；不建立用户 session、不注入用户 ID、不更新交互或活跃时间，并使用独立的 per-Endpoint/per-IP 并发上限。**设计如此：写入 Runtime 本地 manifest 的工作环境进程是 Endpoint 声明主体，`public=true` 不要求 Gitea 页面再次确认；repository 可见性、删除和用户权限也不改变已经声明的值。**
...
- Codespace、Manager 和 registration token 这三类个人关系统一使用 `codespace_user_{user_id}`：写入在专用关系锁内复读个人用户，用户删除在同一锁内完成清理和最终复扫。`user_id=0` 使用 `codespace_user_0` 串行化站点全局 Manager 与 registration token，不对应可删除账户。
- repository 删除、`CreateCodespace` 记录插入、重命名和 transfer start/accept/reject/cancel 使用同一 `repo_working_{repo_id}`，并在锁内复读适用记录；普通 repository 创建、push 和设置修改保持 Gitea 现有并发语义。`CreateCodespace` 只需取得创建者的 Codespace user relation lock 和 repository lock。
- 用户删除按 keyset 分批、逐 Codespace 短事务清理该用户创建的 Codespace、开发凭据和日志，并清理该用户的个人 Manager、地址与 registration token。组织删除不进入 Codespace 用户清理；组织 repository 删除只把相关 `repo_id` 写为 0。
- Manager 删除时，持续持有对应 Codespace user relation lock 和 Manager lock，按 UUID keyset 分批、逐 Codespace 短事务清理 binding、开发凭据和日志，空集合复检后删除 Manager 地址与 Manager；online、offline 和 recovering 都使用相同路径，不联系 Manager。
- **设计如此：Codespace user relation lock 不是 Gitea 的通用账户写锁。**它只解决本功能个人关系与用户删除的提交竞态；repository、package、组织成员和 team 成员继续使用 Gitea 现有服务。需要同时取得多类锁时，顺序固定为 Codespace user relation、repository、Manager、Codespace。
- **设计选择：用户和 Manager 删除对调用方同步完成，内部按有界批次和可重试短事务清理 Gitea 本地资源。**用户删除在其 Codespace、个人 Manager 与 registration token 清空后返回成功，Manager 删除在 binding 与地址清空后返回成功；数据库步骤失败时保留父记录，重试继续处理剩余项。绑定到其他用户或站点全局 Manager 的 Codespace 只取得 Codespace lock。仍有效 Manager 在后续完整 inventory 中清理无记录 UUID；随用户删除而失效的个人 Manager 运行资源由部署运维处理。
...
- `[codespace] GIT_PROTOCOL=http|ssh` 选择新建 Codespace 首次 clone 的首选协议，但不写入 Codespace 记录。功能启用时先推导当前可用的 Git clone 能力：HTTP 由 `[repository] DISABLE_HTTP_GIT=false` 提供；SSH 由可用 SSH 服务和可信 Host Key 提供。默认 `http` 且外部 SSH 没有配置 `[codespace] GIT_SSH_KNOWN_HOSTS` 时，SSH clone 明确关闭，create payload 的 `repo_clone_ssh_url` 为空，HTTP clone 仍可运行。没有可用首选协议时禁用本进程的 Codespace 功能并记录配置错误，Gitea 其他功能继续启动。**设计如此：**`GIT_SSH_KNOWN_HOSTS` 是 Runtime 严格校验 Gitea SSH 服务身份的信任材料，不是启用 Codespace 的前置开关；默认 HTTP 部署不应因为没有外部 SSH Host Key 而无法启动。
...
- repository 删除不级联销毁已初始化 Codespace，Codespace Token 访问仍经过工作状态、仓库能力和 Gitea 原有权限检查。
- Codespace token 访问 Issue/PR API 时，绑定 repository 使用创建用户的身份能力；非绑定公开 repository 的读请求按 Gitea 公开访问规则处理，写入、管理、私有或不可见目标仍不能通过。既有 fork、commit、dependency 和 project 关系产生的后续行为按创建用户当前 Gitea 权限处理，Codespace 生命周期变化不撤销已创建的 auto-merge 等持久业务对象。
...
- 用户删除成功后，不存在以目标用户为创建者或 Manager 用户的 Codespace、个人 Manager 或 registration token；并发写入由 Codespace user relation lock 内的复读与最终复扫裁决。
- 删除普通用户保留 `user_id=0` 的站点全局 Manager、地址和 registration token，也保留其他用户的资源；组织删除不扫描这些表。
...
- Gateway 的 Open 流程使用 Codespace 本地协调边界完成最终检查、session 名额预留和登记。SSH 在受限握手内完成 Gitea 校验与本地后端确认，握手成功后登记 live session 并复查本地目标；stop/delete/cleanup 清除目标时会取消已登记连接，先完成清除时会使并发 SSH 的最终复查失败。两种入口都不会在生命周期关闭完成后保留可用连接。
...
- 默认 open 的 `OpenTokenBinding.endpoint_id` 始终是 `workspace`；同一 URL 固定使用当前 Dev Container 的 code-server Web IDE。
...
- Web IDE 只使用 Manager 当前已验证的 Incus Runtime、Dev Container 和固定 code-server 端口，浏览器不能选择 upstream。HTTP 与 WebSocket 沿用同一 Gateway session；stop、delete、目标替换和复检失败会关闭现有代理连接。
- 创建者详情页面数据中的 workspace、非 workspace Endpoint 列表、CPU/内存/磁盘当前用量和 SSH 命令由服务端规范化；`open_endpoint` 查询参数只用于 Gateway 登录恢复时选择当前页面中的入口，SSH 只展示 Manager 公开地址和 host key。资源指标只用于创建者查看当前工作区状态，缺失时显示暂不可用，不影响操作。完整详情页与状态 HTML 片段使用同一服务端数据和权限判断。站点管理员使用不含详情、日志、自动暂停、Endpoint、资源指标或 SSH 的治理列表数据，只能执行当前状态允许的直接管理动作。
- 组织仓库只是 Codespace 的代码来源；组织设置不提供 Manager、registration token 或成员工作区治理入口。Codespace 由创建者本人管理，站点管理员通过全站治理列表处理必要的 stop/delete/force delete。
- 同一浏览器重复 Open 以旧 cookie 原子替换同 binding session，不额外占用配额或触发自动暂停 0/1 变化；未激活的新 session 在 30 秒后清理。
- SSH 协议握手、Gitea 认证和 Incus 后端确认共用 `gateway.ssh.handshake_timeout`，默认 30 秒；公钥删除立即阻止新连接，已经建立的连接继续受用户状态、功能开关、生命周期、复检、TTL 和空闲超时控制。
