...
| Codespace Manager | 通过 Incus 创建、恢复、停止、删除 Runtime Instance；把 repository tag 映射为本地实例模板；执行可替换的本地初始化脚本；周期验证 running Codespace 的基础交互能力；管理 Gitea Token 交付、Git SSH 材料、Runtime Metadata、CPU/内存/磁盘用量采样、Endpoint proxy 和 Incus 后端 |
...
sequenceDiagram
    ...
    Gitea-->>Gateway: 允许
    Gateway->>Gateway: 解析 Runtime workspace Endpoint 或内置 Web 终端
    Gateway->>Runtime: 有 Runtime workspace 时经 Incus proxy 代理
```
...
- repository Codespace 配置只接受单个 YAML mapping 和字段 `tag`，未知、重复或错误类型字段直接返回配置错误。tag 只选择合格 Manager 已声明的开发能力和资源模板，不承担用户授权或信任等级；站点全局模板适用于全部创建者，个人模板只适用于该用户创建的 Codespace。
- 失败为终态，通过 delete 退出。
...
- 默认 open 始终绑定逻辑 Endpoint `workspace`。Runtime 声明同名 Endpoint 时 Manager 使用其 Endpoint proxy；未声明时由 Manager 内置 Web 终端通过 Incus exec 建立 PTY 登录 shell。Gitea 只授权稳定的 `endpoint_id=workspace`，Manager 负责解析当前实际目标。
