...
- `[codespace] GIT_PROTOCOL=http|ssh` 选择新建 Codespace 的首次 clone 首选协议并固化到记录。功能启用时只校验首选协议的接入能力：默认 `http` 要求 `DISABLE_HTTP_GIT=false`；选择 `ssh` 时要求 SSH 服务可用并能取得可信 Host Key。这样默认安装不会因为未配置外部 SSH known_hosts 阻止启动，管理员明确选择 SSH 首选协议时仍会在启动阶段得到配置错误。
