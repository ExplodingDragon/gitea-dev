...
4. 递增 `inventory_generation`，通过 `ReportInstances(snapshot_complete=true)` 上报完整快照。
...
- `inventory_generation` 由 Manager 单调递增；Gitea 只使用高于 `codespace_manager.inventory_generation` 的完整快照计算 missing，相同版本幂等返回且不重复计算，更低版本返回 stale。
...
- incomplete snapshot 不驱动 missing，旧完整 generation 不驱动任何差异写入。
...
Manager 每次主动 stop/resume 递增 `runtime_generation`。Gitea 接受更高版本，相同版本在主状态已匹配时幂等接受，更低版本返回 stale，从而避免重试或网络乱序把旧事实写回主状态。
...
- online 超时按 operation 失败处理，recovering 和 offline grace 内暂缓。
...
- Manager 重启后使用相同 operation_rversion 继续幂等 stop/resume。
...
- 完整 inventory 只有在 generation 更新时才驱动 missing 判定。
