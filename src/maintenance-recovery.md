...
| `running/stopped` 且无 active operation | failed | 接受，写 `status=failed`、吊销 token 并清除交互 cache。 |
...
超过 `FAILED_RETENTION_DAYS` 的 failed Codespace 由 Gitea 定时任务直接物理删除其 token、日志、记录和 cache，不查询 Manager 状态、不创建 operation 或 instruction。failed 已经不能 resume，保留期结束只表示 Gitea 不再保留该终态及诊断日志；运行侧若仍有同 UUID Runtime，后续完整 inventory 将其视为未知 UUID 并忽略，不借此恢复记录或要求 Manager 清理。
