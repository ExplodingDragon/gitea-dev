...
3. `DeclareManager(manager_runtime_state=recovering)`，取得服务端心跳周期、Runtime Metadata 刷新周期和控制面消息上限；响应非法时以零容量保持 recovering，不开始新的 Incus 枚举或修改，步骤 2 已经持久化的 `cleanup_pending` 继续按原授权完成。
4. 全量枚举带当前 Manager 归属字段的 Incus 实例，并读取每个实例的当前状态。
5. 生成 Runtime inventory 快照。
6. 递增 `inventory_generation`，通过 `ReportInstances` 上报完整快照；正数 observed operation 高于 Gitea 当前版本时，因 Manager 级 `state_history_conflict` 停止恢复并等待运维处理。
7. 恢复 Runtime 映射和本地 worker 上下文，并把全部恢复 worker 保持在暂停状态。只有上下文完整的 active operation 才放入 `FetchOperations(observed_operations=...)`；服务端版本更高时重新取得当前 payload，上下文缺失的 operation 保持不执行并等待原 deadline。
...
9. 执行 `DeclareManager(manager_runtime_state=online)`，表示完整 inventory、Runtime 映射和可恢复 worker 上下文已经建立；随后 Fetch 仍使用 `capacity_available=0` 和空的 `accepted_operation_types`，暂不领取新 create/resume。
10. 逐 Codespace 完成交互恢复：active create/resume 继续当前 operation；Gitea 与本地都为 running 的对象验证凭据、internal SSH 和路由，并通过唯一 metadata 发布任务重报当前 ready 快照；本地 stopped/failed 或待清理对象提交明确状态结果并保持准入关闭。稳定 stopped 保留 Incus 实例及根存储，等待下一次 resume 从该实例重建 metadata。
11. running 对象收到当前 ready 的成功上报且没有待提交状态后，在本地协调锁内开放 session 准入；单个仍在重试的对象保持关闭，不阻塞其他健康 Codespace。
12. 上述逐 Codespace 恢复任务都进入“已开放交互、稳定 stopped/failed、active operation 继续执行或明确后台重试”之一后，恢复领取新的 create/resume。

Manager 重启后先恢复已有 Runtime 信息，再领取新的 create/resume，使已有 Codespace 优先恢复控制。Declare online 表示完整 Incus inventory、Runtime 映射和可继续的 worker 上下文已经恢复，可以接受逐 Codespace RPC；它不直接开放任何 Codespace 的本地 session。Runtime Metadata、凭据检查和待提交状态报告可以在 online 后逐项完成，Gateway/SSH 的前置检查与最终登记都会拒绝本地准入尚未开放的对象。这样外部 cache 即使保留重启前的 ready 快照，也只能让 Gitea 维持数据库主状态，不能越过 Manager 本地恢复边界。Manager 在最后一步 Fetch 前保持 `capacity_available=0` 或省略 create/resume，避免提前领取新工作。`operation_rversion` 相同且本地上下文完整的 running operation 可以请求续租，但收到新的相对有效时长前仍保持暂停；服务端较高版本由 Gitea 返回当前 operation 数据，Manager 按请求发出时的版本起点判断返回结果是可接受更新、延迟响应还是真正倒退。站点排空时，Manager 处理 abort、stop、delete 和状态差异指令；已经到期的 operation 由 Gitea 按普通超时表写入结果。
...
sequenceDiagram
    ...
    M->>G: DeclareManager(online, capacity=0)
    G-->>M: online 确认
    loop 逐 Codespace 交互恢复
        alt running 且本地依赖完整
...
Manager 在发送 Fetch 前记录本地单调时钟，成功收到 payload 或续租回执后将请求开始时间加 `lease_valid_for_milliseconds` 作为普通 worker 的本地截止点。达到该点后停止新的 Incus 变更并保留 operation 上下文；只有收到新的成功 Fetch 续租或 final outcome 才更新处理。服务端绝对 Unix deadline 不通过协议返回。**设计如此：`recovering` 不暂停、不延长 Gitea 侧 operation lease；Manager 重启后因原单调时钟基线丢失而主动暂停 worker，成功续租后才恢复执行。**上下文缺失的 operation 不通过 Fetch 重新开始执行，而是等待原 deadline 触发同一 timeout 结果。abort 的相对时长为 0，只立即执行本轮缩减清理并提交 final failed。
...
Manager 重启后从本地完整快照恢复 Gitea-issued operation，但全部 worker 先暂停。上下文完整的 operation 通过普通 Fetch 请求同版本续租，只有取得新的相对有效时长后才继续；上下文缺失或 Gitea 已按原 deadline 超时的 operation 不重新执行。resume 的 Token、协议所需 Git 凭据和 ready 都在 final done 前完成，因此成功续租的 active resume 可继续未完成步骤；SSH 公钥确认响应丢失时，init.sh 使用已落盘公钥幂等重试。已经 final done 的 resume 没有后置任务。最新 boot 终态结果保留到更高 create/resume 版本或 Runtime 删除，以支持 Runtime API 重试。
...
| resume | Runtime 已运行且基础 metadata 完整 | 在当前 operation 内申请新 Gitea Token，按固化协议刷新 HTTP helper 或确认已有 Git SSH Key，上报当前版本 `ready` metadata，再提交 final done。 |
...
stop 让 running Codespace 退出可交互态并保留可恢复资源。Runtime 不存在则无法满足 stopped 可恢复语义，故进入 failed。resume 在 active operation 内完成 `credential-refresh`、Token、固化协议对应凭据和 ready；临时错误通过 lease 续租继续重试，确认无法写入凭据时停止本轮 Runtime 并 final failed，主状态保持 stopped。
...
- resume 在进入 running 前取得新的 Gitea Token，按固化协议刷新 HTTP helper 或确认 Runtime 已有 Git SSH Key，再上报当前版本 ready；初始化阶段即可使用两类开发凭据验证绑定仓库，final 成功后仍保留最新 boot 结果供幂等重试。repository 初始化和 Git SSH 密钥生成只在 create 中执行。
- 最新 boot 结果使 final 后的相同 POST 重试保持幂等；更高版本 delete 通过普通 operation 替换停止旧 resume worker。
