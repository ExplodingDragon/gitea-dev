...
- 启动路径不直接扫描或改写 Codespace；启动后的普通 Cron 按原时间字段处理已经满足 operation 超时或 failed 保留期到期条件的记录。
...
2. 读取并校验版本 1 的 Manager 状态文件；失败时以固定硬错误退出，不发送 RPC 或修改 Incus 实例。先读取全部已有 Codespace 状态文件并确认格式版本为 3，再为有效对象初始化关闭 session 准入、空 session 集合和空 ready 接受记录；优先续做 `cleanup_pending=true` 的本地清理和 `health_stop_pending=true` 的实例停止。健康停止使用意图中已经保存的 observed operation 版本，不依赖停止前已清除的 Runtime Metadata。已经确认为版本 3、但完整结构校验失败的单个状态文件按“持久状态损坏处理”清理该 UUID；文件不可读、不是合法 JSON 或无法取得受支持格式版本时按 Manager 整体硬错误处理。
...
- 三个必要 listener 在首次 recovering Declare 前成功绑定；任一 listener 或关键全局循环意外终止时关闭 session 并整体退出，Gitea 最终按 heartbeat 派生 offline。
- SIGINT/SIGTERM 在进程关闭期限内暂停启动类 worker 并保存状态；关闭超时、进程崩溃和正常重启都使用相同 lease、快照与 inventory 恢复边界。
...
Manager 可以在重启或运行期间发现本地 Incus 实例已经 stopped、运行健康检查已经按确认规则停止实例，或者实例仍存在但某个 Codespace 已确认不可恢复。Gitea 当前没有对应 active operation 时，Manager 使用 `ReportRuntimeTransition` 上报 stopped/failed；running 只能由当前 create/resume operation 的 final done 建立。
...
Manager 在首次发送前原子保存 `pending_runtime_transition(target_state, runtime_generation, observed_operation_rversion)`。三个字段全部必填，对象存在就表示等待确定结果，不保存原因、时间或重试标志。stopped 报告只有在实例确认停止后才建立；failed 报告只有在资源已确认不可恢复且交互与冲突 worker 已关闭后才建立。网络错误、响应丢失和进程重启都使用相同字段重试，因此同一事实不会仅因重试分配新 generation。
...
failed 状态报告提交前，Manager 先关闭本地 session，并停止该对象尚未完成的 metadata 上报和生命周期 worker。Gitea 返回接受或目标主状态已经成立的幂等成功后，Manager 原子把 transition pending 替换为本地 cleanup，再删除归属 Incus 实例和本地快照；stopped 成功则清除 pending 并保留实例。清理失败或进程重启时由 pending 快照续做，尚存实例继续上报 failed inventory。Gitea 记录仍为 failed 时返回 `cleanup_local_runtime`；记录已经由其他删除流程物理删除时，成功的 inventory 查询按无记录返回同一清理指令。

active operation 与 pending 冲突时，Manager 保留旧 pending 并 Fetch 当前 operation。operation 完成后本地事实仍成立时，使用最新 operation 版本和更高 runtime generation 原子替换旧 pending；原事实已不成立时清除旧 pending，delete 或 cleanup 已经接管时进入本地清理。`stale_operation` 清除旧 pending 并通过完整 inventory 重新取得当前关系；`stale_generation` 以服务端值为基线重新确认本地事实，仍成立时建立更高版本 pending，不再成立时清除旧 pending。`codespace_not_found` 不直接删除 Runtime，继续等待 inventory 的明确 cleanup。

实现验收点：

- 主动 transition 只在无 active operation 时生效。
- generation 确保旧 stopped/failed 状态报告不能覆盖新主状态。
- pending 在首次 RPC 前持久化，响应丢失和重启使用相同目标、generation 与 operation 版本重试。
...
- 单 Codespace 不可恢复但 Runtime 仍存在时使用 failed 状态报告；运行健康检查确认基础交互持续失败时先停止实例并使用 stopped 状态报告。Manager 整体离线、metadata 丢失、单次连接错误和健康调度暂停都不批量改写主状态。
...
- operation conflict、stale operation、stale generation 和 Codespace 不存在分别进入 Fetch、inventory、重建 pending 和等待 cleanup 的固定分支。

## Active Operation 超时

`operation_created_unix + QUEUE_TIMEOUT` 是 queued operation 等待 Manager 领取的硬截止时间；`now >= deadline` 后即使 Cron 尚未扫描也不能再领取。running operation 的总执行期限固定为 `operation_started_unix + OPERATION_MAX_DURATION`，`operation_deadline_unix` 保存当前 lease 与总期限中的较早值。
...
Fetch 遇到已超过该硬截止时间的 queued 候选时，以 UUID 取得 Codespace lock，在短事务中按候选内部 ID 重新读取并确认 operation 版本、queued 状态和截止时间，再执行同一 timeout State Finalization；该项不被领取，也不计入服务端由两类容量推导的 payload 上限。Cron 处理未被 Fetch 扫到的过期记录。**设计如此：**UUID 保证同一外部对象进入同一个锁，内部 ID 则与数据库主键和调度索引一致，适合事务内复读和写入。
...
- queued timeout 使用创建时间；running deadline 同时受当前 lease 和固定总执行期限约束。
...
**设计如此：inventory 对无记录资源拥有唯一的远端清理判定权，但不是 `cleanup_pending` 的唯一来源。**这一区分防止普通 `resource_absent` 或 RPC 错误删除工作区，同时允许 delete、accepted failed 和已确认的当前格式损坏直接进入各自已有的清理闭环。
...
- 单个 Cron 任务的三个阶段各自读取最多 100 条稳定排序候选并使用逐条短事务；单条失败继续，阶段级错误不阻塞其他阶段并使任务返回失败，cache 清理失败不恢复已删除记录。
