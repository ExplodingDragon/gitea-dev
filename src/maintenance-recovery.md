...
- 单个 Cron 任务的三个阶段各自使用 100 条稳定 keyset 批次和逐条短事务；单条失败继续，阶段级错误不阻塞其他阶段并使任务返回失败，cache 清理失败不恢复已删除记录。
