...
Gitea 重启和 Manager 重启都属于日常维护事件。重启会短暂影响本地 cache、worker、Gateway session、Runtime Metadata 或 Runtime 上报数据，codespace 主状态仍由 Gitea 数据库和 State Finalization 维护。
...
| Runtime Report | Manager | 上报本地 Runtime 是否存在、metadata、执行结果 |

生命周期状态以 Gitea 数据库为准，本地 cache 和 Manager inventory 只提供运行信息。维护期间先保持数据库主状态稳定，再根据 Manager 重新上报的信息恢复交互能力或处理差异。
...
Manager 重启后先恢复已有 Runtime 信息，再领取新的 create/resume，可以让 Gitea 中已有 codespace 平稳接回，尤其是正在 stop/resume 的 operation。
...
- Manager 清理完成后下一次快照中该 Runtime 消失。
- cleanup 过程写入 Manager 本地日志和 Gitea 差异审计信息。

## Missing Runtime 处理

missing runtime 表示 Gitea 记录中应该存在 Runtime，但 Manager 完整快照中没有对应 Runtime。
...
Runtime 缺失说明 Manager 本地没有对应实例。对于 stop/delete，缺失满足目标结果；对于 running/create/resume，缺失表示交互能力无法成立，需要由 Gitea 进入失败状态或等待恢复窗口内的继续上报。

实现验收点：

- missing running 由 Gitea finalization 到 `error`。
- missing stopping 通过 stop done 进入 `stopped`。
...
维护重启期间，Gitea 给 Manager 时间重新上报完整运行信息；完整快照到达后，Gitea 按数据库主状态和当前 operation 处理差异。这样既能处理正常维护抖动，也能在真实差异出现时给出明确结果。
