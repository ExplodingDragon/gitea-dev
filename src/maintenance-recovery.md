...
生命周期状态以 Gitea 数据库为准，Gitea cache 和 Manager inventory 只提供运行信息。维护期间 Gitea 保持主状态稳定；Manager 恢复完成并上报完整 inventory 后，Gitea 根据事实处理差异。
...
| `running` | 主状态保持；Runtime Metadata cache miss 时等待 Manager 重建，open/SSH 返回 `metadata_rebuilding`，外部 adapter 保留的合法快照可继续使用。 |
...
Codespace 通过 Gitea cache adapter 保存 Open Token、Runtime Metadata 和短期页面展示数据。memory/twoqueue 重启后通常丢失，Redis/memcache 可在 TTL 内保留；cache miss 时 Runtime Metadata 由 Manager 重建，Gateway Open Token 由用户重新 open 生成，保留的合法 cache 仍必须通过当前数据库和权限校验。进程重启保留数据库状态，可以减少维护重启造成的批量误失败。`globallock` 的既有锁不会作为生命周期状态恢复，重试入口仍在取得新锁后重新读取数据库并执行条件校验。
...
Manager 重启后先恢复已有 Runtime 信息，再领取新的 create/resume，可以让 Gitea 中已有 codespace 平稳接回。`operation_rversion` 相同且本地上下文完整的 running operation 不需要重复下发完整 payload；版本缺失或不同则由 Gitea 返回当前 operation。Manager disabled 时不恢复 create/resume 执行；未超过可恢复边界时只处理 abort、stop、delete 和本地调和指令，超过边界的 operation 由 Gitea timeout。
...
Manager 在启动恢复、每个 `inventory_report_interval` 和 backend 资源异常事件后提交完整快照；周期上报让运行期间发生的外部 Runtime 删除或已知资源状态变化能够收敛，不依赖 Gitea Cron 保存 inventory。只有 backend 全量枚举成功、每个资源状态都可确定且扫描期间没有分页或连接错误时，快照才算完整；任一资源处于无法读取的未知状态或枚举失败时，本轮不调用 `ReportInstances`，保留 generation 并重试完整扫描。这样 Gitea 不会把一次不完整扫描误当作 missing 证据。

inventory 语义：

- 上报 Manager 持有的所有 Runtime 资源，而不只是 running 进程。
- stopped workspace、volume 或可恢复实例也必须上报。
- 第一版每次请求都是完整快照，不提供增量或 incomplete 模式；单个 Manager 最多持有 10000 个带其 label 的 Runtime 资源，单次请求最多 10000 个实例且 UUID 唯一。
- `inventory_generation` 由 Manager 单调递增；更高版本执行差异写入，相同 generation、相同规范化快照不重复写主状态但重新返回当前 instruction，相同 generation、不同快照返回 generation conflict 且不附 stale detail，更低版本返回 stale 和当前 generation。stale 时 Manager 用服务端当前值加一，同代冲突时用请求中的已知值加一；两者都重新完整扫描 backend、持久化新 generation 后提交当前快照。
...
| `creating` 且 active operation 缺失，或 deadline 与 restart grace 都已结束 | 进入 `failed`，吊销 token，清空 active operation。 |
...
Manager 可以在重启或运行期间发现本地 Runtime 已经 stopped、running，或者资源仍存在但某个 Codespace 已确认不可恢复。Gitea 当前没有对应 active operation 时，Manager 使用 `ReportRuntimeTransition` 上报事实。
...
| `running/stopped` 且无 active operation | failed | 接受，写 `status=failed` 并吊销 token；提交后尽力清除交互 cache。 |
| `running/stopped` 且有 active operation | 任意 | 拒绝，返回 `current_operation_conflict`。 |
| `failed` 且相同 generation 的 failed 重试 | failed | 目标状态已收敛，幂等成功，不刷新 failed retention 起点。 |
...
enabled Manager 在 online 或有效 recovering 期间可以上报三种 fact；disabled Manager 在这两种运行态下只上报 stopped/failed；已派生 offline 的 Manager 先声明 recovering，再完成 inventory 和 transition。

主动 running 与用户 resume 使用相同的 token 收敛边界，但不创建 operation：running fact 先携带 `boot.operation_rversion=observed_operation_rversion` 的新 `credential-refresh` 快照；Gitea 写入 running 后，Manager 申请新 token、刷新 credential，再以更高 metadata generation 上报 `ready`。ready 前交互不可用；credential 无法写入时停止 Runtime，递增 runtime generation 后上报 stopped。该恢复事实不更新 `last_active_unix`。
...
failed fact 提交前，Manager 先关闭本地 session，取消 pending metadata/Endpoint mutation 和后置 worker。Gitea 返回接受或目标已收敛的幂等成功后，Manager 可立即删除本地 Runtime；清理失败时保留本地快照并继续上报 failed inventory，Gitea 在仍有 failed 记录时返回 `cleanup_local_runtime`。如果 Gitea 记录已被其他删除流程物理删除，未知 UUID 仍不返回破坏性指令。

实现验收点：

- 主动 transition 只在无 active operation 时生效。
- generation 确保旧 running/stopped/failed 事实不能覆盖新主状态。
...
- failed fact 成功后可立即清理本地 Runtime，未完成清理通过 failed inventory 和 cleanup instruction 继续。
...
- 非 grace 的过期 UpdateOperation 不等待 Cron，直接写 failed 并返回确定 outcome。
...
Manager 重启后继续处理当前 Gitea-issued operation；已经 final done 的 resume 不恢复 operation，而是根据本地 boot 上下文的 `credential-refresh` 阶段继续后置凭据刷新。最新 boot 终态结果不是历史，保留到更高 create/resume 版本或 Runtime 删除，以支持 Runtime API 重试。
...
| resume | Runtime 已运行且基础 metadata 完整 | 上报 `credential-refresh` metadata，再上报 done；进入 running 后保留当前 boot 上下文，申请新 Gitea token、刷新 Git credential 并上报 `ready` metadata，完成后结束后置 worker 并保留最新 boot 结果。 |
...
resume final 或主动 running fact 后的 credential-refresh 可在 Manager online 或有效 recovering 期间申请 token，不要求 active operation 仍存在。`manager_disabled` 使 Manager 取消后置 worker、停止 Runtime 并上报 stopped，停止确认失败时上报 failed；`manager_offline` 先 Declare recovering；`codespace_not_found|manager_unregistered` 停止通信但不清理 Runtime；更高 stop/delete operation 取消后置 worker 并接管。
...
`UpdateOperation` 返回 `lease_renewed` 时 worker 使用新 deadline 继续；`final_accepted` 和 `idempotent_done` 清除同一版本的 operation 上下文，其中 resume done 继续保留 credential-refresh 后置上下文。`stale_operation` 停止旧 worker 并等待 Fetch 或 inventory instruction，不能覆盖本地更高版本上下文；`resource_absent` 只清除通信上下文并停止上报，不要求 Manager 删除 Runtime。
...
- Manager 主动 running/stopped/failed 事实通过 `ReportRuntimeTransition` 收敛。
