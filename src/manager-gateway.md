...
**设计如此：Manager 把脚本私有环境作为不透明键值保存。**内置直接运行或自定义开发环境的内部类型、容器标识、内部用户和转发状态都由脚本维护，不进入 Manager 当前快照的结构化字段；Manager 只消费凭据 UID/GID、workspace 路径和 internal SSH 等通用输出。devcontainer 只是使用该能力的一套自定义脚本案例，Go 代码没有对应模式。
...
**设计如此：运行健康检查复用通用 `internal_ssh`，不是第四个脚本或新的脚本阶段。**内置直接运行和完整自定义脚本套件都已经通过 activate 提供同一个内部 SSH 用户、端口和 Host Key；固定 exec 检查验证的正是 Gateway 后续要使用的入口。devcontainer 案例也只通过这个通用输出接入。Manager 不读取脚本私有变量，也不提供可配置项目命令，从而避免健康检查执行用户构建、访问 repository 或改变 workspace。
...
- `upstream_port` 始终是 Incus 通信地址上已经可以访问的实际端口。脚本若在 Runtime 内使用另一层环境，先自行建立实例内端口转发，再调用 helper 登记实际端口。Manager 不接收内部逻辑端口、实现类型、容器标识或转发描述。
...
- 结果结构、Manager 配置、当前结构化快照和 Endpoint API 不含 devcontainer、Docker、CLI、容器标识、容器用户或逻辑端口字段；内置和自定义脚本使用同一通用输出。
...
**设计如此：Runtime 内部授权位置由 activate 管理。**内置直接运行和自定义脚本可以把 sshd 放在不同内部环境，只有 activate 知道授权文件的实际位置；Manager 通过固定公钥输入和连接校验保持安全边界，认证失效则复用 stopped/resume 状态机恢复。devcontainer 案例不需要 Go 侧知道容器内授权路径。
