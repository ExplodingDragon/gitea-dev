...
- Manager 主动 resume stopped Runtime 时，先原子持久化包含当前 `operation_rversion`、新 runtime generation 和 `credential-refresh` boot 的当前上下文，再启动 Runtime 并调用 `ReportRuntimeTransition(running)`。请求超时或 Manager 重启后先重试该显式 fact，再提交完整 inventory；响应已接受时按目标状态幂等返回，版本冲突或上下文损坏时停止 Runtime 和交互入口、保留 workspace，并由 inventory 的 `stop_local_runtime` 完成收敛。inventory 只报告当前事实，不替代该持久上下文表达主动意图。
...
- report transition instruction 提供的当前 operation 版本可以在本地执行上下文损坏后恢复 stopped/failed fact 的版本基线；running 意图只从启动前持久化的主动 resume 上下文恢复。
- Manager 重启时先重试未确认的主动 running fact 再上报 inventory；无有效主动上下文的 stopped 主状态/running Runtime 最终停止进程并保留 workspace。
...
- 完整 inventory、Runtime 映射和 worker 上下文恢复后可以声明 online；逐 Codespace metadata、credential 和 pending fact 完成前不 Fetch 新 create/resume。
...
**设计说明（有意如此，容易产生歧义）：同步删除表示成功响应前 Manager 和绑定的 Gitea 资源已经清空，不表示内部只有一个数据库事务。**最多 10000 个 Runtime 的规模要求逐项短事务；失败时已提交子项不恢复，父记录保留并作为重试边界。
