...
2. `gitea-codespace register` 通过 `RegisterManager` 提交 registration token 和当前 `protocol_version=1`。
...
一个 `manager_id` 与一个 `state_dir` 共同对应一个活动 Manager 进程。worker 领取状态、`runtime_generation/metadata_generation/inventory_generation` 和 Incus Runtime 映射都由该进程写入本地状态目录，因此单一写入者可以保证版本和 Runtime 映射有确定顺序。本地状态目录独占锁会让共享该目录的第二个进程在发送 RPC 前退出；需要并行部署时，每个进程使用独立注册身份、secret 和状态目录。
...
- RegisterManager 先按唯一 token 索引读取候选 owner，只用来选择 Codespace owner relation lock；锁内再次读取同一 token 行并匹配明文后才认证成功。锁前读取与锁内复读之间发生的轮换、停用或 owner 删除会使注册返回 unauthenticated。
...
**设计理由：Registration Token 表只保存每个 owner 当前有效的注册入口。**轮换原地替换，停用物理删除；记录存在性即可完成认证判断。注册失败通过服务端日志诊断，因此凭据历史无需进入数据库或保留任务。
...
- RegisterManager 与 token 变更使用同一 Codespace owner relation lock，global scope 使用 `codespace_owner_0`。
...
- 每个 ManagerService request 只接受 `protocol_version=1`；不匹配不会创建身份、刷新在线时间或推进任何业务状态，Manager 保留资源并退出。
...
- Manager 只通过 Incus 管理 Runtime；启动时验证专用 project、权限和非集群模式，失败时不领取 create/resume。
...
- create 脚本同时取得规范 HTTP(S)/SSH clone URL 和首选协议；内置脚本按首选顺序尝试，自定义脚本可以选择实际 remote。脚本尝试 SSH 前生成或复用 Ed25519 密钥对，SSH 失败后改用 HTTP(S) 时允许保留已登记公钥，但 ready 只校验 HTTP helper。Manager 只转发公钥，私钥只存在于 Runtime，Git SSH 连接使用专用 known_hosts 和严格 Host Key 校验。
...
进程启动时只读取固定文件名并清理遗留临时文件，优先恢复 `cleanup_pending`，再恢复 `health_stop_pending` 和 `pending_runtime_transition`。`health_stop_pending` 先完成实例停止，确认 stopped 后在同一快照提交中替换为新的 stopped transition pending；已有 transition pending 直接使用原三个字段重试。`manager.json` 缺失、无法解析、校验失败或身份与配置不一致时，Manager 以固定硬错误退出，不发送 RPC，也不修改 Incus。这个文件决定 Manager 身份和 inventory 版本，缺失后无法安全证明调用方或完整快照顺序；管理员通过 Gitea 删除原 Manager，在 Incus 中按归属字段删除其全部实例和状态目录，再注册新 Manager。
...
- registration token 的轮换或停用只改变之后能否调用 `RegisterManager`，不改变已经注册 Manager 的 secret、identity 或 Codespace binding。
...
- registration token 轮换或停用后，已注册 Manager 继续使用原 manager secret 调用 RPC。
...
- Runtime Token 只对绑定的 Incus 实例有效，请求 IP 必须在当时通过指定通信网卡唯一反查到该实例；create/resume 轮换后旧 token 失效。
- Manager 不持久化 Runtime Token 明文，verifier 使用 SHA-256 和常量时间比较。
- Runtime Token 通过实例内 `0600` 文件交付；create/resume 刷新文件并重新运行 prepare/activate，普通 Manager 重启校验原文件且不轮换。
...
- Runtime HTTP API 从连接读取私网 source IP，并通过 Incus 指定通信网卡的当前地址唯一反查 Runtime identity，同时校验 Runtime Token 和 `codespace_uuid`；请求不接受客户端自报 identity header。
...
脚本原子写入专用 `known_hosts` 文件，并为 clone 和 workspace repository 的后续 Git 命令设置固定私钥、`IdentitiesOnly=yes`、该 known_hosts 文件和 `StrictHostKeyChecking=yes`。create 在确保公钥后执行 SSH clone；resume 仅在 workspace 当前 remote 为 SSH 时使用同一公钥刷新 `known_hosts` 和 Git 配置，不执行 clone、fetch 或 checkout。相同公钥可以重试，响应丢失不会产生第二把公钥。
...
- 只有当前已领取且租约有效的 create/resume 可以调用；Runtime Token、来源 Incus identity 和 Codespace binding 任一不匹配时在转发前拒绝。
- start/resume 脚本按固定私钥、公钥和 known_hosts 路径工作；start 脚本在需要 SSH clone 且密钥对均不存在时原子生成，create 重试与 SSH remote 的 resume 校验并复用同一公钥。
- Manager 只解析和转发公钥，不接收、记录或持久化私钥；响应只包含规范化 known_hosts 行。
- create 使用严格 Host Key 校验完成 clone，resume 刷新 Host Key 并保留 workspace 当前 HEAD。
...
- create/resume 输入全部来自 Incus exec 环境；create 有 repository payload 和两种 clone URL，resume 没有 repository payload。两者都先运行 init 取得凭据身份，再原子写入当前 Token 文件并运行 prepare/activate。
- HTTP helper 只匹配 binding repository 的规范化 clone URL，SSH 使用当前 Codespace 专用 Key，API 客户端使用 `server_url` 和同一 Token，服务端在两种 Git 入口都拒绝跨 repository 访问。
...
- Endpoint API 成功响应表示当前本地声明、路由和 metadata generation 已原子持久化；Gitea 暂时不可用不延迟本地成功响应。
- Runtime Endpoint API 拒绝不能安全组成 DNS label 或长度超过 30 的 `endpoint_id`。
- Runtime Endpoint API 对 label 执行合法 UTF-8、去除首尾 Unicode 空白、1 到 64 个 Unicode 字符以及禁止控制字符、`<`、`>` 的统一校验；非法值不提交本地快照或路由，合法中文保持去除首尾空白后的原值。Gitea 使用相同输入表得到一致结果和内容 hash。
...
- Runtime Metadata 上报失败时，Endpoint GET 和 Gateway 继续使用已提交的当前本地声明与路由；更高 generation 的变更可以继续提交并取代较旧待上报快照。
...
- Endpoint 对象始终包含明确的 `public` 布尔值；helper 默认提交 false，只有显式 `--public` 才提交 true，`workspace` 的 true 请求固定失败。
- 持有当前 Runtime Token 的进程可以提交普通 Endpoint 的 `public` 值；Gitea 页面不增加第二次确认或平行持久状态。stop 保留声明但关闭访问，resume ready 后重新发布原值。
- 公共 Endpoint 请求的 path 和 query 从 upstream 根路径转发，不增加独立 upstream path 配置。
- Runtime API 在 HTTP 和 HTTPS 配置下执行相同认证与状态逻辑，错误使用固定 JSON body 和状态码。
- Git SSH Key 与 Endpoint API 按 Manager 本地生命周期矩阵开放；认证与 binding 错误优先于阶段错误。Gitea 功能关闭期间已经在本地允许阶段提交的 Endpoint 变更保留并等待 metadata 重试，后续 Fetch abort 仍能结束 active create/resume。
...
- metadata generation 无法递增时不提交磁盘或内存部分结果，内部原因为 `version_exhausted`；Manager 关闭该 Codespace 准入并按 Incus 归属字段执行带 pending 的单对象清理。Runtime HTTP 调用方仍收到该接口固定的 `503 runtime_unavailable`，详细原因写入 Manager 诊断日志。
...
- Endpoint 支持 HTTP 和 WebSocket；SSH 使用独立接入面。
...
- session ID 使用安全随机源生成 32 字节并以不可预测字符串编码。HTTPS 外部地址使用 `__Host-gitea_codespace_session`，属性固定为 `Secure`、`HttpOnly`、`SameSite=Lax`、`Path=/` 且没有 Domain；HTTP 使用无 Secure 的 `gitea_codespace_session`。`gateway_cookie_secure` 的最终配置必须与 `gateway_url` 外部 scheme 一致，Cookie 名称由该 scheme 唯一确定。
...
- Runtime Endpoint 目标从根路径直接代理用户请求，不增加或剥离包含 Codespace/Endpoint 身份的业务 path。保留的 `/.gitea-codespace/open` 只用于 code 交换；内置 Web SSH 目标另外处理本节定义的根页面、静态资源和 terminal WebSocket，这些保留路径都不转发到 Runtime upstream。
...
- Gateway 在代理 Runtime Endpoint 前删除客户端提交的同名上下文和 `Forwarded/X-Forwarded-*` header，再根据当前访问方式和实际连接信息覆盖写入，Runtime 只能看到 Gateway 生成的可信值。认证访问写入 `X-Gitea-Codespace-Access: authenticated` 和 session 的用户 ID；公共访问写入 `X-Gitea-Codespace-Access: public`，省略用户 ID。两者都写入 UUID、Endpoint ID 和转发信息。
- Gateway 在代理 Runtime Endpoint 前删除自身 session cookie；内置 Web SSH 管理器直接接收 Gateway 已校验的内部 session context，不解析浏览器身份 header，也不接收原始 cookie。
- Gateway 不向 Endpoint 或 Web SSH 传递 `code`、Codespace Gitea Token、Manager Secret 或 Runtime Token。
- Gateway 在全部 Endpoint host 上先检查 Service Worker 标记；存在 `Service-Worker: script`、`Sec-Fetch-Dest: serviceworker` 或对应字段格式异常时固定返回 403，不连接 upstream。其他响应删除 `Service-Worker-Allowed`。普通 Web Worker、Shared Worker 和 Worklet 保持可用。**设计如此：Endpoint 的认证方式和实际目标可以在公共/认证、Runtime upstream/内置 Web SSH 之间切换，持久 Service Worker 不能跨这些变化继续控制同一 origin 或截获 Open Code。**
...
- `public=true` 的普通 Endpoint 在没有 session cookie 时可以访问；本地公共路由和 Gitea RPC 任一不允许时都不连接 upstream。
- 本地交互准入未开放时，保留的公共路由不能建立连接；当前 ready 重报和对象恢复完成后才允许新请求。
- 普通 HTTP 每次请求检查本地状态和最多 1 秒的新鲜公共 allowed，缺失时调用 Gitea；WebSocket 和长时间 HTTP 在一个复检周期内收敛。stop、delete、failed、删除路由或改为需要认证会立即使 allowed 失效并关闭连接。
- 公共请求不建立 session、不注入用户 ID、不更新用户交互和活跃时间，也不阻止自动暂停；请求只获得 `Access: public`、UUID、Endpoint 和转发信息。
