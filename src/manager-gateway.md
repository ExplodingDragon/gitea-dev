...
- capacity 是 Manager 通过 `DeclareManager` 上报到 `meta_json` 的展示字段，不是 Gitea quota，仅用于管理页面展示和诊断。
...
- Fetch 先返回当前 Manager 已领取但未在 `observed_operations` 中确认的 running payload，再领取新的 queued operation；running payload 重发刷新 lease 但不占新容量。
...
Manager 本地持久化当前运行侧事实：

```text
codespace_uuid -> backend Runtime identity
Runtime Token verifier 与绑定 source IP
Endpoint upstream declarations
当前 operation_rversion、operation 类型和执行阶段
每个 codespace 的 runtime_generation 和 metadata_generation
Manager 的 inventory_generation
每个 operation_rversion 的 boot session 结果
```

这些数据属于 Manager 后端状态，不写入 Gitea。Manager 重启时将本地记录与 backend scan 合并，再通过 inventory、metadata 和 operation 恢复接口与 Gitea 收敛。
...
入口统一先判断 `admin_state`，再判断 `runtime_state`。因此 disabled Manager 即使同时处于 recovering，也只具备上述 disabled 能力；active operation 冲突检查仍优先于主动 stopped 上报，已领取 stop 必须通过 `UpdateOperation(done)` 完成。
...
- 凭据更换由 Manager 使用安全随机源生成至少 256 位熵的新 secret 并先持久化，再使用旧 secret 认证调用 `RotateManagerSecret(new_manager_secret)`。Gitea 原子替换 hash/salt 后旧 secret 立即失效，Manager 后续请求使用新 secret；响应丢失时 Manager 已保存新 secret，不会因一次网络中断失去身份。
...
- create boot 信息由 Manager 根据 `CreateOperationPayload` 和 Manager 本地配置组合生成，包含完整 repository/ref/commit、owner、创建者和 token 数据。
...
- create 的 `gitea_token` 来自 Gitea `RequestGiteaToken`。resume 在 operation done、主状态进入 running 后由 Manager 单独申请并刷新 Runtime credential。
...
- 每个 create/resume boot session 完成后调用一次。
- 成功后 Manager 将 boot 结果作为 create/resume operation 的完成依据之一。
- 请求携带当前 `operation_rversion`；同一版本重复调用返回 conflict，不覆盖第一次结果，不同 resume operation 使用新的版本和 boot session。
- 请求内容包含：

| 字段 | 必填 | 说明 |
| --- | --- | --- |
| `success` | 是 | `true` / `false` |
| `operation_rversion` | 是 | 当前 create/resume operation 版本 |
...
| 字段 | 说明 |
| --- | --- |
| `workspace_head_sha` | 仅 create 返回；当前 HEAD SHA 必须与锁定 `commit_sha` 一致，resume 不提交该字段 |
...
- boot 完成后 Runtime 仍可管理 Endpoint，但同一 `operation_rversion` 的 `POST /boot` 仅接受一次。
...
- Endpoint upstream declarations 持久化在 Manager 本地；Manager 重启后先恢复 upstream，再重建 Gitea Runtime Metadata cache。
...
```text
302 Location: {安全 join(gateway_url, "/open")}?code={code}
```

规则：
- 使用 URL 路径安全拼接：若 `gateway_url` 末尾有 `/`，join 结果不会产生 `//open`。
- `gateway_url` 的校验规则见 [DeclareManager 声明校验](gitea-server.md#declaremanager)（absolute URL、不含 userinfo/query/fragment、path 可为空或固定 base path）。
...
```text
https://gateway.example.com/open?code=...
...
- 公钥认证确认 public_key_blob 归属 codespace 创建用户且 `Type == KeyTypeUser`（`models/asymkey/ssh_key.go`），部署密钥（`KeyTypeDeploy`）和授权主体（`KeyTypePrincipal`）不接受；若站点强制 2FA，用户必须已启用符合站点要求的 2FA。
- 创建用户当前允许登录。
...
- Gitea 在 create 前置校验阶段可通过 codespace 内部日志入口写入摘要；Manager 领取 operation 时从 payload 的 `log_offset` 继续追加。
...
- Manager 以 `FetchOperations` 返回的 `log_offset` 初始化当前 operation 的上传 offset；遇到 offset conflict 时重新 Fetch 当前 operation 取得服务端 offset，不从 0 覆盖已有内容。
...
- Manager 从 operation payload 的 `log_offset` 继续追加，Gitea 前置摘要和后续 operation 日志保持单文件连续。
