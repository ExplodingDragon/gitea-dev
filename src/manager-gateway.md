...
- Fetch 先恢复当前 Manager 的 running operation，再领取新的 queued operation；enabled Manager 和 disabled stop/delete 的普通 payload 重发刷新 lease 但不占新容量，disabled create/resume 返回 abort 且不刷新 lease。
...
同一 Codespace 的 boot 结果处理、Endpoint 变更和 Runtime Metadata 上报使用一个提交顺序。Manager 一次只推进一个尚未被 Gitea 接受的 metadata generation：相同 boot 结果或相同 Endpoint mutation 的重试继续提交该 generation；不同的 boot 结果、不同 Endpoint mutation 或其他会产生新快照的请求返回 `409 operation_conflict`。周期 TTL 重发也先重试 pending generation，不能越过它提交旧快照或新 generation。Gitea 接受当前 generation 后，Manager 才处理下一项变更。这个顺序保证本地路由、boot 状态和 Gitea Metadata 对同一 Codespace 的观察顺序一致，不需要增加队列字段或新协议。

stop/delete operation、failed fact 或其他使当前 Runtime API 阶段失效的生命周期变化，与 pending metadata mutation 使用同一 Codespace 本地锁确定顺序。metadata generation 已先被 Gitea 接受时，mutation 正常完成后再执行生命周期变化；尚未接受时，Manager 取消该 pending 重试，等待中的 Runtime API 请求返回 `409 operation_conflict`。已递增的 `metadata_generation` 作为本地高水位保留，不回退也不复用；以后重建 metadata 使用更高值，因此响应丢失时 Gitea 无论是否已接受旧值都能收敛。
...
- Report 临时失败时保留本地结果、返回 `503 backend_unavailable` 并在后台重试相同 generation；Runtime 重试完全相同的 mutation 时按当前规范化结果幂等继续上报，任何不同的 Endpoint 或 boot mutation 都返回 `409 operation_conflict`。DELETE 在本地对象已不存在但该 generation 尚未被 Gitea 接受时继续幂等上报；接受后才返回 204，临时失败仍返回 503。
- stop/delete/failed 等生命周期变化先成立时，取消未接受的 pending mutation，返回 `409 operation_conflict`，并保留已递增的 generation 高水位。
