...
- `gateway_ssh_addr`
- `tags`
- 可选诊断 `meta_json`（含 `last_capacity_total`、`last_capacity_available` 等容量快照）
...
- `public_key_fingerprint` 和 `public_key_algorithm` 仅日志诊断和审计使用，授权判定依据为 `public_key_blob` 归属验证。
