...
这样设计的原因是注册入口可以直接复用 Gitea 的 owner 身份模型。个人用户和组织使用同一字段，设置页也能按当前上下文直接定位对应的注册入口。
...
这样设计的原因是 Manager ID 提供稳定身份定位，Manager Secret 提供身份认证。认证逻辑集中在 ManagerService interceptor 中，可以让所有注册后 RPC 使用同一认证路径。
...
Registration Token 明文保存，是因为它是管理员复制给 Manager 完成首次注册的入口凭据。唯一索引用于让 `RegisterManager` 通过提交的 token 直接定位注册入口。Registration Token 和 Manager Secret 承担不同职责，可以让注册入口和已注册 Manager 通信身份分别管理。
...
label 只承担展示职责，因此输入校验关注 UI 可读性和 HTML 展示安全。路由和授权使用 `endpoint_id`，可以避免用户修改 label 后影响 Gateway 转发或日志关联。
...
sequenceDiagram
    ...
    U->>G: open endpoint
    G->>G: check access and endpoint
    alt not allowed
        G-->>U: failure category
    else allowed
        G-->>U: redirect with open token
        U->>W: GET open token
...
这样设计的原因是 HTTP/WebSocket 覆盖 Web IDE 和端口预览主场景。第一版不提供任意 TCP tunnel，可以减少鉴权和资源占用复杂度。Gateway 统一终止 TLS，可以集中管理证书、access log 和 session。Open Token 只用于换取 Gateway session，避免一次性 bearer token 泄漏到 Runtime 或后续浏览器请求中。
...
这样设计的原因是 TTL 限制长期遗留 session，idle timeout 控制资源占用，周期 revalidate 用于处理用户登录状态、codespace 状态和 Manager 状态变化，同时减少每个请求回到 Gitea 的开销。stop/delete/disabled 是明确管理事件，由 Manager/Gateway 本地事件立即关闭连接。
...
这样设计的原因是 SSH 是长连接交互面，只有 running 状态能保证 internal SSH metadata 与 Manager/Gateway 转发同时成立。stopped 自动唤醒会把认证尝试变成生命周期操作，容易让普通 SSH 客户端重试触发意外资源启动。
...
sequenceDiagram
    ...
    C->>W: connect cs uuid
    W->>W: parse uuid and key
    W->>G: VerifySSHPublicKey
    alt auth rejected
        W-->>C: auth failed
    else auth accepted
        W->>W: read internal SSH metadata
        alt metadata missing
            W-->>C: auth failed
        else metadata ready
            W->>R: connect internal sshd
            R-->>W: sshd ready
            W->>W: record live session
            C->>W: SSH channel
            W->>R: forward channel
            R-->>W: channel response
            W-->>C: channel response
        end
    end
```
...
内部 SSH 使用 Gateway 固定密钥，是为了把用户认证放在 Gitea/Gateway 边界完成，Runtime Instance 只信任 Manager deployment 内部通道。这样 Runtime 不需要保存用户 SSH key，也能在用户登录状态变化后由 Gitea 实时返回 SSH 认证失败分类。
...
codespace 日志是生命周期操作的执行证据，单文件连续追加。只有当前 `operation_status=running` 且 `operation_rversion` 匹配时允许追加，active operation 清空后封闭。连接级事件保留在 Manager/Gateway 本地日志，是因为这些事件数量大、包含网络诊断细节，放入 Gitea codespace 日志会干扰用户阅读生命周期过程。
...
这样设计的原因是 Gateway access log 面向运维排障和访问记录，不是用户生命周期日志。JSON line 便于接入 Loki/ELK 等日志系统。记录模板、hash 和失败分类可以满足排障需求，同时降低 token、cookie、query 参数和用户隐私泄漏风险。
