...
- user access lost 后，新的 open 由 Gitea `ValidateOpenToken` 返回对应失败分类。
...
- user access lost 后，新 session 由 Gitea 返回对应失败分类；已建立 session 在下一次周期 revalidate 时关闭。
...
- user access lost 后，新的 SSH auth 由 Gitea `VerifySSHPublicKey` 返回对应失败分类。
