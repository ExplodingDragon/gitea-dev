...
- registration token 明文只在创建或重置时展示一次。
...
统一 Manager 池通过 tag 匹配。`admin_state` 控制管理态（disabled Manager 仅能执行已有任务的 stop/delete 和状态分歧上报）；`last_online_unix` + timeout 推导在线态。
