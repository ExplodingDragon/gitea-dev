...
- 用户 Endpoint 访问入口
- 用户 SSH 访问入口
- 通过 Codespace Manager 身份或 Manager 内部调用消费 Gateway Access Ticket
...
flowchart LR
    ...
    Gateway -->|"ValidateAccessTicket / SSH auth<br/>via Manager identity"| ManagerRPC
...
flowchart TB
    ...
    GatewayIngress -->|"ticket / SSH auth validation<br/>Manager identity"| GiteaWeb
...
sequenceDiagram
    ...
    Gitea-->>User: redirect to Gateway with access ticket
    User->>Gateway: open Endpoint
    Gateway->>Gitea: ValidateAccessTicket via Manager identity
...
| <a id="term-codespace-gateway"></a>Codespace Gateway | Codespace Manager deployment 下的接入组件，负责用户 Endpoint 与 SSH 接入，并通过 Codespace Manager 身份或 Manager 内部调用消费 Gateway Access Ticket。 |
...
| <a id="term-endpoint"></a>Endpoint | Runtime Instance 声明给 Codespace Manager 的可打开入口，使用 `endpoint_id` 标识；协议和 upstream 由 Codespace Gateway 与 Codespace Manager 解释。 |
| <a id="term-reserved-endpoint"></a>Reserved Endpoint | 保留的 Endpoint ID；`workspace` 表示默认 Web IDE。 |
| <a id="term-gateway-access-ticket"></a>Gateway Access Ticket | Gitea 为打开 Endpoint 签发的一次性短期票据。 |
...
- `queued` 表示请求已创建，但尚未被 Codespace Manager 领取 operation
...
- repository 删除后，global / owner scope 下引用该 `repo_id` 的 codespace 由 State Reconciliation 进入 cleanup 或 error，不在 repository 删除入口同步阻塞
...
6. 创建短期 access ticket
7. 重定向到 [Codespace Gateway](#term-codespace-gateway)
8. Codespace Gateway 通过 Codespace Manager 身份或 Manager 内部调用消费 `ValidateAccessTicket`
9. 认证通过后跳转到对应 Endpoint
...
Gateway 收到 SSH 密码后，不本地校验，直接委托 Gitea 认证。
...
- SSH port forwarding 只允许访问当前 codespace 的内部网络范围
- 不允许借 Gateway 访问 Gateway 宿主机、控制面内网或其他 codespace 网络
...
- repository 为 `empty` 时不允许 create
...
[Manager Selection](#term-manager-selection) 规则：

1. 先查 repo scope 是否存在 `repo_id = 当前 repository.id` 的 Codespace Manager
2. 若 repo scope 存在 Codespace Manager，则只在该 scope 候选 Codespace Manager 中继续筛选
3. 若 repo scope 不存在 Codespace Manager，则查找 owner scope 的 `owner_id = repository.owner_id` Codespace Manager
4. 若 owner scope 存在 Codespace Manager，则只在该 scope 候选 Codespace Manager 中继续筛选
5. 若 owner scope 不存在 Codespace Manager，则回退到 `global` scope
6. 在命中的 scope 中，再过滤：
   - Codespace Manager 在线
   - Codespace Manager 支持 `repo.tag`
7. 若命中的 scope 没有可调度 Codespace Manager，则创建失败
8. 只有当前 scope 根本不存在 Codespace Manager 时，才继续向外层回退
...
- 现有 codespace 在创建时固定绑定 `manager_id`、`scope_type`、`scope_id`
- 后续 stop / resume / delete 仍回到原 manager
...
用户点击 Create
  ...
  -> Gitea 按 `repo -> owner -> global` 执行 Manager Selection
  -> Gitea 固定记录 manager_id、scope_type、scope_id
  -> Gitea 创建 codespace，status=queued
  -> Gitea 创建 create operation
  -> Gitea redirect /codespace/{uuid}
  -> Codespace Manager 领取 create operation
  -> Gitea 状态进入 booting
  -> Codespace Manager 按 repo.tag 查询本地配置，决定 Incus / Docker 后端、Runtime Instance 和 bootstrap
...
用户点击 Delete
  ...
  -> Codespace Manager 上报结果
  -> Gitea 吊销 Gitea token
  -> Gitea 删除 access ticket / operation / codespace 记录和 codespace 日志文件
```
...
- delete 成功后直接物理删除 codespace、operation、access ticket 与 codespace 日志文件
...
### Gateway Access Ticket

用于用户从 Gitea 跳转到 Codespace Gateway。

- 对应术语是 [Gateway Access Ticket](#term-gateway-access-ticket)
- 短期有效
- 一次性消费
- 绑定 user / codespace / endpoint_id
- 由 Codespace Gateway 通过 Codespace Manager 身份或 Manager 内部调用消费
- 不授予 git 权限
...
- 申请 Gitea token
- 通过 Codespace Manager 身份或内部调用为 Codespace Gateway 校验 access ticket
- 委托 Gitea 校验 SSH 密码
- 委托 Gitea 校验 SSH 公钥
...
Gitea 与 Codespace Manager 之间不提供 REST 控制面旁路，所有 Codespace Manager 到 Gitea 的任务、日志、Runtime Metadata、token、SSH 校验和 access ticket 校验都走该 RPC 服务。
...
- `DeclareManager` 用于 Codespace Manager 启动后声明版本、tags、SSH 能力、gateway 地址与只读诊断能力
- `FetchOperation` 只返回当前 Codespace Manager 可领取的 operation
- `UpdateOperation` 用于续租、阶段推进与最终结果上报，但不直接绕过 Gitea State Finalization
- `UpdateLog` 必须按 offset 追加，不允许产生日志空洞
- `ReportRuntimeMetadata` 只更新 Runtime Metadata 到 `meta_json`
- `RequestGiteaToken` 只允许 create / resume active operation 申请
- `ValidateAccessTicket` 只用于 Codespace Gateway 通过 Codespace Manager 身份或 Manager 内部调用消费短期打开票据
...
- Codespace Gateway 根据 Codespace Manager 维护的 Runtime Metadata，以 `codespace_uuid + endpoint_id` 在内部解析真实 upstream
...
- `endpoint_id` 由 Codespace Manager 上报并负责唯一
- `endpoint_id` 只能包含大小写字母、数字、中划线、下划线
