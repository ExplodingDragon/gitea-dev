...
- Manager 不得用自己的身份访问 repository 内容。
...
设计决策：Gateway Open Token 不持久化到数据库，纯随机 + sha256 即可满足一次性校验需求，不需要现有 access_token 的 pbkdf2 方案（`HashToken`），也不需要 HMAC 绑定 SECRET_KEY。
...
Cache 丢失即 token 失效，用户重新从 Gitea 发起 open 即可。
...
Manager 保证 Runtime Instance 存在兼容 OpenSSH 的 sshd。
...
Gateway 每次 SSH 认证尝试都调用 Gitea。Gateway 不得跨连接缓存密码或公钥认证成功结果。
...
- 请求中的 `public_key_fingerprint` 和 `public_key_algorithm` 仅用于日志诊断，可以为空，不作为认证判断依据。
...
- 使用统一 Manager 池，repository 只需要通过 tag 表达运行侧能力需求。
...
- Manager 是实际运行容量权威，自行保证不会拉取超过本地真实容量的 create/resume operation。
...
- Gitea 看不到 Manager 本地 Runtime 队列、资源占用和启动中任务，不能作为真实容量权威，只保存最近容量快照用于 UI、诊断和本次 `FetchOperation` 准入检查。
...
- Runtime cleanup 通过 codespace delete operation 表达，不能由删除 Manager 记录隐式触发。
