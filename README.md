...
- Gitea 应向创建者和具备管理权限的组织管理员发送站内通知；通知不阻塞 repository 删除提交
...
Gateway 与 Gitea 都必须对 SSH 认证做限流。

规则：

- 限流维度至少包含 source IP、`ssh_user`、`codespace_uuid`、Gitea `user_id`
- 密码认证失败必须触发退避
- 公钥认证失败必须触发退避
- TOTP 格式错误或消费失败按密码认证失败处理
- 同一连接内连续失败达到阈值后，Gateway 必须断开连接
- 同一 source IP 或同一 `ssh_user` 在窗口期内失败过多时，Gateway 必须延迟响应或临时拒绝
- Gitea 的 `VerifySSHPassword` 与 `VerifySSHPublicKey` 必须执行服务端限流，不能只依赖 Gateway
- Gitea 服务端限流必须在实际密码 / TOTP / 公钥校验前执行，避免暴力认证直接打满上游校验路径
- 失败审计只记录 user、codespace、source IP、认证方式、失败原因类别和时间，不记录密码、TOTP 或公钥原文
- 限流与退避不改变 Gitea 作为认证权威的事实，不能用缓存绕过上游实时校验
...
CREATE_RATE_LIMIT_PER_USER = 10/h
CREATE_RATE_LIMIT_PER_IP = 30/h
```
...
- create rate limit 是滥用保护，不替代 quota
...
- Codespace Manager 重启后继续处理同一 operation 时，必须重新加载或重建该 operation 的必要 mask set；无法确认脱敏安全时不得继续上传原始日志
...
SSH_AUTH_MAX_FAILURES_PER_CONNECTION = 3
SSH_AUTH_RATE_LIMIT_PER_IP = 30/5m
SSH_AUTH_RATE_LIMIT_PER_USER = 10/5m
SSH_AUTH_RATE_LIMIT_PER_CODESPACE = 10/5m
...
- `SSH_AUTH_*` 同时约束 Gateway 侧认证入口和 Gitea `VerifySSHPassword` / `VerifySSHPublicKey` 服务端校验
- SSH 认证限流与退避只限制认证请求频率，不引入认证缓存或预认证 token
