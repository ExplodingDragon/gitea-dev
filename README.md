...
- Codespace 跟随 Gitea 当前部署模型；Gitea 本身不提供集群模式，本文档不定义跨 Gitea Web 节点的共享 cache、共享 session 或 token 消费语义。
...
- Runtime Instance 可以访问 Gitea 标准数据面能力，例如 Git HTTP、repository web URL 和其他非 codespace 专用入口。
...
- Gitea 没有内建集群控制面，Codespace 不单独发明跨 Web 节点一致性协议。
- 用户 Endpoint、WebSocket 和 SSH channel 是长连接流量，不适合让 Gitea Web 进程代理。
- Manager/Gateway 与 Runtime Instance 在同一运行侧部署内，能直接解析 upstream 和内部 SSH 连接；Gitea 保持为短路径鉴权与状态权威。
...
| Manager Capacity | Manager 最近上报的本地 create/resume 可接收能力快照，仅用于展示、诊断和 `FetchOperation` 准入判断。 |
...
| Manager Secret | Codespace Manager 调用 ManagerService RPC 的长期凭据。 |
...
- Runtime git 访问必须使用基于创建用户当前权限签发的 Gitea access token，并且只走 Git HTTP(S)。
...
- codespace 不设计 quota。Manager 的真实并发容量由 Manager 自己控制，Gitea 不维护运行容量计数。

这样设计的原因：

- Codespace 使用创建用户的权限和 token 访问 repository，因此它是用户私有开发环境，不是 repository 共享资源。
- create 失败后 Runtime、token、日志和 generation 可能已经部分产生；不在同一对象上 retry 可以避免旧运行时状态与新初始化过程混杂。
- quota 和运行容量属于 Manager 本地资源管理问题，Gitea 只保存生命周期状态和授权边界。
...
open_token_key = HMAC(Gitea SECRET_KEY, "codespace open token")
open_token = base64url(HMAC(open_token_key, random_bytes + issued_unix_nano + token_salt))
token_hash = sha256(open_token)
```

规则：

- Gateway Open Token 使用从 Gitea `SECRET_KEY` 派生的专用 key，不直接拼接或暴露原始 `SECRET_KEY`。
...
- Gateway Open Token 是短期一次性跳转凭据，不是需要恢复的业务状态。
- cache 丢失只会要求用户重新 open，不会改变 codespace 生命周期状态。
- 不写数据库可以避免高频短生命周期 token 带来的 DB 写入、索引和清理成本。

Gateway session 规则：

- Gateway 必须维护 `codespace_uuid -> live sessions` 的本地索引。
- Gateway 和 Manager 是同一 deployment 内的一体化组件。
- Manager 执行 stop/delete 前，必须先通知本地 Gateway 关闭该 `codespace_uuid` 的 HTTP/WebSocket/IDE 会话。
- Manager disabled 后，本地 Gateway 必须拒绝新 open，并关闭该 Manager 负责的 live sessions。
...
- 认证者是 codespace 创建用户。
- 创建用户当前允许登录。
- 密码认证必须满足 password 与 TOTP 要求。
- 公钥认证不再要求输入 TOTP；若站点强制 2FA，用户必须已经启用符合站点要求的 2FA。
- WebAuthn-only 用户不能使用 SSH 密码认证。
- repository access precondition 仍通过。
- 绑定 Manager 当前在线且未被 disabled。
- 密码认证被该 codespace 允许。
- 本地密码和 TOTP 使用 Gitea 现有校验与消费逻辑。
- 公钥认证必须解析 `public_key_blob`，并确认其通过 Gitea 现有 SSH key 模型归属于创建用户。
- 请求中的 `public_key_fingerprint` 和 `public_key_algorithm` 仅用于日志诊断，可以为空，不能作为认证判断依据。
- `public_key_blob` 解析失败返回 `invalid_credentials`。
- Gateway 不得只传 fingerprint 让 Gitea 查 key。

Gateway 必须至少按 source IP、`ssh_user`、`codespace_uuid` 做限流和退避。Gitea 不新增通用 rate limiter。

Gitea 可以向 Gateway 返回失败分类用于日志和退避。Gateway 对 SSH client 只能返回统一认证失败。

SSH session 规则：

- Gateway 必须维护 `codespace_uuid -> live SSH sessions` 的本地索引。
- Manager 执行 stop/delete 前，必须先通知本地 Gateway 关闭该 `codespace_uuid` 的 SSH sessions。
- Manager disabled 后，本地 Gateway 必须拒绝新 SSH，并关闭该 Manager 负责的 live SSH sessions。
...
- `.gitea/codespace.yaml` 只用于选择 Manager tag，不用于决定实际 checkout 内容。
- tag/commit 场景读取 default branch，可以避免任意历史 commit 改变 Manager 选择。
- PR 场景使用 base branch，是为了让目标仓库维护者控制 codespace 运行侧选择；实际代码仍按用户选择的 ref 锁定到具体 commit SHA。
...
- Manager 不得把 `CODESPACE_NAME` 当 Runtime Instance name 的唯一来源。
...
- 创建或重置 registration secret 时，旧的未消费 registration secret 必须置为 inactive。
- `RegisterManager` 消费 registration secret 时必须在数据库事务内校验 active、未过期、未消费，并写入 `consumed_unix`。
- 并发重复消费只能有一个成功。
...
- `gateway_url` path 允许为空或作为固定 base path；生成 open URL 时必须安全 join `/open`。
...
- 当前设计使用统一 Manager 池，repository 只需要通过 tag 表达运行侧能力需求。
- 取消 owner/global/repository scope 顺序，可以避免 repository、owner/org 删除流程与 Manager 生命周期互相耦合。
...
- claim 条件必须包含 caller Manager enabled、caller Manager 支持 `repo_tag`、本次 `FetchOperation` 声明可接收 create、`manager_id=0`、当前 status、active operation 和 generation。
...
- Manager 是实际运行容量权威，必须自行保证不会拉取超过本地真实容量的 create/resume operation。
- Gitea 只保证 operation 不被重复领取，不负责保护 Manager 的本地运行并发。
...
- Gitea 看不到 Manager 本地 Runtime 队列、资源占用和启动中任务，不能作为真实容量权威。
- Gitea 只保存最近容量快照，用于 UI、诊断和本次 `FetchOperation` 准入判断。
...
- disabled Manager 必须拒绝 `FetchOperation(create|resume)`、`ReportRuntimeMetadata`、`RequestGiteaToken`、`ValidateOpenToken`、`VerifySSHPassword` 和 `VerifySSHPublicKey`。
...
- request 必须包含 `capacity_total`、`capacity_available` 和 `accepted_operation_types`。
- 更新 `last_online_unix`。
...
- `capacity_available=0` 时不得返回 `create|resume`。
...
- 必须匹配 `operation.uuid`、`operation.manager_id`、`codespace.active_operation_id` 和 `codespace.generation`。
- operation 已进入 `done|failed` 后终态不可再改变。
- 重复提交相同终态可以幂等返回当前结果，但不得重复执行 State Finalization。
...
- 永远不写主状态。
- 必须校验调用方 Manager 已通过认证。
- 必须校验 `codespace.manager_id == caller.manager_id`。
- 必须校验 `generation == codespace.generation`。
...
- PR 场景下 `create|resume` payload 必须同时包含 base/head clone URL 与 web URL。
...
- lifecycle operation 进入 `done|failed` 并完成 State Finalization 后，`active_operation_id` 必须清空。
...
- Runtime Instance 只需要获取初始化信息和声明可打开入口。
- 生命周期状态、Gitea token、日志和 Runtime Metadata 都由 Manager 统一转接到 Gitea，Runtime 不直接承担 Gitea 控制面语义。
- 接口面保持在 boot/endpoints，可以减少 Runtime 与 Gitea 设计的耦合。
...
- `success=true` 时，必须满足 workspace checkout 到锁定 `commit_sha`。
- `success=true` 时，必须满足 `workspace_head_sha == commit_sha`。
- `success=true` 时，internal SSH 信息必须完整，且 internal SSH 可被 Gateway 连通。
...
- `DELETE /endpoints/{endpoint_id}` 删除 Endpoint；不存在固定返回 204。
- `endpoint_id` 必须匹配 `[A-Za-z0-9_-]+`。
...
- `upstream_*` 只保存在 Manager/Gateway 内部状态，不上报 Gitea。
...
- Gitea 永远不保存 upstream。

这样设计的原因：

- upstream 是 Manager/Gateway 内部网络细节，可能包含私网地址、端口和本地路径。
- Gitea 只需要 Endpoint 是否存在、展示 label 和可选 health，用于 UI 与 open 前置校验。
- 不保存 upstream 可以避免内部网络拓扑进入数据库、API 或普通 UI 输出。
...
- Runtime Instance 永远不直接调用 Gitea 上报 Runtime Metadata。

这样设计的原因：

- Runtime Metadata 是动态观测数据，不是 lifecycle 事实。
- cache miss 只影响展示和交互入口，不应推进或回滚主状态。
...
- 这样做的原因是 Gitea 现有 access token scope 只有 category 级，没有单仓库语义；把“能做 repository 类操作”和“只能访问哪个 repository”拆开，才能在不扩展通用 token 系统的前提下得到可执行且可追踪的边界。
...
- Git HTTP 认证链路必须识别 codespace-bound token。
- repository 访问只在现有支持 token/basic auth 的入口做 repo binding 校验。
- API v1 repository routes 必须在 `APIContext.TokenCanAccessRepo(repo)` 或同等公共入口追加 codespace-bound token 校验。
- Web/git HTTP 必须在 `CheckRepoScopedToken(ctx, repo, level)` 或同等公共入口追加 codespace-bound token 校验。
- 显式启用 `AllowBasic` 或 `AllowOAuth2` 的 repository HTTP 路径必须复用上述公共校验入口。
...
- 同一 generation 重复 `RequestGiteaToken` 时，Gitea 必须先吊销旧 token，再签发新 token，并更新 `codespace.gitea_token_id`。
...
- 手动删除返回：
  ...
  - API：`409 Conflict`。
- 删除保护必须在 `DeleteAccessTokenByID` 或其统一服务入口执行，Web、API 和当前 token 删除都必须走同一判断。
...
- `DeleteAccessTokenByIDForCodespaceLifecycle` 必须在同一事务内清空或更新 `codespace.gitea_token_id`。
...
- Gitea 不需要理解 Runtime Token，也不应把 Runtime Token 纳入 Gitea token 生命周期。
...
- 重置 registration secret 只影响尚未消费的注册凭据，不会、也不应该影响任何现有 Manager 的 manager secret。
...
- repository 删除成功页或确认摘要必须展示受影响的 codespace 数量。
- repository 删除必须在 `repo_service.DeleteRepository` / `DeleteRepositoryDirectly` 删除 DB repository 记录前执行 codespace pre-cleanup。
- 对引用该 repository 的关联 codespace，repository 删除事务内必须：
  - 吊销 Gitea Token。
  - 禁止 open/SSH/resume。
...
- repository DB 记录删除后，Manager 仍必须能通过 `codespace_uuid + generation` 领取 delete operation 并完成 Runtime cleanup。
- source repository 删除后，相关 codespace 列表和详情页显示 `source repository deleted`。
- repository 删除不发送站点通知。

Owner/user/org 删除：

- owner 删除前，Gitea 现有流程必须先处理该 owner 下 repositories。
...
- 物理删除 Manager 记录前必须确认没有 active operation 绑定该 Manager。
...
- Manager 记录被物理删除后，后续 ManagerService RPC 必须按 unregistered manager 返回 unauthenticated。

这样设计的原因：

- Runtime cleanup 必须通过 codespace delete operation 表达，不能由删除 Manager 记录隐式触发。
- Manager 记录被物理删除后，Gitea 已无法可靠认证该 Manager 或下发清理指令。
- 因此物理删除 Manager 前必须先清理引用它的 codespaces 和未完成 operations。
...
- `last_capacity_total / last_capacity_available` 是 Manager 最近通过 `DeclareManager` 或 `FetchOperation` 上报的容量快照，用于 UI、诊断和 `FetchOperation` 准入判断；不是 Gitea 运行容量计数，不会因 claim、`done|failed` 或 timeout 自动变化。
...
- `active_operation_id` 只表示当前未完成 lifecycle operation；operation 进入 `done|failed` 并完成 State Finalization 后必须清空。
...
- `last_active_unix` 只用于 UI 排序和清理参考，不作为权限或状态权威。
- `status_message` 最长 1024 字符，超长截断，禁止控制字符。
- `status_message` UI 必须按普通文本 escape 后展示。
...
- cache 内容都是短期、可失效或可由 Manager 重建的数据。
- cache 丢失不会改变 codespace 生命周期、权限、删除闭环或 operation 超时判断。
...
- operation log 是生命周期操作的执行证据，必须在 operation 完成前封闭，避免终态之后继续改变该 operation 的可见输出。
- running 期间没有未完成 lifecycle operation；为了连接事件引入空 `operation_uuid`、长期 runtime operation 或第二套 Gitea 日志都会扩大状态模型。
- Gitea 对运行期连接只需要保存授权结果影响到的权威状态，例如 `last_active_unix`；排障细节属于 Manager/Gateway 本地日志。
